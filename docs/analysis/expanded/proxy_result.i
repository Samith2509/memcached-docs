...
int mcplib_response_raw_string(lua_State *L) {
    mcp_resp_t *r = luaL_checkudata(L, -1, "mcp.response");
    const char *buf = (r->cresp ? r->cresp->iov[0].iov_base : r->buf);
    if (buf) {
    ...
    if (reresp.type == 101) {
        ...
        while (pos != end) {
            if (*pos == flag && (pos == reresp.rline || *(pos-1) == ' ')) {
                while (pos != end && !((*__ctype_b_loc ())[(int) ((*pos))] & (unsigned short int) _ISspace)) {
                    *pos = ' ';
                    pos++;
                }
                lua_pushboolean(L, 1);
