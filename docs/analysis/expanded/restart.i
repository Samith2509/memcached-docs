...
struct _restart_data_cb {
    void *data;
    struct _restart_data_cb *next;
    restart_check_cb ccb;
    restart_save_cb scb;
    char tag[255];
};
static int mmap_fd = 0;
static void *mmap_base = ((void *)0);
static size_t slabmem_limit = 0;
char *memory_file = ((void *)0);
static restart_data_cb *cb_stack = ((void *)0);
void restart_register(const char *tag, restart_check_cb ccb, restart_save_cb scb, void *data) {
...
static int restart_save(const char *file) {
    ...
    mode_t oldmask = umask(~(0400 | 0200));
...
void restart_set_kv(void *ctx, const char *key, const char *fmt, ...) {
    ...
    int vlen = vsnprintf(valbuf, 4096 -1, fmt, ap);
    __builtin_va_end(ap);
    if (vlen >= 4096) {
...
bool restart_mmap_open(const size_t limit, const char *file, void **mem_base) {
    ...
    mmap_fd = open(file, 02|0100, (0400|0200|0100));
    if (mmap_fd == -1) {
    ...
    mmap_base = mmap(((void *)0), limit, 0x1|0x2, 0x01, mmap_fd, 0);
    if (mmap_base == ((void *) -1)) {
...
void restart_mmap_close(void) {
    msync(mmap_base, slabmem_limit, 4);
...
unsigned int restart_fixup(void *orig_addr) {
    ...
    while (checked < slabmem_limit) {
        ...
        if (it->it_flags & (32|64)) {
            item_chunk *ch;
            if (it->it_flags & 32) {
                ch = (item_chunk *) ((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0));
