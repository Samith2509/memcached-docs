...
auth_t main_auth_entries[8];
int entry_cnt = 0;
char *main_auth_data = ((void *)0);
pthread_mutex_t lock = { { 0, 0, 0, 0, PTHREAD_MUTEX_TIMED_NP, 0, 0, { ((void *)0), ((void *)0) } } };
enum authfile_ret authfile_load(const char *file) {
    struct stat sb;
    char *auth_data = ((void *)0);
    auth_t auth_entries[8];
    ...
    int used = 0;
    while ((fgets(auth_cur, auth_end - auth_cur < 256 ? auth_end - auth_cur : 256, pwfile)) != ((void *)0)) {
        int x;
        int found = 0;
        for (x = 0; x < 256; x++) {
            if (!found) {
