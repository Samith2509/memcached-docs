...
static int _process_request_mset(mcp_parser_t *pr) {
    ...
    _process_request_key(pr);
    int vlen = 0;
    if (mcmc_token_get_32(pr->request, &pr->tok, 2, &vlen) != 0) {
        return -1;
    }
    if (vlen < 0 || vlen > (0x7fffffff - 2)) {
       return -1;
    }
    vlen += 2;
    pr->vlen = vlen;
...
static int _process_request_noreply(mcp_parser_t *pr) {
    ...
    if (len == sizeof("noreply")-1 && strncmp("noreply", tok, sizeof("noreply")-1) == 0) {
        pr->noreply = true;
    }
    return 0;
}
static int _process_request_storage(mcp_parser_t *pr) {
    ...
    _process_request_key(pr);
    int vlen = 0;
    if (mcmc_token_get_32(pr->request, &pr->tok, 4, &vlen) != 0) {
        return -1;
    }
    if (vlen < 0 || vlen > (0x7fffffff - 2)) {
       return -1;
...
int process_request(mcp_parser_t *pr, const char *command, size_t cmdlen) {
    ...
    const char *s = _Generic (0 ? (command) : (void *) 1, const void *: (const void *) (memchr (command, ' ', endlen)), default: memchr (command, ' ', endlen));
    if (s != ((void *)0)) {
        cl = s - command;
    } else {
        cl = endlen;
...
bool mc_toktou32(mcp_parser_t *pr, int token, uint32_t *val) {
    if (mcmc_token_get_u32(pr->request, &pr->tok, token, val) != 0) {
        return false;
    }
    return true;
}
bool mc_tokto32(mcp_parser_t *pr, int token, int32_t *val) {
    if (mcmc_token_get_32(pr->request, &pr->tok, token, val) != 0) {
        return false;
    }
    return true;
}
...
bool mc_parse_exptime(mc_resp *resp, mcp_parser_t *pr, int token, rel_time_t *exptime) {
    ...
    if (mcmc_token_get_32(pr->request, &pr->tok, token, &exptime_int) != 0) {
        pout_string(resp, "CLIENT_ERROR invalid exptime argument");
        return false;
    }
    *exptime = realtime((exptime_int < 0) ? 60*60*24*30 + 1 : exptime_int);
    return true;
}
...
static int _store_item_copy_from_buf(item *d_it, char *buf, const int len) {
    if (d_it->it_flags & 32) {
        ...
        memcpy(((char*) &((d_it)->data) + (d_it)->nkey + 1 + (((d_it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((d_it)->it_flags & 2) ? sizeof(uint64_t) : 0)), buf, len);
    }
    return 0;
}
static int _meta_flag_preparse(mcp_parser_t *pr, const size_t start,
    ...
    int32_t tmp_int;
    uint8_t seen[127] = {0};
    of->key = (&(pr)->request[(pr)->tok.tokens[(pr)->keytoken]]);
    of->key_len = pr->klen;
    if (pr->klen > 250) {
        *errstr = "CLIENT_ERROR bad command line format";
        return -1;
    }
    if (pr->tok.ntokens > 20) {
    ...
    for (i = start; i < pr->tok.ntokens; i++) {
        ...
        switch (o) {
            case 'b':
                ret = base64_decode((unsigned char *)(&(pr)->request[(pr)->tok.tokens[(pr)->keytoken]]), pr->klen,
            ...
            case 'N':
                of->locked = 1;
                of->vivify = 1;
                if (mcmc_token_get_flag_arg_32(pr->request, &pr->tok, i, &tmp_int) == 0) {
                    of->autoviv_exptime = realtime((tmp_int < 0) ? 60*60*24*30 + 1 : tmp_int);
                } else {
            ...
            case 'T':
                of->locked = 1;
                if (mcmc_token_get_flag_arg_32(pr->request, &pr->tok, i, &tmp_int) == 0) {
                    of->exptime = realtime((tmp_int < 0) ? 60*60*24*30 + 1 : tmp_int);
                    of->new_ttl = true;
                } else {
                    *errstr = "CLIENT_ERROR bad token in command line format";
                    of->has_error = 1;
            ...
            case 'R':
                of->locked = 1;
                if (mcmc_token_get_flag_arg_32(pr->request, &pr->tok, i, &tmp_int) == 0) {
                    of->recache_time = realtime((tmp_int < 0) ? 60*60*24*30 + 1 : tmp_int);
                } else {
                    *errstr = "CLIENT_ERROR bad token in command line format";
                    of->has_error = 1;
                }
            ...
            case 'x':
                ...
                break;
            case 'F':
                if (mcmc_token_get_flag_arg_u32(pr->request, &pr->tok, i, &of->client_flags) != 0) {
                    of->has_error = true;
                }
                break;
            case 'C':
                if (mcmc_token_get_flag_arg_u64(pr->request, &pr->tok, i, &of->req_cas_id) == 0) {
                    of->has_cas = true;
                } else {
                    *errstr = "CLIENT_ERROR bad token in command line format";
                    of->has_error = 1;
                ...
                break;
            case 'E':
                if (mcmc_token_get_flag_arg_u64(pr->request, &pr->tok, i, &of->cas_id_in) == 0) {
                    of->has_cas_in = true;
                } else {
                    *errstr = "CLIENT_ERROR bad token in command line format";
                    of->has_error = 1;
            ...
            case 'M':
                ...
                break;
            case 'J':
                if (mcmc_token_get_flag_arg_u64(pr->request, &pr->tok, i, &of->initial) != 0) {
                    *errstr = "CLIENT_ERROR invalid numeric initial value";
                    of->has_error = 1;
                }
                break;
            case 'D':
                if (mcmc_token_get_flag_arg_u64(pr->request, &pr->tok, i, &of->delta) != 0) {
...
static inline int make_ascii_get_suffix(char *suffix, item *it, bool return_cas, int nbytes) {
    ...
    *p = ' ';
    p++;
    if ((((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) == 0) {
        *p = '0';
        p++;
    } else {
    ...
    *p = ' ';
    ...
    if (return_cas) {
        *p = ' ';
        p = itoa_u64((((it)->it_flags & 2) ? (it)->data->cas : (uint64_t)0), p+1);
    }
    *p = '\r';
    *(p+1) = '\n';
    *(p+2) = '\0';
...
int process_get_cmd(LIBEVENT_THREAD *t, const char *key, const int nkey, mc_resp *resp, parser_storage_get_cb storage_cb, rel_time_t exptime, bool return_cas, bool should_touch) {
    ...
    item *it = limited_get(key, nkey, t, exptime, should_touch, true, &overflow);
    if (settings.detail_enabled) {
        stats_prefix_record_get(key, nkey, ((void *)0) != it);
    }
    if (it) {
      ...
      memcpy(p, "VALUE ", 6);
      p += 6;
      memcpy(p, (((char*)&((it)->data)) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), it->nkey);
      p += it->nkey;
      ...
      if (it->it_flags & 128) {
      ...
      } else if ((it->it_flags & 32) == 0) {
          resp_add_iov(resp, ((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), it->nbytes);
      } else {
        ...
        if (should_touch) {
            t->stats.touch_cmds++;
            t->stats.slab_stats[((it)->slabs_clsid & ~(3<<6))].touch_hits++;
        } else {
...
item *process_update_cmd_start(LIBEVENT_THREAD *t, mcp_parser_t *pr, mc_resp *resp, int comm, bool handle_cas) {
    ...
    if (! (safe_strtoul(&pr->request[pr->tok.tokens[2]], &flags)
    ...
    exptime = realtime((exptime_int < 0) ? 60*60*24*30 + 1 : exptime_int);
    if (handle_cas) {
        if (mcmc_token_get_u64(pr->request, &pr->tok, 5, &req_cas_id) != 0) {
            pout_string(resp, "CLIENT_ERROR bad command line format");
            return ((void *)0);
        }
    }
    ...
    if (it == 0) {
        ...
        do { logger *myl = t->l; if (t->l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<3)) logger_log(myl, LOGGER_ITEM_STORE, ((void *)0), status, comm, key, nkey, 0, 0, t->cur_sfd); } while (0);
        if (comm == 2) {
            it = item_get(key, nkey, t, false);
            if (it) {
                item_unlink(it);
    ...
    { if ((it)->it_flags & 2) { (it)->data->cas = req_cas_id; } };
    pthread_mutex_lock(&t->stats.mutex);
    t->stats.slab_stats[((it)->slabs_clsid & ~(3<<6))].set_cmds++;
    pthread_mutex_unlock(&t->stats.mutex);
    return it;
}
void process_update_cmd(LIBEVENT_THREAD *t, mcp_parser_t *pr, mc_resp *resp, int comm, bool handle_cas) {
    ...
    int ret = store_item(it, comm, t, ((void *)0), ((void *)0), (settings.use_cas) ? get_cas_id() : 0, false);
    switch (ret) {
    case STORED:
...
void process_arithmetic_cmd(LIBEVENT_THREAD *t, mcp_parser_t *pr, mc_resp *resp, const bool incr) {
    ...
    if (mcmc_token_get_u64(pr->request, &pr->tok, 2, &delta) != 0) {
        pout_string(resp, "CLIENT_ERROR invalid numeric delta argument");
        return;
    }
    switch(add_delta(t, key, nkey, incr, delta, temp, ((void *)0))) {
    case OK:
...
void process_delete_cmd(LIBEVENT_THREAD *t, mcp_parser_t *pr, mc_resp *resp) {
    ...
    it = item_get_locked(key, nkey, t, false, &hv);
    if (it) {
        pthread_mutex_lock(&t->stats.mutex);
        t->stats.slab_stats[((it)->slabs_clsid & ~(3<<6))].delete_hits++;
        pthread_mutex_unlock(&t->stats.mutex);
        do { logger *myl = t->l; if (t->l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<13)) logger_log(myl, LOGGER_DELETIONS, it, 1); } while (0);
...
void process_touch_cmd(LIBEVENT_THREAD *t, mcp_parser_t *pr, mc_resp *resp) {
    ...
    if (mcmc_token_get_32(pr->request, &pr->tok, 2, &exptime_int) != 0) {
        pout_string(resp, "CLIENT_ERROR invalid exptime argument");
        return;
    }
    exptime = realtime((exptime_int < 0) ? 60*60*24*30 + 1 : exptime_int);
...
void process_mget_cmd(LIBEVENT_THREAD *t, mcp_parser_t *pr, mc_resp *resp,
    ...
    if (!of.locked) {
        ...
        it = limited_get_locked(key, nkey, t, false, &hv, &overflow);
    }
    if (overflow) {
        ((void) (0));
    ...
    if (it == ((void *)0) && of.vivify) {
        ...
        if (it != ((void *)0)) {
            memcpy(((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), "\r\n", 2);
            do_item_link(it, hv, of.has_cas_in ? of.cas_id_in : get_cas_id());
            item_created = true;
        }
    }
    if (it) {
        if (of.has_cas && (((it)->it_flags & 2) ? (it)->data->cas : (uint64_t)0) == of.req_cas_id) {
            of.value = false;
        }
        if (of.value) {
            memcpy(p, "VA ", 3);
        ...
        for (i = pr->keytoken+1; i < pr->tok.ntokens; i++) {
            switch (pr->request[pr->tok.tokens[i]]) {
                ...
                case 'c':
                    { *p = ' '; *(p+1) = 'c'; p += 2; };
                    p = itoa_u64((((it)->it_flags & 2) ? (it)->data->cas : (uint64_t)0), p);
                    break;
                case 'f':
                    { *p = ' '; *(p+1) = 'f'; p += 2; };
                    if ((((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) == 0) {
                ...
                case 'O':
                    ...
                    break;
                case 'k':
                    { { *p = ' '; *(p+1) = 'k'; p += 2; }; if (!(it->it_flags & 4096)) { memcpy(p, (((char*)&((it)->data)) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), it->nkey); p += it->nkey; } else { p += base64_encode((unsigned char *) (((char*)&((it)->data)) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), it->nkey, (unsigned char *)p, 1024 / 2); *p = ' '; *(p+1) = 'b'; p += 2; } };
                    break;
            }
        }
        if (it->it_flags & 512) {
        ...
        for (i = pr->keytoken+1; i < pr->tok.ntokens; i++) {
            switch (pr->request[pr->tok.tokens[i]]) {
                case 'O':
                    ...
                    break;
                case 'k':
                    { { *p = ' '; *(p+1) = 'k'; p += 2; }; if (!of.key_binary) { memcpy(p, key, nkey); p += nkey; } else { p += base64_encode((unsigned char *) key, nkey, (unsigned char *)p, 1024 / 2); *p = ' '; *(p+1) = 'b'; p += 2; } };
                    break;
            }
        }
...
item *process_mset_cmd_start(LIBEVENT_THREAD *t, mcp_parser_t *pr, mc_resp *resp,
    ...
    *comm = 2;
    ...
    if (it == 0) {
        ...
        do { logger *myl = t->l; if (t->l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<3)) logger_log(myl, LOGGER_ITEM_STORE, ((void *)0), status, comm, key, nkey, 0, t->cur_sfd); } while (0);
        it = item_get_locked(key, nkey, t, false, &hv);
        if (it) {
            do_item_unlink(it, hv);
            do { storage_delete(t->storage, it); } while (0);
    ...
    { if ((it)->it_flags & 2) { (it)->data->cas = of.req_cas_id; } };
    if (of.key_binary) {
        it->it_flags |= 4096;
    }
    resp->set_stale = false;
    if (of.set_stale && *comm == 6) {
        resp->set_stale = true;
    }
...
void process_mdelete_cmd(LIBEVENT_THREAD *t, mcp_parser_t *pr, mc_resp *resp) {
    ...
    if (it) {
        ...
        if (of.remove_val) {
            ...
            if (new_it != ((void *)0)) {
                memcpy(((char*) &((new_it)->data) + (new_it)->nkey + 1 + (((new_it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((new_it)->it_flags & 2) ? sizeof(uint64_t) : 0)), "\r\n", 2);
                if (do_store_item(new_it, 2, t, hv, ((void *)0), ((void *)0),
                            of.has_cas_in ? of.cas_id_in : (((it)->it_flags & 2) ? (it)->data->cas : (uint64_t)0), false)) {
                    do_item_remove(it);
                    it = new_it;
                } else {
        ...
        if (of.set_stale) {
            ...
            it->it_flags |= 2048;
            it->it_flags &= ~512;
            { if ((it)->it_flags & 2) { (it)->data->cas = of.has_cas_in ? of.cas_id_in : get_cas_id(); } };
            if (of.no_reply)
                resp->skip = true;
            ...
            t->stats.slab_stats[((it)->slabs_clsid & ~(3<<6))].delete_hits++;
            pthread_mutex_unlock(&t->stats.mutex);
            do { logger *myl = t->l; if (t->l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<13)) logger_log(myl, LOGGER_DELETIONS, it, 2); } while (0);
            if (!of.remove_val) {
                do_item_unlink(it, hv);
                do { storage_delete(t->storage, it); } while (0);
            }
...
void process_marithmetic_cmd(LIBEVENT_THREAD *t, mcp_parser_t *pr, mc_resp *resp) {
    ...
    case OK:
        if (of.no_reply)
            resp->skip = true;
        if (of.has_cas_in) {
            { if ((it)->it_flags & 2) { (it)->data->cas = of.cas_id_in; } };
        }
    ...
    case DELTA_ITEM_NOT_FOUND:
        if (of.vivify) {
            ...
            if (it != ((void *)0)) {
                memcpy(((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), tmpbuf, vlen);
                memcpy(((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)) + vlen, "\r\n", 2);
                if (do_store_item(it, 1, t, hv, ((void *)0), &cas,
                            of.has_cas_in ? of.cas_id_in : get_cas_id(), false)) {
                    item_created = true;
                    if (of.no_reply)
                        resp->skip = true;
                } else {
