...
static void _mut_check_idx(lua_State *L, int tidx) {
    if (lua_getfield(L, tidx, "idx") != 0) {
        luaL_checktype(L, -1, 3);
        int isnum = 0;
        lua_Integer i = lua_tointegerx(L, -1, &isnum);
        if (!isnum) {
            do { lua_pushfstring(L, "mutator step %d: must provide 'idx' argument as an integer", tidx); lua_error(L); } while (0);
        }
        if (i < 1) {
            do { lua_pushfstring(L, "mutator step %d: 'idx' argument must be greater than 0", tidx); lua_error(L); } while (0);
        }
    } else {
        do { lua_pushfstring(L, "mutator step %d: must provide 'idx' argument", tidx); lua_error(L); } while (0);
    }
    lua_settop(L, -(1)-1);
}
static size_t _mut_check_strlen(lua_State *L, int tidx, const char *n) {
    size_t len = 0;
    if (lua_getfield(L, tidx, n) != 0) {
        lua_tolstring(L, -1, &len);
    } else {
        do { lua_pushfstring(L, "mutator step %d: must provide '%s' argument", tidx, n); lua_error(L); } while (0);
    }
    lua_settop(L, -(1)-1);
    return len;
}
static void _mut_check_flag(lua_State *L, int tidx) {
    if (lua_getfield(L, tidx, "flag") != 0) {
        ...
        if (len != 1) {
            do { lua_pushfstring(L, "mutator step %d: 'flag' must be a single character", tidx); lua_error(L); } while (0);
        }
        if ((flag[0] < 65 || flag[0] > 122)) {
            do { lua_pushfstring(L, "mutator step %d: 'flag' must be alphanumeric", tidx); lua_error(L); } while (0);
        }
    } else {
        do { lua_pushfstring(L, "mutator step %d: must provide 'flag' argument", tidx); lua_error(L); } while (0);
    }
    lua_settop(L, -(1)-1);
}
...
static int mcp_mutator_cmdset_c(lua_State *L, int tidx) {
    size_t len = 0;
    if (lua_getfield(L, tidx, "cmd") != 0) {
        lua_tolstring(L, -1, &len);
        if (len > 20) {
            do { lua_pushfstring(L, "mutator step %d: 'cmd' too long", tidx); lua_error(L); } while (0);
        }
    } else {
        do { lua_pushfstring(L, "mutator step %d: must provide 'cmd' argument", tidx); lua_error(L); } while (0);
    }
    lua_settop(L, -(1)-1);
    return len;
}
static int mcp_mutator_cmdset_i(lua_State *L, int tidx, int sc, struct mcp_mutator *mut) {
...
static int mcp_mutator_cmdset_n(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    struct mcp_mut_string *c = &s->c.string;
    return c->len;
}
static bool mcp_mutator_cmdset_r(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
...
static int mcp_mutator_cmdcopy_c(lua_State *L, int tidx) {
    _mut_check_idx(L, tidx);
    return 0;
}
static int mcp_mutator_cmdcopy_i(lua_State *L, int tidx, int sc, struct mcp_mutator *mut) {
    struct mcp_mut_step *s = &mut->steps[sc];
    if (lua_getfield(L, tidx, "idx") != 0) {
        s->idx = lua_tointegerx(L,(-1),((void *)0));
    }
    lua_settop(L, -(1)-1);
    return 0;
}
static int mcp_mutator_cmdcopy_n(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    lua_State *L = run->L;
    unsigned idx = s->idx;
    mcp_request_t *srq = ((void *)0);
    mcp_resp_t *srs = ((void *)0);
...
static bool mcp_mutator_cmdcopy_r(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    memcpy(run->d_pos, p->src, p->slen);
    run->d_pos += p->slen;
    return true;
}
static int mcp_mutator_keycopy_c(lua_State *L, int tidx) {
    _mut_check_idx(L, tidx);
    return 0;
}
static int mcp_mutator_keycopy_i(lua_State *L, int tidx, int sc, struct mcp_mutator *mut) {
    struct mcp_mut_step *s = &mut->steps[sc];
    if (lua_getfield(L, tidx, "idx") != 0) {
        s->idx = lua_tointegerx(L,(-1),((void *)0));
    }
...
static int mcp_mutator_keycopy_n(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    lua_State *L = run->L;
    unsigned idx = s->idx;
    mcp_request_t *srq = ((void *)0);
    mcp_resp_t *srs = ((void *)0);
    switch (lua_type(L, idx)) {
        ...
        case 7:
            _mut_checkudata(L, idx, &srq, &srs);
            if (srq) {
                p->src = (&(&srq->pr)->request[(&srq->pr)->tok.tokens[(&srq->pr)->keytoken]]);
                p->slen = srq->pr.klen;
            } else {
                return -1;
            }
...
static bool mcp_mutator_keycopy_r(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    memcpy(run->d_pos, p->src, p->slen);
    run->d_pos += p->slen;
    return true;
}
static int mcp_mutator_keyset_c(lua_State *L, int tidx) {
    size_t len = 0;
    if (lua_getfield(L, tidx, "val") != 0) {
        lua_tolstring(L, -1, &len);
        if (len < 1) {
            do { lua_pushfstring(L, "mutator step %d: 'val' must have nonzero length", tidx); lua_error(L); } while (0);
        }
    } else {
        do { lua_pushfstring(L, "mutator step %d: must provide 'val' argument", tidx); lua_error(L); } while (0);
    }
    lua_settop(L, -(1)-1);
    return len;
}
static int mcp_mutator_keyset_i(lua_State *L, int tidx, int sc, struct mcp_mutator *mut) {
    struct mcp_mut_step *s = &mut->steps[sc];
    struct mcp_mut_string *c = &s->c.string;
    size_t len = 0;
    if (lua_getfield(L, tidx, "val") != 0) {
...
static int mcp_mutator_keyset_n(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    struct mcp_mut_string *c = &s->c.string;
    return c->len;
}
static bool mcp_mutator_keyset_r(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    struct mcp_mutator *mut = run->mut;
    struct mcp_mut_string *c = &s->c.string;
    const char *str = mut->arena + c->str;
    memcpy(run->d_pos, str, c->len);
...
static int mcp_mutator_resnull_c(lua_State *L, int tidx) {
    return 0;
}
static int mcp_mutator_resnull_i(lua_State *L, int tidx, int sc, struct mcp_mutator *mut) {
    struct mcp_mut_step *s = &mut->steps[sc];
    if (lua_getfield(L, tidx, "idx") != 0) {
        s->idx = lua_tointegerx(L,(-1),((void *)0));
    }
...
static int mcp_mutator_resnull_n(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    lua_State *L = run->L;
...
static bool mcp_mutator_resnull_r(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    memcpy(run->d_pos, p->src, p->slen);
    run->d_pos += p->slen;
    return true;
}
static int mcp_mutator_rescodeset_c(lua_State *L, int tidx) {
    return _mut_check_strlen(L, tidx, "val");
}
static int mcp_mutator_rescodeset_i(lua_State *L, int tidx, int sc, struct mcp_mutator *mut) {
    struct mcp_mut_step *s = &mut->steps[sc];
    struct mcp_mut_string *c = &s->c.string;
    size_t len = 0;
    if (lua_getfield(L, tidx, "val") != 0) {
...
static int mcp_mutator_rescodeset_n(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    struct mcp_mut_string *c = &s->c.string;
    return c->len;
}
static bool mcp_mutator_rescodeset_r(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    struct mcp_mutator *mut = run->mut;
    struct mcp_mut_string *c = &s->c.string;
    const char *str = mut->arena + c->str;
    memcpy(run->d_pos, str, c->len);
...
static int mcp_mutator_rescodecopy_c(lua_State *L, int tidx) {
    _mut_check_idx(L, tidx);
    return 0;
}
static int mcp_mutator_rescodecopy_i(lua_State *L, int tidx, int sc, struct mcp_mutator *mut) {
    struct mcp_mut_step *s = &mut->steps[sc];
    if (lua_getfield(L, tidx, "idx") != 0) {
        s->idx = lua_tointegerx(L,(-1),((void *)0));
    }
...
static int mcp_mutator_rescodecopy_n(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    lua_State *L = run->L;
    unsigned idx = s->idx;
    mcp_request_t *srq = ((void *)0);
    mcp_resp_t *srs = ((void *)0);
    switch (lua_type(L, idx)) {
        ...
        case 7:
            _mut_checkudata(L, idx, &srq, &srs);
            if (srs) {
                if (srs->resp.type == 101) {
                    mcmc_tokenize_res((srs->cresp ? srs->cresp->iov[0].iov_base : srs->buf), srs->resp.reslen, &srs->tok);
                } else {
                    return -1;
                }
                int len = 0;
                p->src = mcmc_token_get((srs->cresp ? srs->cresp->iov[0].iov_base : srs->buf), &srs->tok, 0, &len);
                p->slen = len;
                if (len < 2) {
                    return -1;
                }
...
static bool mcp_mutator_rescodecopy_r(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    memcpy(run->d_pos, p->src, p->slen);
    run->d_pos += p->slen;
    return true;
}
static int mcp_mutator_reserr_c(lua_State *L, int tidx) {
    size_t total = 0;
    char *code = ((void *)0);
    if (lua_getfield(L, tidx, "code") != 0) {
        const char *val = lua_tolstring(L, (-1), ((void *)0));
        if (strcmp(val, "error") == 0) {
            ...
            do { lua_pushfstring(L, "mutator step %d: code must be 'error', server', or 'client'", tidx); lua_error(L); } while (0);
        }
        total += strlen(code);
    } else {
        do { lua_pushfstring(L, "mutator step %d: must provide 'code' argument", tidx); lua_error(L); } while (0);
    }
    lua_settop(L, -(1)-1);
    if (lua_getfield(L, tidx, "msg") != 0) {
        size_t len = 0;
        lua_tolstring(L, -1, &len);
        if (len < 1) {
            do { lua_pushfstring(L, "mutator step %d: 'msg' must be a nonzero length string", tidx); lua_error(L); } while (0);
        }
        total += len + 1;
    }
    lua_settop(L, -(1)-1);
...
static int mcp_mutator_reserr_i(lua_State *L, int tidx, int sc, struct mcp_mutator *mut) {
    struct mcp_mut_step *s = &mut->steps[sc];
    struct mcp_mut_string *c = &s->c.string;
    size_t len = 0;
...
static int mcp_mutator_reserr_n(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    struct mcp_mut_string *c = &s->c.string;
    return c->len;
}
static bool mcp_mutator_reserr_r(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    struct mcp_mutator *mut = run->mut;
    struct mcp_mut_string *c = &s->c.string;
    const char *str = mut->arena + c->str;
    int len = c->len;
...
static int mcp_mutator_flagset_c(lua_State *L, int tidx) {
    ...
    if (vtype == 3 || vtype == 4) {
        ...
        do { lua_pushfstring(L, "mutator step %d: unsupported type for 'val'", tidx); lua_error(L); } while (0);
    }
    lua_settop(L, -(1)-1);
    return len;
}
static int mcp_mutator_flagset_i(lua_State *L, int tidx, int sc, struct mcp_mutator *mut) {
    struct mcp_mut_step *s = &mut->steps[sc];
...
static int mcp_mutator_flagset_n(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    struct mcp_mut_flagval *c = &s->c.flagval;
    return c->str.len + 1;
}
static bool mcp_mutator_flagset_r(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    struct mcp_mutator *mut = run->mut;
    struct mcp_mut_flagval *c = &s->c.flagval;
...
static int mcp_mutator_flagcopy_c(lua_State *L, int tidx) {
    _mut_check_flag(L, tidx);
    _mut_check_idx(L, tidx);
    return 0;
}
static int mcp_mutator_flagcopy_i(lua_State *L, int tidx, int sc, struct mcp_mutator *mut) {
    struct mcp_mut_step *s = &mut->steps[sc];
...
static int mcp_mutator_flagcopy_n(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    struct mcp_mut_flag *c = &s->c.flag;
    lua_State *L = run->L;
    unsigned int idx = s->idx;
    mcp_request_t *srq = ((void *)0);
    ...
    switch (lua_type(L, idx)) {
        ...
        case 7:
            ...
            if (srq) {
                ...
                int tlen = 0;
                if ((((&srq->pr.tok)->metaflags & c->bit) ? 0 : -2) == 0) {
                ...
                mcmc_tokenize_res((srs->cresp ? srs->cresp->iov[0].iov_base : srs->buf), srs->resp.reslen, &srs->tok);
                if ((((&srs->tok)->metaflags & c->bit) ? 0 : -2) == 0) {
                    int len = 0;
                    p->slen = 1;
                    const char *tok = mcmc_token_get_flag((srs->cresp ? srs->cresp->iov[0].iov_base : srs->buf), &srs->tok, c->f, &len);
                    if (len > 0) {
                        p->src = tok;
                        p->slen += len;
                    }
...
static bool mcp_mutator_flagcopy_r(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    struct mcp_mut_flag *c = &s->c.flag;
    if (p->slen) {
        *(run->d_pos) = c->f;
        run->d_pos++;
...
static int mcp_mutator_flagcopyall_c(lua_State *L, int tidx) {
    return 0;
}
static int mcp_mutator_flagcopyall_i(lua_State *L, int tidx, int sc, struct mcp_mutator *mut) {
    struct mcp_mut_step *s = &mut->steps[sc];
    if (lua_getfield(L, tidx, "idx") != 0) {
        s->idx = lua_tointegerx(L,(-1),((void *)0));
    }
...
static int mcp_mutator_flagcopyall_n(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    lua_State *L = run->L;
    unsigned idx = s->idx;
    mcp_request_t *srq = ((void *)0);
    mcp_resp_t *srs = ((void *)0);
    switch (lua_type(L, idx)) {
        ...
        case 7:
            ...
            if (srq) {
                ...
                if (srs->resp.type == 101 && srs->resp.rline) {
...
static bool mcp_mutator_flagcopyall_r(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    memcpy(run->d_pos, p->src, p->slen);
    run->d_pos += p->slen;
    return true;
}
static int mcp_mutator_valcopy_c(lua_State *L, int tidx) {
    _mut_check_idx(L, tidx);
    return 0;
}
static int mcp_mutator_valcopy_i(lua_State *L, int tidx, int sc, struct mcp_mutator *mut) {
    struct mcp_mut_step *s = &mut->steps[sc];
    if (lua_getfield(L, tidx, "idx") != 0) {
        s->idx = lua_tointegerx(L,(-1),((void *)0));
    }
...
static int mcp_mutator_valcopy_n(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    lua_State *L = run->L;
    unsigned int idx = s->idx;
    mcp_request_t *srq = ((void *)0);
    mcp_resp_t *srs = ((void *)0);
...
static bool mcp_mutator_valcopy_r(struct mcp_mut_run *run, struct mcp_mut_step *s, struct mcp_mut_part *p) {
    run->d_pos = itoa_u64(run->vlen-2, run->d_pos);
    return true;
}
static const struct mcp_mut_entry mcp_mut_entries[] = {
    [mcp_mut_step_none] = {((void *)0), ((void *)0), ((void *)0), ((void *)0), ((void *)0), 0, 0},
    ...
    [mcp_mut_step_final] = {((void *)0), ((void *)0), ((void *)0), ((void *)0), ((void *)0), 0, 0},
};
...
static int mcp_mutator_new(lua_State *L, enum mcp_mut_type type) {
    ...
    for (int x = 1; x <= argc; x++) {
        ...
        if (lua_getfield(L, x, "t") != 0) {
            ...
            if (!(e->t & type)) {
                do { lua_pushfstring(L, "mutator step %d: step incompatible with mutator type", x); lua_error(L); } while (0);
            }
            if ((st == mcp_mut_step_none) || e->c == ((void *)0)) {
                do { lua_pushfstring(L, "mutator step %d: unknown step type", x); lua_error(L); } while (0);
            }
            size += e->c(L, x);
        }
        lua_settop(L, -(1)-1);
...
static int mcp_mut_run(struct mcp_mut_run *run) {
    struct mcp_mutator *mut = run->mut;
    LIBEVENT_THREAD *t = (*(LIBEVENT_THREAD **)((void *)((char *)(run->L) - (sizeof(void *)))));
    int ret = 0;
...
int mcplib_mutator_call(lua_State *L) {
    ...
    lua_getiuservalue(L, 1, mut->type);
    ((void)((__builtin_expect(((lua_rawequal(L, -1, -2)) != 0), 1)) || luaL_argerror(L, (2), ("invalid argument to mutator object"))));
    lua_settop(L, -(2)-1);
    void *arg = lua_touserdata(L, 2);
    struct mcp_mut_run run = {L, mut, arg, ((void *)0), ((void *)0), 0};
