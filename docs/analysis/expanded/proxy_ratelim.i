...
int mcplib_ratelim_proxy_tbf_gc(lua_State *L) {
    ...
    proxy_ctx_t *ctx = ((*(LIBEVENT_THREAD **)((void *)((char *)(L) - (sizeof(void *)))))->proxy_ctx);
...
int mcp_ratelim_proxy_tbf(lua_State *from, lua_State *to) {
    ...
    *lim_p = lim;
    lua_pushvalue(from, -3);
    mcp_gobj_ref(from, &lim->g);
    return 0;
}
static lua_Integer _tbf_check(lua_State *L, char *key) {
    lua_Integer n = 0;
    if (lua_getfield(L, 1, key) != 0) {
        n = lua_tointegerx(L,(-1),((void *)0));
        if (n < 0 || n > (0x7fffffff * 2U + 1U)-1) {
...
static void _setup_tbf(lua_State *L, struct mcp_ratelim_tbf *lim) {
    ...
    lim->last_update = (now.tv_usec / 1000 + now.tv_sec * (uint64_t)1000);
}
int mcplib_ratelim_tbf(lua_State *L) {
...
int mcplib_ratelim_global_tbf(lua_State *L) {
    ...
    memset(lim, 0, sizeof(*lim));
    pthread_mutex_init(&lim->g.lock, ((void *)0));
...
int mcplib_ratelim_tbf_call(lua_State *L) {
    ...
    int take = lua_tointegerx(L,(2),((void *)0));
    struct timeval now;
    uint64_t now_millis = 0;
    gettimeofday(&now, ((void *)0));
    now_millis = (now.tv_usec / 1000 + now.tv_sec * (uint64_t)1000);
