...
static int wait_for_process(pid_t pid)
{
    ...
    for (i = 0; ;i++) {
        ...
        if (p == pid) {
            ...
            switch (i) {
            case 0:
                sig = caught_sig > 0 ? caught_sig : 15;
                if (caught_sig == 14) {
...
int main(int argc, char **argv)
{
    ...
    if (argc < 3)
        usage();
    naptime = atoi(argv[1]);
    ((void) sizeof (__assert_single_arg (naptime > 0 && naptime < 1800)), __extension__ ({ if (naptime > 0 && naptime < 1800) ; else __assert_fail ("naptime > 0 && naptime < 1800", "timedrun.c", 116, __extension__ __PRETTY_FUNCTION__); }));
    alarm(naptime);
