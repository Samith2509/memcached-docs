...
static void _add_server_default(ketama_t *kt, size_t hashstring_size, const char **parts,
    ...
    for (int k = 0; k < bucket_size; k++) {
        ...
        kt->continuum[*cont].point = (unsigned int) XXH_INLINE_XXH3_64bits(hashstring, len);
...
static int ketama_new(lua_State *L) {
    ...
    if (argc > 1) {
        ...
        if (lua_getfield(L, 2, "omode") != 0) {
            luaL_checktype(L, -1, 4);
            const char *mode = lua_tolstring(L, (-1), ((void *)0));
        ...
        lua_settop(L, -(1)-1);
        if (lua_getfield(L, 2, "obuckets") != 0) {
          int success = 0;
    ...
    while (lua_next(L, 1) != 0) {
        lua_getfield(L, -1, "id");
        lua_Integer id = lua_tointegerx(L,(-1),((void *)0));
        lua_settop(L, -(1)-1);
...
int mcplib_open_dist_ring_hash(lua_State *L) {
    ...
    (luaL_checkversion_(L, 504, (sizeof(lua_Integer)*16 + sizeof(lua_Number))), lua_createtable(L, 0, sizeof(ketama_f)/sizeof((ketama_f)[0]) - 1), luaL_setfuncs(L,ketama_f,0));
