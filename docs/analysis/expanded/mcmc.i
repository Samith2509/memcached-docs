...
typedef struct mcmc_ctx {
    int fd;
    int gai_status;
    int sent_bytes_partial;
    int error;
    uint32_t status_flags;
    size_t buffer_used;
    char *buffer_head;
} mcmc_ctx_t;
static int _mcmc_tokenize_meta(mcmc_tokenizer_t *t, const char *line, size_t len, const int mstart, const int max) {
    const char *s;
    const char *end;
    const char *n = s = line;
    t->metaflags = 0;
        ...
        if (*n == ' ') {
            if (s != n) {
                ...
                if (curtoken >= mstart) {
                    ...
                    } else if (((*__ctype_b_loc ())[(int) ((*s))] & (unsigned short int) _ISdigit) == 0) {
                        return -2;
                    }
                }
                if (++curtoken == max) {
                    s++;
                    s = _Generic (0 ? (s) : (void *) 1, const void *: (const void *) (memchr (s, ' ', end - s)), default: memchr (s, ' ', end - s));
                    if (!s) {
                        s = end;
                    }
                    n = s;
...
__attribute__((unused)) static int _mcmc_token_len(const char *line, mcmc_tokenizer_t *t, size_t token) {
...
__attribute__((unused)) static const char *_mcmc_token(const char *line, mcmc_tokenizer_t *t, size_t token, int *len) {
    const char *s = line + t->tokens[token];
    if (len != ((void *)0)) {
        const char *e = line + t->tokens[token+1];
        while (*(e-1) == ' ') {
...
static int _mcmc_parse_value_line(const char *buf, size_t read, mcmc_resp_t *r) {
    ...
    const char *key = p;
    int keylen;
    p = _Generic (0 ? (p) : (void *) 1, const void *: (const void *) (memchr (p, ' ', l - 6)), default: memchr (p, ' ', l - 6));
    if (p == ((void *)0)) {
        return -20;
    }
    keylen = p - key;
    ...
    if (((*__errno_location ()) == 34) || (p == n) || (*n != ' ')) {
        return -20;
    }
    p = n;
    (*__errno_location ()) = 0;
    uint32_t bytes = strtoul(p, &n, 10);
    if (((*__errno_location ()) == 34) || (p == n) || (bytes >= (4294967295U)-2)) {
        return -20;
    }
    p = n;
    uint64_t cas = 0;
...
static int _mcmc_parse_stat_line(const char *buf, mcmc_resp_t *r) {
    ...
    size_t l = r->reslen;
    const char *sname = p;
    p = _Generic (0 ? (p) : (void *) 1, const void *: (const void *) (memchr (p, ' ', l-5)), default: memchr (p, ' ', l-5));
    if (p == ((void *)0)) {
        return -20;
    }
            ...
            case 'V':
                if (buf[1] == 'A') {
                    if (more) {
                        ...
                        if (((*__errno_location ()) == 34) || (cur == n) || (vsize >= (4294967295U)-2)) {
                            r->type = 109;
                            code = -19;
                        } else {
                            r->vlen = vsize + 2;
...
static int mcmc_toktou32(const char *t, size_t len, uint32_t *out) {
    uint32_t sum = 0;
    const char *pos = t;
    if (len > 11) {
        return -2;
    ...
    while (len--) {
        char num = pos[0] - '0';
        if (num > -1 && num < 10) {
            uint32_t lim = ((4294967295U) - num) / 10;
            if (sum > lim) {
                return -1;
            }
            sum = sum * 10 + num;
...
static int mcmc_toktou64(const char *t, size_t len, uint64_t *out) {
    uint64_t sum = 0;
    const char *pos = t;
    if (len > 22) {
        return -2;
    ...
    while (len--) {
        char num = pos[0] - '0';
        if (num > -1 && num < 10) {
            uint64_t lim = ((18446744073709551615UL) - num) / 10;
            if (sum > lim) {
                return -1;
            }
            sum = sum * 10 + num;
...
static int mcmc_tokto32(const char *t, size_t len, int32_t *out) {
    int32_t sum = 0;
    const char *pos = t;
    ...
    while (pos != end) {
        ...
        if (num > -1 && num < 10) {
            if (is_sig) {
                int32_t lim = ((-2147483647-1) + num) / 10;
                if (sum < lim) {
                    return -1;
                }
                sum = sum * 10 - num;
            } else {
                int32_t lim = ((2147483647) - num) / 10;
                if (sum > lim) {
                    return -1;
                }
                sum = sum * 10 + num;
...
static int mcmc_tokto64(const char *t, size_t len, int64_t *out) {
    int64_t sum = 0;
    const char *pos = t;
    const char *end = pos + len;
    int is_sig = 0;
    ...
    while (pos != end) {
        ...
        if (num > -1 && num < 10) {
            if (is_sig) {
                int64_t lim = ((-9223372036854775807L -1) + num) / 10;
                if (sum < lim) {
                    return -1;
                }
                sum = sum * 10 - num;
            } else {
                int64_t lim = ((9223372036854775807L) - num) / 10;
                if (sum > lim) {
                    return -1;
                }
                sum = sum * 10 + num;
...
int mcmc_tokenize_res(const char *l, size_t len, mcmc_tokenizer_t *t) {
    if (t->ntokens == 0) {
        return _mcmc_tokenize_meta(t, l, len, 1, 24 -1);
    }
    return 0;
}
int mcmc_tokenize(const char *l, size_t len, mcmc_tokenizer_t *t, int meta_offset) {
    return _mcmc_tokenize_meta(t, l, len, meta_offset, 24 -1);
}
const char *mcmc_token_get(const char *l, mcmc_tokenizer_t *t, int idx, int *len) {
...
int mcmc_token_get_u32(const char *l, mcmc_tokenizer_t *t, int idx, uint32_t *val) { if (idx > 0 && idx < t->ntokens) { int tlen = 0; const char *tok = _mcmc_token(l, t, idx, &tlen); return mcmc_toktou32(tok, tlen, val); } else { return -1; } }
int mcmc_token_get_u64(const char *l, mcmc_tokenizer_t *t, int idx, uint64_t *val) { if (idx > 0 && idx < t->ntokens) { int tlen = 0; const char *tok = _mcmc_token(l, t, idx, &tlen); return mcmc_toktou64(tok, tlen, val); } else { return -1; } }
int mcmc_token_get_32(const char *l, mcmc_tokenizer_t *t, int idx, int32_t *val) { if (idx > 0 && idx < t->ntokens) { int tlen = 0; const char *tok = _mcmc_token(l, t, idx, &tlen); return mcmc_tokto32(tok, tlen, val); } else { return -1; } }
int mcmc_token_get_64(const char *l, mcmc_tokenizer_t *t, int idx, int64_t *val) { if (idx > 0 && idx < t->ntokens) { int tlen = 0; const char *tok = _mcmc_token(l, t, idx, &tlen); return mcmc_tokto64(tok, tlen, val); } else { return -1; } }
int mcmc_token_get_flag_arg_u32(const char *l, mcmc_tokenizer_t *t, int idx, uint32_t *val) { int tlen = 0; const char *tok = _mcmc_token(l, t, idx, &tlen); if (tlen > 1) { return mcmc_toktou32(tok+1, tlen-1, val); } return -1; }
int mcmc_token_get_flag_arg_u64(const char *l, mcmc_tokenizer_t *t, int idx, uint64_t *val) { int tlen = 0; const char *tok = _mcmc_token(l, t, idx, &tlen); if (tlen > 1) { return mcmc_toktou64(tok+1, tlen-1, val); } return -1; }
int mcmc_token_get_flag_arg_32(const char *l, mcmc_tokenizer_t *t, int idx, int32_t *val) { int tlen = 0; const char *tok = _mcmc_token(l, t, idx, &tlen); if (tlen > 1) { return mcmc_tokto32(tok+1, tlen-1, val); } return -1; }
int mcmc_token_get_flag_arg_64(const char *l, mcmc_tokenizer_t *t, int idx, int64_t *val) { int tlen = 0; const char *tok = _mcmc_token(l, t, idx, &tlen); if (tlen > 1) { return mcmc_tokto64(tok+1, tlen-1, val); } return -1; }
int mcmc_token_has_flag(const char *l, mcmc_tokenizer_t *t, char flag) {
    if (flag < 65 || flag > 122) {
        return -1;
    }
...
const char *mcmc_token_get_flag(const char *l, mcmc_tokenizer_t *t, char flag, int *len) {
    for (int x = t->mstart; x < t->ntokens; x++) {
        ...
        if (tflag[0] == flag) {
            int tlen = _mcmc_token_len(l, t, x) - 1;
            if (tlen > 0) {
                if (len) {
                    *len = tlen;
                }
...
int mcmc_parse_buf(const char *buf, size_t read, mcmc_resp_t *r) {
    char *el;
    memset(r, 0, sizeof(*r));
    el = _Generic (0 ? (buf) : (void *) 1, const void *: (const void *) (memchr (buf, '\n', read)), default: memchr (buf, '\n', read));
    if (el == ((void *)0)) {
        r->code = 5;
        return -1;
    }
...
int mcmc_check_nonblock_connect(void *c, int *err) {
    ...
    if (getsockopt(ctx->fd, 1, 4, err, &errsize) == 0) {
        if (*err == 0) {
            return 0;
        }
    } else {
...
int mcmc_connect(void *c, char *host, char *port, int options) {
    ...
    for (next = ai; next != ((void *)0); next = next->ai_next) {
        ...
        if (options & (1<<1)) {
            int optval = 1;
            if (setsockopt(sock, 1, 9, &optval, sizeof(optval)) < 0) {
                res = -1;
                close(sock);
                goto end;
            }
        ...
        if (options & 1) {
            int flags = fcntl(sock, 3);
            if (flags < 0) {
                res = -1;
                close(sock);
                goto end;
            }
            if (fcntl(sock, 4, flags | 04000) < 0) {
                res = -1;
                close(sock);
                goto end;
            }
