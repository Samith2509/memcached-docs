...
int try_read_command_binary(conn *c) {
    if (c->rbytes < sizeof(c->binary_header)) {
        ...
        c->binary_header = *req;
        c->binary_header.request.keylen = __bswap_16 (req->request.keylen);
        c->binary_header.request.bodylen = __bswap_32 (req->request.bodylen);
        ...
        char extbuf[sizeof(c->binary_header) + 20 +4];
...
static void add_bin_header(conn *c, uint16_t err, uint8_t hdr_len, uint16_t key_len, uint32_t body_len) {
    ...
    header->response.keylen = (uint16_t)__bswap_16 (key_len);
    header->response.extlen = (uint8_t)hdr_len;
    header->response.datatype = (uint8_t)PROTOCOL_BINARY_RAW_BYTES;
    header->response.status = (uint16_t)__bswap_16 (err);
...
static void complete_incr_bin(conn *c, char *extbuf) {
    ...
    req->message.body.expiration = __bswap_32 (req->message.body.expiration);
    ...
    case DELTA_ITEM_NOT_FOUND:
        if (req->message.body.expiration != 0xffffffff) {
            ...
            if (it != ((void *)0)) {
                uint64_t cas = 0;
                memcpy(((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), tmpbuf, res);
                memcpy(((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)) + res, "\r\n", 2);
                c->thread->cur_sfd = c->sfd;
                if (store_item(it, 1, c->thread, ((void *)0), &cas, (settings.use_cas) ? get_cas_id() : 0, false)) {
                    c->cas = cas;
...
static void complete_update_bin(conn *c) {
    ...
    item *it = c->item;
    pthread_mutex_lock(&c->thread->stats.mutex);
    c->thread->stats.slab_stats[((it)->slabs_clsid & ~(3<<6))].set_cmds++;
    pthread_mutex_unlock(&c->thread->stats.mutex);
    if ((it->it_flags & 32) == 0) {
        *(((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)) + it->nbytes - 2) = '\r';
        *(((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)) + it->nbytes - 1) = '\n';
    } else {
        ((void) (0));
    ...
    uint64_t cas = 0;
    c->thread->cur_sfd = c->sfd;
    ret = store_item(it, c->cmd, c->thread, ((void *)0), &cas, (settings.use_cas) ? get_cas_id() : 0, false);
    c->cas = cas;
    switch (ret) {
    case STORED:
...
static void process_bin_get_or_touch(conn *c, char *extbuf) {
    ...
    if (should_touch) {
        ...
        time_t exptime = __bswap_32 (t->message.body.expiration);
        it = limited_get(key, nkey, c->thread, realtime(exptime), true, true, &overflow);
    } else {
        it = limited_get(key, nkey, c->thread, 0, false, true, &overflow);
    }
    if (it) {
        uint16_t keylen = 0;
        ...
        if (should_touch) {
            ...
            c->thread->stats.slab_stats[((it)->slabs_clsid & ~(3<<6))].touch_hits++;
        } else {
        ...
        rsp->message.header.response.cas = htonll((((it)->it_flags & 2) ? (it)->data->cas : (uint64_t)0));
        { if ((it)->it_flags & 256) { rsp->message.body.flags = *((client_flags_t *)((char*) &(((it))->data) + ((it))->nkey + 1 + ((((it))->it_flags & 2) ? sizeof(uint64_t) : 0))); } else { rsp->message.body.flags = 0; } };
        rsp->message.body.flags = __bswap_32 (rsp->message.body.flags);
        resp_add_iov(c->resp, &rsp->message.body, sizeof(rsp->message.body));
        if (should_return_key) {
            resp_add_iov(c->resp, (((char*)&((it)->data)) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), nkey);
        }
        if (should_return_value) {
            if (it->it_flags & 128) {
                ...
                if (storage_get_item(c->thread, it, resp) != 0) {
                    ...
                    ((void) (0));
                    resp->io_pending->c = c;
                    do { resp->suspended = true; c->resps_suspended++; } while (0);
                }
            } else if ((it->it_flags & 32) == 0) {
                resp_add_iov(c->resp, ((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), it->nbytes - 2);
            } else {
        ...
        if (!failed) {
            conn_set_state(c, conn_new_cmd);
            if ((it->it_flags & 128) != 0 && should_return_value) {
                c->resp->item = ((void *)0);
            } else {
                c->resp->item = it;
            }
    ...
    if (failed) {
        ...
        if (c->resp->noreply) {
            ...
            if (should_return_key) {
                ...
                write_bin_miss_response(c, ((void *)0), 0);
            }
        }
    }
...
static void process_bin_sasl_auth(conn *c) {
    ...
    if (it == 0 || (it->it_flags & 32)) {
        write_bin_error(c, PROTOCOL_BINARY_RESPONSE_ENOMEM, ((void *)0), vlen);
        conn_set_state(c, conn_swallow);
        if (it) {
            do_item_remove(it);
        }
    ...
    c->item = it;
    c->ritem = ((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0));
    c->rlbytes = vlen;
...
static void process_bin_complete_sasl_auth(conn *c) {
    ...
    char mech[nkey+1];
    memcpy(mech, (((char*)&(((item*)c->item)->data)) + ((((item*)c->item)->it_flags & 2) ? sizeof(uint64_t) : 0)), nkey);
    mech[nkey] = 0x00;
    if (settings.verbose)
        fprintf(stderr, "mech:  ``%s'' with %d bytes of data\n", mech, vlen);
    const char *challenge = vlen == 0 ? ((void *)0) : ((char*) &(((item*) c->item)->data) + ((item*) c->item)->nkey + 1 + ((((item*) c->item)->it_flags & 256) ? sizeof(client_flags_t) : 0) + ((((item*) c->item)->it_flags & 2) ? sizeof(uint64_t) : 0));
    ...
    case PROTOCOL_BINARY_CMD_SASL_AUTH:
        result = 1;
        c->sasl_started = (result == 0 || result == -1);
        break;
    ...
    switch (c->cmd) {
        case PROTOCOL_BINARY_CMD_VERSION:
            if (extlen == 0 && keylen == 0 && bodylen == 0) {
                write_bin_response(c, "1.6.45", 0, 0, strlen("1.6.45"));
            } else {
                protocol_error = 1;
            }
            break;
...
static void process_bin_update(conn *c, char *extbuf) {
    ...
    key = binary_get_key(c);
    nkey = c->binary_header.request.keylen;
    req->message.body.flags = __bswap_32 (req->message.body.flags);
    req->message.body.expiration = __bswap_32 (req->message.body.expiration);
    ...
    if (it == 0) {
        ...
        do { logger *myl = c->thread->l; if (c->thread->l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<3)) logger_log(myl, LOGGER_ITEM_STORE, ((void *)0), status, 0, key, nkey, req->message.body.expiration, 0, c->sfd); } while (0);
        if (c->cmd == PROTOCOL_BINARY_CMD_SET) {
            it = item_get(key, nkey, c->thread, false);
            if (it) {
                item_unlink(it);
                do { storage_delete(c->thread->storage, it); } while (0);
                item_remove(it);
            }
        }
        conn_set_state(c, conn_swallow);
    ...
    { if ((it)->it_flags & 2) { (it)->data->cas = c->binary_header.request.cas; } };
    switch (c->cmd) {
        case PROTOCOL_BINARY_CMD_ADD:
            c->cmd = 1;
            break;
...
static void process_bin_flush(conn *c, char *extbuf) {
    ...
    if (c->binary_header.request.extlen == sizeof(req->message.body)) {
        exptime = __bswap_32 (req->message.body.expiration);
    }
    if (exptime > 0) {
...
static void process_bin_delete(conn *c) {
    ...
    it = item_get_locked(key, nkey, c->thread, false, &hv);
    if (it) {
        uint64_t cas = c->binary_header.request.cas;
        if (cas == 0 || cas == (((it)->it_flags & 2) ? (it)->data->cas : (uint64_t)0)) {
            ;
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.slab_stats[((it)->slabs_clsid & ~(3<<6))].delete_hits++;
            pthread_mutex_unlock(&c->thread->stats.mutex);
