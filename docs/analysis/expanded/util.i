...
void uriencode_init(void) {
    ...
    for (x = 0; x < 256; x++) {
        if (((*__ctype_b_loc ())[(int) ((x))] & (unsigned short int) _ISalnum) || x == '-' || x == '.' || x == '_' || x == '~') {
...
bool safe_strtoull(const char *str, uint64_t *out) {
    ...
    *out = 0;
    ...
    if (((*__errno_location ()) == 34) || (str == endptr)) {
        return false;
    }
    if (((*__ctype_b_loc ())[(int) (((unsigned char)*endptr))] & (unsigned short int) _ISspace) || (*endptr == '\0' && endptr != str)) {
        if ((long long) ull < 0) {
            if (_Generic (0 ? (str) : (void *) 1, const void *: (const void *) (memchr (str, '-', endptr - str)), default: memchr (str, '-', endptr - str)) != ((void *)0)) {
                return false;
            }
        }
        *out = ull;
