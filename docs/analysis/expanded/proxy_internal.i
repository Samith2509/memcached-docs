...
static void _storage_get_item_cb(void *e, obj_io *eio, int ret) {
    ...
    if (ret < 1) {
        ...
        crc2 = crc32c(0, (char *)read_it+__builtin_offsetof (item, nbytes), eio->len-__builtin_offsetof (item, nbytes));
        if (crc != crc2) {
            miss = true;
            io->badcrc = true;
        }
    ...
    if (!miss) {
        resp->iov[io->iovec_data].iov_base = ((char*) &((read_it)->data) + (read_it)->nkey + 1 + (((read_it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((read_it)->it_flags & 2) ? sizeof(uint64_t) : 0));
    }
    io->miss = miss;
    io->active = false;
...
static int _proxy_storage_get(LIBEVENT_THREAD *t, item *it, mc_resp *resp,
    ...
    io->payload = __builtin_offsetof (io_pending_proxy_t, eio);
    obj_io *eio = &io->eio;
...
static int proxy_storage_mg(LIBEVENT_THREAD *t, item *it, mc_resp *resp) {
    return _proxy_storage_get(t, it, resp, 1);
}
static int proxy_storage_get(LIBEVENT_THREAD *t, item *it, mc_resp *resp) {
    return _proxy_storage_get(t, it, resp, 0);
}
static void proxy_get_cmd(LIBEVENT_THREAD *t, mcp_parser_t *pr, mc_resp *resp, bool return_cas, bool should_touch) {
    ...
    if (process_get_cmd(t, (&(pr)->request[(pr)->tok.tokens[(pr)->keytoken]]), pr->klen, resp, proxy_storage_get, exptime, return_cas, should_touch) == 0) {
...
static inline int _mcplib_internal_run(LIBEVENT_THREAD *t, mcp_request_t *rq, mcp_resp_t *r, mc_resp *resp) {
    ...
    switch (pr->command) {
        ...
        case CMD_MA:
            ...
            break;
        case CMD_GET:
            proxy_get_cmd(t, pr, resp, false, false);
            break;
        case CMD_GETS:
            proxy_get_cmd(t, pr, resp, true, false);
            break;
        case CMD_GAT:
            proxy_get_cmd(t, pr, resp, false, true);
            break;
        case CMD_GATS:
            proxy_get_cmd(t, pr, resp, true, true);
            break;
        case CMD_SET:
            process_update_cmd(t, pr, resp, 2, false);
            break;
        case CMD_ADD:
            process_update_cmd(t, pr, resp, 1, false);
            break;
        case CMD_APPEND:
            process_update_cmd(t, pr, resp, 4, false);
            break;
        case CMD_PREPEND:
            process_update_cmd(t, pr, resp, 5, false);
            break;
        case CMD_CAS:
            process_update_cmd(t, pr, resp, 6, true);
            break;
        case CMD_REPLACE:
            process_update_cmd(t, pr, resp, 3, true);
            break;
        case CMD_INCR:
...
int mcplib_internal(lua_State *L) {
    ...
    lua_pushinteger(L, 1);
    return lua_yieldk(L, (2), 0, ((void *)0));
}
void *mcp_rcontext_internal(mcp_rcontext_t *rctx, mcp_request_t *rq, mcp_resp_t *r) {
