...
struct conn_queue_item {
    ...
    io_pending_t *io;
    struct { struct conn_queue_item *stqe_next; } i_next;
};
typedef struct conn_queue CQ;
struct conn_queue {
    struct conn_ev_head { struct conn_queue_item *stqh_first; struct conn_queue_item **stqh_last; } head;
    pthread_mutex_t lock;
    cache_t *cache;
};
pthread_mutex_t lru_locks[256];
pthread_mutex_t conn_lock = { { 0, 0, 0, 0, PTHREAD_MUTEX_TIMED_NP, 0, 0, { ((void *)0), ((void *)0) } } };
static pthread_mutex_t stats_lock = { { 0, 0, 0, 0, PTHREAD_MUTEX_TIMED_NP, 0, 0, { ((void *)0), ((void *)0) } } };
...
static void thread_libevent_ionotify(int fd, short which, void *arg);
void item_lock(uint32_t hv) {
    pthread_mutex_lock(&item_locks[hv & (((unsigned long int)1<<(item_lock_hashpower))-1)]);
}
void *item_trylock(uint32_t hv) {
    pthread_mutex_t *lock = &item_locks[hv & (((unsigned long int)1<<(item_lock_hashpower))-1)];
...
void item_unlock(uint32_t hv) {
    pthread_mutex_unlock(&item_locks[hv & (((unsigned long int)1<<(item_lock_hashpower))-1)]);
}
...
void stop_threads(void) {
    ...
    for (i = 0; i < settings.num_threads; i++) {
        pthread_join(threads[i].thread_id, ((void *)0));
    }
    if (settings.verbose > 0)
...
static void cq_init(CQ *cq) {
    ...
    do { (((&cq->head))->stqh_first) = ((void *)0); (&cq->head)->stqh_last = &(((&cq->head))->stqh_first); } while (0);
...
static CQ_ITEM *cq_pop(CQ *cq) {
    ...
    item = ((&cq->head)->stqh_first);
    if (item != ((void *)0)) {
        do { if (((((&cq->head))->stqh_first) = (((((&cq->head))->stqh_first))->i_next.stqe_next)) == ((void *)0)) (&cq->head)->stqh_last = &(((&cq->head))->stqh_first); } while (0);
    }
...
static void cq_push(CQ *cq, CQ_ITEM *item) {
    pthread_mutex_lock(&cq->lock);
    do { (((item))->i_next.stqe_next) = ((void *)0); *(&cq->head)->stqh_last = (item); (&cq->head)->stqh_last = &(((item))->i_next.stqe_next); } while (0);
    pthread_mutex_unlock(&cq->lock);
}
static CQ_ITEM *cqi_new(CQ *cq) {
...
static void notify_worker_fd(LIBEVENT_THREAD *t, int sfd, enum conn_queue_item_modes mode) {
    CQ_ITEM *item;
    while ( (item = cqi_new(t->ev_queue)) == ((void *)0) ) {
    }
    item->mode = mode;
    item->sfd = sfd;
...
static void setup_thread(LIBEVENT_THREAD *me) {
    ...
    pthread_mutex_init(&me->ion_lock, ((void *)0));
    do { (((&me->ion_head))->stqh_first) = ((void *)0); (&me->ion_head)->stqh_last = &(((&me->ion_head))->stqh_first); } while (0);
    me->ev_queue = malloc(sizeof(struct conn_queue));
    if (me->ev_queue == ((void *)0)) {
    ...
    me->rbuf_cache = cache_create("rbuf", 16384, sizeof(char *));
    if (me->rbuf_cache == ((void *)0)) {
...
static void thread_libevent_ionotify(int fd, short which, void *arg) {
    ...
    do { if (!(((&me->ion_head))->stqh_first == ((void *)0))) { *(&head)->stqh_last = (&me->ion_head)->stqh_first; (&head)->stqh_last = (&me->ion_head)->stqh_last; do { ((((&me->ion_head)))->stqh_first) = ((void *)0); ((&me->ion_head))->stqh_last = &((((&me->ion_head)))->stqh_first); } while (0); } } while (0);
    pthread_mutex_unlock(&me->ion_lock);
    while (!((&head)->stqh_first == ((void *)0))) {
        io_pending_t *io = ((&head)->stqh_first);
        ...
        switch (item->mode) {
            case queue_new_conn:
                ...
                if (c == ((void *)0)) {
                    ...
                    c->thread = me;
                    if (settings.ssl_enabled && c->ssl != ((void *)0)) {
                        ((void) (0));
...
static LIBEVENT_THREAD *select_thread_by_napi_id(int sfd)
{
    ...
    err = getsockopt(sfd, 1, 56, &napi_id, &len);
    if ((err == -1) || (napi_id == 0)) {
        STATS_LOCK();
...
void return_io_pending(io_pending_t *io) {
    ...
    if (((&t->ion_head)->stqh_first == ((void *)0))) {
        do_notify = true;
    }
    do { (((io))->iop_next.stqe_next) = ((void *)0); *(&t->ion_head)->stqh_last = (io); (&t->ion_head)->stqh_last = &(((io))->iop_next.stqe_next); } while (0);
...
void item_remove(item *item) {
    uint32_t hv;
    hv = hash((((char*)&((item)->data)) + (((item)->it_flags & 2) ? sizeof(uint64_t) : 0)), item->nkey);
    item_lock(hv);
    do_item_remove(item);
    item_unlock(hv);
}
...
enum delta_result_type add_delta(LIBEVENT_THREAD *t, const char *key,
    ...
    hv = hash(key, nkey);
    item_lock(hv);
    ret = do_add_delta(t, key, nkey, incr, delta, buf, cas, hv, ((void *)0));
    item_unlock(hv);
    return ret;
}
enum store_item_type store_item(item *item, int comm, LIBEVENT_THREAD *t, int *nbytes, uint64_t *cas, const uint64_t cas_in, bool cas_stale) {
...
void threadlocal_stats_reset(void) {
    ...
    for (ii = 0; ii < settings.num_threads; ++ii) {
        ...
        threads[ii].stats.get_cmds = 0; threads[ii].stats.get_misses = 0; threads[ii].stats.get_expired = 0; threads[ii].stats.get_flushed = 0; threads[ii].stats.touch_cmds = 0; threads[ii].stats.touch_misses = 0; threads[ii].stats.delete_misses = 0; threads[ii].stats.incr_misses = 0; threads[ii].stats.decr_misses = 0; threads[ii].stats.cas_misses = 0; threads[ii].stats.meta_cmds = 0; threads[ii].stats.bytes_read = 0; threads[ii].stats.bytes_written = 0; threads[ii].stats.flush_cmds = 0; threads[ii].stats.conn_yields = 0; threads[ii].stats.auth_cmds = 0; threads[ii].stats.auth_errors = 0; threads[ii].stats.idle_kicks = 0; threads[ii].stats.response_obj_oom = 0; threads[ii].stats.response_obj_count = 0; threads[ii].stats.response_obj_bytes = 0; threads[ii].stats.read_buf_oom = 0; threads[ii].stats.store_too_large = 0; threads[ii].stats.store_no_memory = 0;
        threads[ii].stats.get_extstore = 0; threads[ii].stats.get_aborted_extstore = 0; threads[ii].stats.get_oom_extstore = 0; threads[ii].stats.recache_from_extstore = 0; threads[ii].stats.miss_from_extstore = 0; threads[ii].stats.badcrc_from_extstore = 0;
        threads[ii].stats.proxy_conn_requests = 0; threads[ii].stats.proxy_conn_errors = 0; threads[ii].stats.proxy_conn_oom = 0;
...
void threadlocal_stats_aggregate(struct thread_stats *stats) {
    ...
    for (ii = 0; ii < settings.num_threads; ++ii) {
        pthread_mutex_lock(&threads[ii].stats.mutex);
        stats->get_cmds += threads[ii].stats.get_cmds; stats->get_misses += threads[ii].stats.get_misses; stats->get_expired += threads[ii].stats.get_expired; stats->get_flushed += threads[ii].stats.get_flushed; stats->touch_cmds += threads[ii].stats.touch_cmds; stats->touch_misses += threads[ii].stats.touch_misses; stats->delete_misses += threads[ii].stats.delete_misses; stats->incr_misses += threads[ii].stats.incr_misses; stats->decr_misses += threads[ii].stats.decr_misses; stats->cas_misses += threads[ii].stats.cas_misses; stats->meta_cmds += threads[ii].stats.meta_cmds; stats->bytes_read += threads[ii].stats.bytes_read; stats->bytes_written += threads[ii].stats.bytes_written; stats->flush_cmds += threads[ii].stats.flush_cmds; stats->conn_yields += threads[ii].stats.conn_yields; stats->auth_cmds += threads[ii].stats.auth_cmds; stats->auth_errors += threads[ii].stats.auth_errors; stats->idle_kicks += threads[ii].stats.idle_kicks; stats->response_obj_oom += threads[ii].stats.response_obj_oom; stats->response_obj_count += threads[ii].stats.response_obj_count; stats->response_obj_bytes += threads[ii].stats.response_obj_bytes; stats->read_buf_oom += threads[ii].stats.read_buf_oom; stats->store_too_large += threads[ii].stats.store_too_large; stats->store_no_memory += threads[ii].stats.store_no_memory;
        stats->get_extstore += threads[ii].stats.get_extstore; stats->get_aborted_extstore += threads[ii].stats.get_aborted_extstore; stats->get_oom_extstore += threads[ii].stats.get_oom_extstore; stats->recache_from_extstore += threads[ii].stats.recache_from_extstore; stats->miss_from_extstore += threads[ii].stats.miss_from_extstore; stats->badcrc_from_extstore += threads[ii].stats.badcrc_from_extstore;
        stats->proxy_conn_requests += threads[ii].stats.proxy_conn_requests; stats->proxy_conn_errors += threads[ii].stats.proxy_conn_errors; stats->proxy_conn_oom += threads[ii].stats.proxy_conn_oom;
        stats->proxy_req_active += threads[ii].stats.proxy_req_active;
        for (sid = 0; sid < (63 + 1); sid++) {
            stats->slab_stats[sid].set_cmds += threads[ii].stats.slab_stats[sid].set_cmds; stats->slab_stats[sid].get_hits += threads[ii].stats.slab_stats[sid].get_hits; stats->slab_stats[sid].touch_hits += threads[ii].stats.slab_stats[sid].touch_hits; stats->slab_stats[sid].delete_hits += threads[ii].stats.slab_stats[sid].delete_hits; stats->slab_stats[sid].cas_hits += threads[ii].stats.slab_stats[sid].cas_hits; stats->slab_stats[sid].cas_badval += threads[ii].stats.slab_stats[sid].cas_badval; stats->slab_stats[sid].incr_hits += threads[ii].stats.slab_stats[sid].incr_hits; stats->slab_stats[sid].decr_hits += threads[ii].stats.slab_stats[sid].decr_hits;
        }
        for (sid = 0; sid < 256; sid++) {
            stats->lru_hits[sid] +=
                threads[ii].stats.lru_hits[sid];
            stats->slab_stats[(sid & ~(3<<6))].get_hits +=
                threads[ii].stats.lru_hits[sid];
        }
        stats->read_buf_count += threads[ii].rbuf_cache->total;
        stats->read_buf_bytes += threads[ii].rbuf_cache->total * 16384;
        stats->read_buf_bytes_free += threads[ii].rbuf_cache->freecurr * 16384;
        pthread_mutex_unlock(&threads[ii].stats.mutex);
    }
}
void slab_stats_aggregate(struct thread_stats *stats, struct slab_stats *out) {
    ...
    for (sid = 0; sid < (63 + 1); sid++) {
        out->set_cmds += stats->slab_stats[sid].set_cmds; out->get_hits += stats->slab_stats[sid].get_hits; out->touch_hits += stats->slab_stats[sid].touch_hits; out->delete_hits += stats->slab_stats[sid].delete_hits; out->cas_hits += stats->slab_stats[sid].cas_hits; out->cas_badval += stats->slab_stats[sid].cas_badval; out->incr_hits += stats->slab_stats[sid].incr_hits; out->decr_hits += stats->slab_stats[sid].decr_hits;
    }
}
