...
static void proxy_event_handler(int fd, short which, void *arg);
static void proxy_event_beconn(int fd, short which, void *arg);
...
static int _proxy_beconn_checkconnect(struct mcp_backendconn_s *be) {
    int err = 0;
    if (mcmc_check_nonblock_connect(be->client, &err) != 0) {
        ;
    ...
    gettimeofday(&be->last_failed, ((void *)0));
    be->validating = true;
    return 0;
}
struct mcp_backendconn_s *proxy_choose_beconn(mcp_backend_t *be) {
...
static void _proxy_event_handler_dequeue(proxy_event_thread_t *t) {
    ...
    do { (((&t->be_head))->stqh_first) = ((void *)0); (&t->be_head)->stqh_last = &(((&t->be_head))->stqh_first); } while (0);
    pthread_mutex_lock(&t->mutex);
    do { if (!(((&t->iop_head_in))->stqh_first == ((void *)0))) { *(&head)->stqh_last = (&t->iop_head_in)->stqh_first; (&head)->stqh_last = (&t->iop_head_in)->stqh_last; do { ((((&t->iop_head_in)))->stqh_first) = ((void *)0); ((&t->iop_head_in))->stqh_last = &((((&t->iop_head_in)))->stqh_first); } while (0); } } while (0);
    pthread_mutex_unlock(&t->mutex);
    while (!((&head)->stqh_first == ((void *)0))) {
        io_pending_proxy_t *p = (io_pending_proxy_t *)((&head)->stqh_first);
        p->flushed = false;
        do { if (((((&head))->stqh_first) = (((((&head))->stqh_first))->iop_next.stqe_next)) == ((void *)0)) (&head)->stqh_last = &(((&head))->stqh_first); } while (0);
        p->iop_next.stqe_next = ((void *)0);
        ...
        if (!be->stacked) {
            be->stacked = true;
            do { (((be))->be_next.stqe_next) = ((void *)0); *(&t->be_head)->stqh_last = (be); (&t->be_head)->stqh_last = &(((be))->be_next.stqe_next); } while (0);
        }
    }
}
static void _cleanup_backend(mcp_backend_t *be) {
    if (be->use_logging) {
        if (be->logging.detail) {
            ...
            be->logging.detail = ((void *)0);
        }
    }
    for (int x = 0; x < be->conncount; x++) {
        struct mcp_backendconn_s *bec = &be->be[x];
        if (!be->tunables.down) {
            int pending = event_pending(&bec->main_event, 0x02|0x04|0x01, ((void *)0));
            if (pending != 0) {
                event_del(&bec->main_event);
            }
            pending = event_pending(&bec->write_event, 0x02|0x04|0x01, ((void *)0));
            if (pending != 0) {
                event_del(&bec->write_event);
            }
            pending = event_pending(&bec->timeout_event, 0x01, ((void *)0));
            if (pending != 0) {
                event_del(&bec->timeout_event);
            }
            ((void) (0));
            ((void) (0));
            ;;
            ...
            if (bec->bad) {
                mcp_sharedvm_delta(bec->event_thread->ctx, 3,
...
static void _setup_backend(mcp_backend_t *be) {
    for (int x = 0; x < be->conncount; x++) {
        ...
        event_assign(&bec->main_event, bec->event_thread->base, mcmc_fd(bec->client), 0x04|0x01, _beconn_handler, bec);
        event_assign(&bec->write_event, bec->event_thread->base, mcmc_fd(bec->client), 0x04|0x01, _backend_handler, bec);
        event_assign(&bec->timeout_event, bec->event_thread->base, -1, 0x01, _backend_handler, bec);
...
static void proxy_event_beconn(int fd, short which, void *arg) {
    ...
    do { (((&head))->stqh_first) = ((void *)0); (&head)->stqh_last = &(((&head))->stqh_first); } while (0);
    pthread_mutex_lock(&t->mutex);
    do { if (!(((&t->beconn_head_in))->stqh_first == ((void *)0))) { *(&head)->stqh_last = (&t->beconn_head_in)->stqh_first; (&head)->stqh_last = (&t->beconn_head_in)->stqh_last; do { ((((&t->beconn_head_in)))->stqh_first) = ((void *)0); ((&t->beconn_head_in))->stqh_last = &((((&t->beconn_head_in)))->stqh_first); } while (0); } } while (0);
    pthread_mutex_unlock(&t->mutex);
    mcp_backend_t *be = ((void *)0);
    while (!((&head)->stqh_first == ((void *)0))) {
        be = ((&head)->stqh_first);
        do { if (((((&head))->stqh_first) = (((((&head))->stqh_first))->beconn_next.stqe_next)) == ((void *)0)) (&head)->stqh_last = &(((&head))->stqh_first); } while (0);
        if (be->transferred) {
            _cleanup_backend(be);
        } else {
            be->transferred = true;
...
static void _proxy_flush_backend_queue(mcp_backend_t *be) {
    io_pending_proxy_t *io = ((void *)0);
    ;
    while (!((&be->iop_head)->stqh_first == ((void *)0))) {
        io = (io_pending_proxy_t *)((&be->iop_head)->stqh_first);
        do { if (((((&be->iop_head))->stqh_first) = (((((&be->iop_head))->stqh_first))->iop_next.stqe_next)) == ((void *)0)) (&be->iop_head)->stqh_last = &(((&be->iop_head))->stqh_first); } while (0);
        mcp_resp_set_elapsed(io->client_resp);
        io->client_resp->status = -1;
        io->client_resp->resp.code = 23;
        be->depth--;
        ((void) (0));
...
void proxy_run_backend_queue(be_head_t *head) {
    mcp_backend_t *be;
    for((be) = (((head))->stqh_first); (be); (be) = (((be))->be_next.stqe_next)) {
        be->stacked = false;
        int flags = 0;
        ...
        if (bec->bad) {
            ...
            { pthread_mutex_lock(&ctx->stats_lock); ctx->global_stats.request_failed_depth += be->depth; pthread_mutex_unlock(&ctx->stats_lock); };
            _proxy_flush_backend_queue(be);
            continue;
        }
        do { if (!(((&be->iop_head))->stqh_first == ((void *)0))) { *(&bec->iop_write)->stqh_last = (&be->iop_head)->stqh_first; (&bec->iop_write)->stqh_last = (&be->iop_head)->stqh_last; do { ((((&be->iop_head)))->stqh_first) = ((void *)0); ((&be->iop_head))->stqh_last = &((((&be->iop_head)))->stqh_first); } while (0); } } while (0);
        bec->depth += be->depth;
        be->depth = 0;
...
static void _set_main_event(struct mcp_backendconn_s *be, struct event_base *base, int flags, struct timeval *t, event_callback_fn callback) {
    int pending = event_pending(&be->main_event, 0x02|0x04|0x01, ((void *)0));
    if (pending != 0) {
...
static void _start_write_event(struct mcp_backendconn_s *be) {
    int pending = event_pending(&be->write_event, 0x04|0x01, ((void *)0));
    if (pending != 0) {
        return;
    }
...
static void _start_timeout_event(struct mcp_backendconn_s *be) {
    int pending = event_pending(&be->timeout_event, 0x01, ((void *)0));
    if (pending != 0) {
        return;
    }
...
static void _drive_machine_next(struct mcp_backendconn_s *be, io_pending_proxy_t *p) {
    ((void) (0));
    do { if (((((&be->iop_read))->stqh_first) = (((((&be->iop_read))->stqh_first))->iop_next.stqe_next)) == ((void *)0)) (&be->iop_read)->stqh_last = &(((&be->iop_read))->stqh_first); } while (0);
    be->depth--;
    ((void) (0));
    be->pending_read--;
    ((void) (0));
...
static int proxy_backend_drive_machine(struct mcp_backendconn_s *be) {
    bool stop = false;
    io_pending_proxy_t *p = ((void *)0);
    int flags = 0;
    p = (io_pending_proxy_t *)((&be->iop_read)->stqh_first);
    if (p == ((void *)0)) {
    ...
    switch(be->state) {
        ...
        case mcp_backend_read_end:
            r = p->client_resp;
            if (be->rbufused >= (sizeof("END\r\n")-1)) {
                if (memcmp(be->rbuf, "END\r\n", (sizeof("END\r\n")-1)) != 0) {
                    flags = P_BE_FAIL_ENDSYNC;
                    stop = true;
                    break;
                } else {
                    memcpy(r->buf+r->blen, "END\r\n", (sizeof("END\r\n")-1));
                    r->blen += 5;
                    be->rbufused -= (sizeof("END\r\n")-1);
                    if (be->rbufused > 0) {
                        memmove(be->rbuf, be->rbuf+(sizeof("END\r\n")-1), be->rbufused);
                    }
                }
            } else {
                flags = 0;
        ...
        case mcp_backend_next:
            ...
            if (((&be->iop_read)->stqh_first == ((void *)0))) {
                stop = true;
                if (be->rbufused > 0) {
...
static void _backend_reconnect(struct mcp_backendconn_s *be) {
    ...
    event_assign(&be->write_event, be->event_thread->base, mcmc_fd(be->client), 0x04|0x01, _backend_handler, be);
}
static void proxy_backend_retry_handler(const int fd, const short which, void *arg) {
    ...
    _set_main_event(be, be->event_thread->base, 0x04, &tmp_time, _backend_handler);
}
static void _backend_reschedule(struct mcp_backendconn_s *be) {
    ...
    if (failed) {
        if (!be->bad) {
            ;
            { pthread_mutex_lock(&be->event_thread->ctx->stats_lock); be->event_thread->ctx->global_stats.backend_marked_bad += 1; pthread_mutex_unlock(&be->event_thread->ctx->stats_lock); };
            mcp_sharedvm_delta(be->event_thread->ctx, 3,
                    be->be_parent->label, 1);
            do { logger *myl = ((void *)0); if (((void *)0) == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<11)) logger_log(myl, LOGGER_PROXY_BE_ERROR, ((void *)0), badtext, be->be_parent->name, be->be_parent->port, be->be_parent->label, 0, ((void *)0), 0, retry_time); } while (0);
        }
        be->bad = true;
       _set_main_event(be, be->event_thread->base, 0x01, &tmp_time, proxy_backend_retry_handler);
    } else {
        struct timeval tmp_time = be->tunables.connect;
        { pthread_mutex_lock(&be->event_thread->ctx->stats_lock); be->event_thread->ctx->global_stats.backend_failed += 1; pthread_mutex_unlock(&be->event_thread->ctx->stats_lock); };
        _backend_reconnect(be);
        event_callback_fn _backend_handler = &proxy_beconn_handler;
        if (be->be_parent->tunables.use_tls) {
            _backend_handler = &proxy_beconn_tls_handler;
...
static void _reset_bad_backend(struct mcp_backendconn_s *be, enum proxy_be_failures err) {
    ...
    ;
    int depth = be->depth;
    while (!((&be->iop_write)->stqh_first == ((void *)0))) {
        io = (io_pending_proxy_t *)((&be->iop_write)->stqh_first);
        do { if (((((&be->iop_write))->stqh_first) = (((((&be->iop_write))->stqh_first))->iop_next.stqe_next)) == ((void *)0)) (&be->iop_write)->stqh_last = &(((&be->iop_write))->stqh_first); } while (0);
        mcp_resp_set_elapsed(io->client_resp);
        io->client_resp->status = -1;
        io->client_resp->resp.code = 23;
        be->depth--;
    ...
    while (!((&be->iop_read)->stqh_first == ((void *)0))) {
        io = (io_pending_proxy_t *)((&be->iop_read)->stqh_first);
        do { if (((((&be->iop_read))->stqh_first) = (((((&be->iop_read))->stqh_first))->iop_next.stqe_next)) == ((void *)0)) (&be->iop_read)->stqh_last = &(((&be->iop_read))->stqh_first); } while (0);
        mcp_resp_set_elapsed(io->client_resp);
        io->client_resp->status = -1;
        io->client_resp->resp.code = 23;
    ...
    do { (((&be->iop_write))->stqh_first) = ((void *)0); (&be->iop_write)->stqh_last = &(((&be->iop_write))->stqh_first); } while (0);
    do { (((&be->iop_read))->stqh_first) = ((void *)0); (&be->iop_read)->stqh_last = &(((&be->iop_read))->stqh_first); } while (0);
    if (!be->bad) {
        do { logger *myl = ((void *)0); if (((void *)0) == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<11)) logger_log(myl, LOGGER_PROXY_BE_ERROR, ((void *)0), proxy_be_failure_text[err], be->be_parent->name, be->be_parent->port, be->be_parent->label, depth, be->rbuf, be->rbufused, 0); } while (0);
    }
    be->rbufused = 0;
    be->pending_read = 0;
...
static int _prep_pending_write(struct mcp_backendconn_s *be, int *count, int *bytes, bool *iov_limit) {
    ...
    io = (io_pending_proxy_t *)((&be->iop_write)->stqh_first);
    ((void) (0));
    for (; io; io = (io_pending_proxy_t *)((io)->iop_next.stqe_next)) {
        ((void) (0));
        if (io->iovcnt + iovused > 1024) {
            *iov_limit = true;
            break;
        }
...
static void _post_pending_write(struct mcp_backendconn_s *be, ssize_t sent) {
    io_pending_proxy_t *io = (io_pending_proxy_t *)((&be->iop_write)->stqh_first);
    while (!((&be->iop_write)->stqh_first == ((void *)0))) {
        io = (io_pending_proxy_t *)((&be->iop_write)->stqh_first);
        bool flushed = true;
        ((void) (0));
        ...
        if (flushed) {
            do { if (((((&be->iop_write))->stqh_first) = (((((&be->iop_write))->stqh_first))->iop_next.stqe_next)) == ((void *)0)) (&be->iop_write)->stqh_last = &(((&be->iop_write))->stqh_first); } while (0);
            do { ((((io_pending_t *)io))->iop_next.stqe_next) = ((void *)0); *(&be->iop_read)->stqh_last = ((io_pending_t *)io); (&be->iop_read)->stqh_last = &((((io_pending_t *)io))->iop_next.stqe_next); } while (0);
            be->pending_read++;
        }
        if (sent <= 0) {
            ((void) (0));
...
static int _flush_pending_write(struct mcp_backendconn_s *be) {
    ...
    if (((&be->iop_write)->stqh_first == ((void *)0))) {
        return 0;
    }
    int count = 0;
    int bytes = 0;
    ...
    if (sent > 0) {
        if (bytes == sent && !iov_limit) {
            ...
            do { if (!(((&be->iop_write))->stqh_first == ((void *)0))) { *(&be->iop_read)->stqh_last = (&be->iop_write)->stqh_first; (&be->iop_read)->stqh_last = (&be->iop_write)->stqh_last; do { ((((&be->iop_write)))->stqh_first) = ((void *)0); ((&be->iop_write))->stqh_last = &((((&be->iop_write)))->stqh_first); } while (0); } } while (0);
        } else {
...
static void proxy_bevalidate_tls_handler(const int fd, const short which, void *arg) {
    ...
    if (which & 0x02) {
        ...
        if (read > 0) {
            ...
            if (status == -1) {
                if (r.code == 5) {
                    _set_main_event(be, be->event_thread->base, 0x02, &tmp_time, proxy_bevalidate_tls_handler);
                    return;
                }
                _reset_bad_backend(be, P_BE_FAIL_READVALIDATE);
                return;
    ...
    if (!be->validating) {
        _set_main_event(be, be->event_thread->base, 0x02|0x10, ((void *)0), proxy_backend_tls_handler);
        if (be->bad) {
            mcp_sharedvm_delta(be->event_thread->ctx, 3,
                    be->be_parent->label, -1);
        }
...
static void proxy_beconn_tls_handler(const int fd, const short which, void *arg) {
    ...
    if (ret == MCP_TLS_NEEDIO) {
        _set_main_event(be, be->event_thread->base, 0x02, &tmp_time, proxy_beconn_tls_handler);
        return;
    } else if (ret == 1) {
        if (0 != MCP_TLS_OK) {
            _reset_bad_backend(be, P_BE_FAIL_BADVALIDATE);
...
static void proxy_beconn_handler(const int fd, const short which, void *arg) {
    ...
    if (which & 0x04) {
        ...
        if (be->connecting) {
            ...
            _set_main_event(be, be->event_thread->base, 0x02, &tmp_time, proxy_beconn_handler);
        }
        if (!be->validating) {
    ...
    if (which & 0x02) {
        ((void) (0));
        int read = recv(mcmc_fd(be->client), be->rbuf + be->rbufused, 16384 - be->rbufused, 0);
        if (read > 0) {
            mcmc_resp_t r;
            be->rbufused += read;
            int status = mcmc_parse_buf(be->rbuf, be->rbufused, &r);
    ...
    if (!be->validating) {
        _set_main_event(be, be->event_thread->base, 0x02|0x10, ((void *)0), proxy_backend_handler);
        if (be->bad) {
            mcp_sharedvm_delta(be->event_thread->ctx, 3,
                    be->be_parent->label, -1);
        }
...
void proxy_init_event_thread(proxy_event_thread_t *t, proxy_ctx_t *ctx, struct event_base *base) {
    ...
    do { (((&t->iop_head_in))->stqh_first) = ((void *)0); (&t->iop_head_in)->stqh_last = &(((&t->iop_head_in))->stqh_first); } while (0);
    do { (((&t->beconn_head_in))->stqh_first) = ((void *)0); (&t->beconn_head_in)->stqh_last = &(((&t->beconn_head_in))->stqh_first); } while (0);
    pthread_mutex_init(&t->mutex, ((void *)0));
    pthread_cond_init(&t->cond, ((void *)0));
    if (base == ((void *)0)) {
    ...
    event_set(&t->notify_event, t->event_fd,
          0x02 | 0x10, proxy_event_handler, t);
    event_set(&t->beconn_event, t->be_event_fd,
          0x02 | 0x10, proxy_event_beconn, t);
