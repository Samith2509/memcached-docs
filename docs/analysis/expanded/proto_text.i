...
void complete_nread_ascii(conn *c) {
    ...
    int nbytes = 0;
    if ((it->it_flags & 32) == 0) {
        if (strncmp(((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)) + it->nbytes - 2, "\r\n", 2) == 0) {
            is_valid = true;
        }
    } else {
        char buf[2];
...
int try_read_command_asciiauth(conn *c) {
    ...
    if (!c->sasl_started) {
        ...
        if (mcmc_tokenize(st, el - st, &tok, 255) != 0) {
            conn_set_state(c, conn_closing);
            return 1;
        }
        ...
        if (tok.ntokens < 5
                || strncmp(st, "set", 3) != 0
                || mcmc_token_get_u32(st, &tok, 4, &size) != 0) {
            if (!c->resp) {
                if (!resp_start(c)) {
                    conn_set_state(c, conn_closing);
                    return 1;
    ...
    mid = _Generic (0 ? (cont) : (void *) 1, const void *: (const void *) (memchr (cont, ' ', c->rlbytes)), default: memchr (cont, ' ', c->rlbytes));
    if (!mid) {
...
static void process_get_command(conn *c, LIBEVENT_THREAD *t, mcp_parser_t *pr, parser_storage_get_cb storage_cb, bool return_cas, bool should_touch) {
    ...
    while (klen != 0) {
        ...
        if (process_get_cmd(t, curkey, klen, c->resp, storage_get_item, exptime, return_cas, should_touch) != 0) {
            process_get_command_err(c, ((void *)0));
            return;
        }
        if (resp->io_pending) {
            resp->io_pending->c = c;
        ...
        while (curkey != kend) {
            if (*curkey == ' ') {
                ...
                const char *s = _Generic (0 ? (curkey) : (void *) 1, const void *: (const void *) (memchr (curkey, ' ', kend - curkey)), default: memchr (curkey, ' ', kend - curkey));
                if (s != ((void *)0)) {
                    klen = s - curkey;
                } else {
                    klen = kend - curkey;
...
static void process_stat(conn *c, mcp_parser_t *pr) {
    ...
    const char *subcommand = mcmc_token_get(pr->request, &pr->tok, 1, &len);
    ((void) (0));
    if (ntokens < 1) {
    ...
    if (ntokens == 1) {
        ...
        if ((mcmc_token_get_u32(pr->request, &pr->tok, 2, &id) != 0) ||
            (mcmc_token_get_u32(pr->request, &pr->tok, 3, &limit) != 0)) {
...
static void process_meta_command(conn *c, mcp_parser_t *pr) {
    ...
    bool overflow;
    item *it = limited_get(key, nkey, c->thread, 0, false, false, &overflow);
    if (it) {
        mc_resp *resp = c->resp;
        size_t total = 0;
        size_t ret;
        ...
        total += 3;
        if (it->it_flags & 4096) {
            total += base64_encode((unsigned char *) (((char*)&((it)->data)) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), it->nkey, (unsigned char *)resp->wbuf + total, 1024 - total);
        } else {
            memcpy(resp->wbuf + total, (((char*)&((it)->data)) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), it->nkey);
            total += it->nkey;
        }
        resp->wbuf[total] = ' ';
        total++;
        ret = snprintf(resp->wbuf + total, 1024 - (it->nkey + 12),
                ...
                (it->it_flags & 8) ? "yes" : "no",
                ((it)->slabs_clsid & ~(3<<6)),
                (unsigned long) (sizeof(struct _stritem) + (it)->nkey + 1 + (it)->nbytes + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)));
        item_remove(it);
...
static void process_verbosity_command(conn *c, mcp_parser_t *pr) {
    ((void) (0));
    uint32_t level;
    if (mcmc_token_get_u32(pr->request, &pr->tok, 1, &level) != 0) {
        out_string(c, "CLIENT_ERROR bad command line format");
        return;
    }
    settings.verbose = level > 2 ? 2 : level;
    out_string(c, "OK");
    return;
}
static void process_slabs_automove_command(conn *c, mcp_parser_t *pr) {
    ...
    if (strncmp(subcmd, "ratio", len) == 0) {
        ...
        if (ntokens < 4 || mcmc_token_get_u32(pr->request, &pr->tok, 3, &level) != 0) {
            out_string(c, "CLIENT_ERROR bad command line format");
            return;
        }
        ...
        if (mcmc_token_get_u32(pr->request, &pr->tok, 2, &level) != 0) {
            out_string(c, "CLIENT_ERROR bad command line format");
            return;
        }
        if (level == 0) {
...
static void process_memlimit_command(conn *c, mcp_parser_t *pr) {
    uint32_t memlimit;
    ((void) (0));
    if (mcmc_token_get_u32(pr->request, &pr->tok, 1, &memlimit) != 0) {
        out_string(c, "ERROR");
    } else {
        if (memlimit < 8) {
...
static void process_shutdown_command(conn *c, mcp_parser_t *pr) {
    ...
    if (pr->tok.ntokens == 1) {
        ...
        conn_set_state(c, conn_closing);
        raise(2);
    } else if (pr->tok.ntokens == 2 && mc_prcmp(pr, 1, "graceful") == 0) {
        c->close_reason = SHUTDOWN_CLOSE;
        conn_set_state(c, conn_closing);
        raise(10);
    } else {
...
static void process_slabs_command(conn *c, mcp_parser_t *pr) {
    ...
    if (ntokens == 4 && strncmp(subcmd, "reassign", len) == 0) {
        ...
        rv = slabs_reassign(settings.slab_rebal, src, dst, 1);
        switch (rv) {
        case REASSIGN_OK:
            out_string(c, "OK");
            break;
...
static void process_lru_crawler_command(conn *c, mcp_parser_t *pr) {
    ...
    if (ntokens == 3 && strncmp(subcmd, "crawl", len) == 0) {
        ...
        sc_buf[len] = '\0';
        rv = lru_crawler_crawl(sc_buf, CRAWLER_EXPIRED, ((void *)0), 0,
    ...
    if (ret == 0) {
        ...
        switch (pr.command) {
            ...
            case CMD_ME:
                ...
                break;
            case CMD_GET:
                process_get_command(c, t, &pr, storage_get_item, false, false);
                break;
            case CMD_GETS:
                process_get_command(c, t, &pr, storage_get_item, true, false);
                break;
            case CMD_GAT:
                process_get_command(c, t, &pr, storage_get_item, false, true);
                break;
            case CMD_GATS:
                process_get_command(c, t, &pr, storage_get_item, true, true);
                break;
             case CMD_DELETE:
             ...
             case CMD_TOUCH:
                ...
                break;
             case CMD_SET:
                process_update_command(c, &pr, resp, 2, false);
                break;
            case CMD_ADD:
                process_update_command(c, &pr, resp, 1, false);
                break;
            case CMD_APPEND:
                process_update_command(c, &pr, resp, 4, false);
                break;
            case CMD_PREPEND:
                process_update_command(c, &pr, resp, 5, false);
                break;
            case CMD_REPLACE:
                process_update_command(c, &pr, resp, 3, false);
                break;
            case CMD_CAS:
                process_update_command(c, &pr, resp, 6, true);
                break;
            case CMD_QUIT:
