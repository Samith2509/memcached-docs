...
static void mcp_rcontext_cleanup(lua_State *L, mcp_funcgen_t *fgen, mcp_rcontext_t *rctx, int fgen_idx) {
    luaL_unref(L, (-1000000 - 1000), rctx->coroutine_ref);
    luaL_unref(L, (-1000000 - 1000), rctx->function_ref);
    if (rctx->request_ref) {
        luaL_unref(L, (-1000000 - 1000), rctx->request_ref);
    }
    ((void) (0));
    for (int x = 0; x < fgen->max_queues; x++) {
        struct mcp_rqueue_s *rqu = &rctx->qslots[x];
        if (rqu->obj_type == 1 || rqu->obj_type == 2) {
        } else if (rqu->obj_type == 3) {
            mcp_rcontext_t *subrctx = rqu->obj;
            lua_rawgeti(L, (-1000000 - 1000), subrctx->fgen->self_ref);
            mcp_rcontext_cleanup(L, subrctx->fgen, subrctx, lua_absindex(L, -1));
            lua_settop(L, -(1)-1);
        } else if (rqu->obj_type != 0) {
            ((void) (0));
        }
        if (rqu->res_ref) {
            luaL_unref(L, (-1000000 - 1000), rqu->res_ref);
            rqu->res_ref = 0;
        }
        if (rqu->cb_ref) {
            luaL_unref(L, (-1000000 - 1000), rqu->cb_ref);
            rqu->cb_ref = 0;
        }
    }
    if (rctx->uobj_count) {
        ...
        for (int x = fgen->max_queues; x < lim; x++) {
            struct mcp_rqueue_s *rqu = &rctx->qslots[x];
            luaL_unref(L, (-1000000 - 1000), rqu->obj_ref);
            rqu->obj_ref = 0;
        }
    }
    if (event_pending(&rctx->timeout_event, 0x01, ((void *)0))) {
    ...
    mcp_sharedvm_delta(t->proxy_ctx, 2, fgen->name, -1);
}
static void _mcplib_funcgen_cache(mcp_funcgen_t *fgen, mcp_rcontext_t *rctx) {
    bool do_cache = true;
    if (fgen->free > fgen->total/2 - 1) {
        if (fgen->free_pressure++ > 5000) {
            struct timespec now;
            clock_gettime(0, &now);
            if (fgen->free_waiter.tv_sec == 0) {
                fgen->free_waiter.tv_sec = now.tv_sec + 60;
            }
            if (now.tv_sec >= fgen->free_waiter.tv_sec) {
                do_cache = false;
            }
            fgen->free_pressure -= 200;
        }
    } else {
    ...
    if (do_cache) {
        ...
        lua_rawgeti(L, (-1000000 - 1000), fgen->self_ref);
...
static int _mcplib_funcgen_gencall(lua_State *L) {
    ...
    (lua_getfield(L, (-1000000 - 1000), ("mcp.rcontext")));
    ...
    for (int x = 0; x < fgen->max_queues; x++) {
        ...
        if (frqu->obj_type == 1 || frqu->obj_type == 2) {
            rqu->obj_ref = 0;
            rqu->obj = frqu->obj;
            mcp_resp_t *r = mcp_prep_bare_resobj(L, fgen->thread);
            rqu->res_ref = luaL_ref(L, (-1000000 - 1000));
            rqu->res_obj = r;
        } else if (frqu->obj_type == 3) {
            ...
            if (subrctx->request_ref) {
                luaL_unref(L, (-1000000 - 1000), subrctx->request_ref);
                subrctx->request_ref = 0;
                subrctx->request = ((void *)0);
            }
            subrctx->parent = rc;
    ...
    if (fgen->argument_ref) {
        lua_rawgeti(L, (-1000000 - 1000), fgen->argument_ref);
        call_argnum++;
    }
    lua_callk(L, (call_argnum), (1), 0, ((void *)0));
    if (!(lua_type(L, (-1)) == 6)) {
        proxy_lua_error(L, "function generator didn't return a function");
        return 0;
    }
    rc->function_ref = luaL_ref(L, (-1000000 - 1000));
    fgen->total++;
    lua_getiuservalue(L, fgen_idx, 1);
    lua_rotate(L, -2, 1);
    rc->self_ref = luaL_ref(L, -2);
    lua_settop(L, -(1)-1);
    _mcplib_funcgen_cache(fgen, rc);
    rc->Lc = lua_newthread(L);
    ((void) (0));
    rc->coroutine_ref = luaL_ref(L, (-1000000 - 1000));
    LIBEVENT_THREAD *t = (*(LIBEVENT_THREAD **)((void *)((char *)(L) - (sizeof(void *)))));
    mcp_sharedvm_delta(t->proxy_ctx, 2, fgen->name, 1);
    event_assign(&rc->timeout_event, t->base, -1, 0x01, mcp_funcgen_wait_handler, rc);
    return 1;
}
static void _mcp_funcgen_return_rctx(mcp_rcontext_t *rctx) {
    ...
    if (res != 0) {
        ...
        ((void) (0));
        rctx->coroutine_ref = luaL_ref(L, (-1000000 - 1000));
    } else {
        lua_settop(rctx->Lc, 0);
    }
    rctx->wait_mode = QWAIT_IDLE;
    ...
    for (int x = 0; x < fgen->max_queues; x++) {
        ...
        if (rqu->res_ref) {
            if (rqu->res_obj) {
                ...
                luaL_unref(rctx->Lc, (-1000000 - 1000), rqu->res_ref);
                rqu->res_ref = 0;
            }
        }
        if (rqu->req_ref) {
            luaL_unref(rctx->Lc, (-1000000 - 1000), rqu->req_ref);
            rqu->req_ref = 0;
        }
        ((void) (0));
...
void mcp_funcgen_return_rctx(mcp_rcontext_t *rctx) {
    ...
    { pthread_mutex_lock(&rctx->fgen->thread->stats.mutex); rctx->fgen->thread->stats.proxy_req_active -= 1; pthread_mutex_unlock(&rctx->fgen->thread->stats.mutex); };
    ((void) (0));
    _mcp_funcgen_return_rctx(rctx);
    _mcplib_funcgen_cache(fgen, rctx);
}
mcp_rcontext_t *mcp_funcgen_get_rctx(lua_State *L, int fgen_ref, mcp_funcgen_t *fgen) {
    mcp_rcontext_t *rctx = ((void *)0);
    if (fgen->free == 0) {
        fgen->free_pressure = 0;
        fgen->free_waiter.tv_sec = 0;
        lua_pushcclosure(L, _mcplib_funcgen_gencall, 0);
        lua_rawgeti(L, (-1000000 - 1000), fgen_ref);
        lua_rawgeti(L, (-1000000 - 1000), fgen->generator_ref);
        int res = lua_pcallk(L, (2), (1), (0), 0, ((void *)0));
        if (res != 0) {
            do { logger *myl = ((void *)0); if (((void *)0) == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<11)) logger_log(myl, LOGGER_PROXY_ERROR, ((void *)0), lua_tolstring(L, (-1), ((void *)0))); } while (0);
            lua_settop(L, 0);
            return ((void *)0);
        }
        lua_settop(L, -(1)-1);
...
mcp_rcontext_t *mcp_funcgen_start(lua_State *L, mcp_funcgen_t *fgen, mcp_parser_t *pr) {
    ...
    if (rctx->request_ref == 0) {
        mcp_request_t *rq = lua_newuserdatauv(L, sizeof(mcp_request_t) + 250 * 2, 0);
        memset(rq, 0, sizeof(mcp_request_t));
        (lua_getfield(L, (-1000000 - 1000), ("mcp.request")));
        lua_setmetatable(L, -2);
        rctx->request_ref = luaL_ref(L, (-1000000 - 1000));
        rctx->request = rq;
    }
    { pthread_mutex_lock(&fgen->thread->stats.mutex); fgen->thread->stats.proxy_req_active += 1; pthread_mutex_unlock(&fgen->thread->stats.mutex); };
    return rctx;
}
static void mcp_funcgen_cleanup(lua_State *L, mcp_funcgen_t *fgen) {
    int fgen_idx = 0;
    ...
    if (fgen->self_ref) {
        ...
        luaL_unref(L, (-1000000 - 1000), fgen->self_ref);
        fgen->self_ref = 0;
    } else if (fgen->closed) {
        return;
    } else {
    ...
    LIBEVENT_THREAD *t = (*(LIBEVENT_THREAD **)((void *)((char *)(L) - (sizeof(void *)))));
    mcp_sharedvm_delta(t->proxy_ctx, 1, fgen->name, -1);
    ...
    if (fgen->argument_ref) {
        luaL_unref(L, (-1000000 - 1000), fgen->argument_ref);
        fgen->argument_ref = 0;
    }
    if (fgen->generator_ref) {
        luaL_unref(L, (-1000000 - 1000), fgen->generator_ref);
...
void mcp_funcgen_reference(lua_State *L) {
    ...
    if (fgen->self_ref) {
        ...
        fgen->self_ref = luaL_ref(L, (-1000000 - 1000));
        fgen->refcount = 1;
    }
    ;
}
...
int mcplib_funcgenbare_new(lua_State *L) {
    ...
    lua_pushvalue(L, -1);
    int gen_ref = luaL_ref(L, (-1000000 - 1000));
    mcplib_funcgen_new(L);
    mcp_funcgen_t *fgen = lua_touserdata(L, -1);
    strncpy(fgen->name, "anonymous", 80);
    mcp_sharedvm_delta(fgen->thread->proxy_ctx, 1, fgen->name, 1);
...
int mcplib_funcgen_new(lua_State *L) {
    ...
    (lua_getfield(L, (-1000000 - 1000), ("mcp.funcgen")));
    lua_setmetatable(L, -2);
...
int mcplib_funcgen_new_handle(lua_State *L) {
    mcp_funcgen_t *fgen = lua_touserdata(L, 1);
    mcp_pool_proxy_t *pp = ((void *)0);
    mcp_funcgen_t *fg = ((void *)0);
    void *test = ((void *)0);
    if (fgen->ready) {
        proxy_lua_error(L, "cannot modify function generator after calling ready");
        return 0;
    }
    if ((pp = luaL_testudata(L, 2, "mcp.pool_proxy")) != ((void *)0)) {
    } else if ((test = luaL_testudata(L, 2, "mcp.internal_be")) != ((void *)0)) {
    } else if ((fg = luaL_testudata(L, 2, "mcp.funcgen")) != ((void *)0)) {
        if (fg->is_router) {
    ...
    if (pp) {
        rqu->obj_ref = luaL_ref(L, (-1000000 - 1000));
        rqu->obj_type = 1;
        rqu->obj = pp;
    } else if (test) {
        rqu->obj_ref = luaL_ref(L, (-1000000 - 1000));
...
int mcplib_funcgen_ready(lua_State *L) {
    ...
    if (lua_getfield(L, 2, "f") != 6) {
        proxy_lua_error(L, "Must specify generator function ('f') to fgen:ready");
        return 0;
    }
    fgen->generator_ref = luaL_ref(L, (-1000000 - 1000));
    if (lua_getfield(L, 2, "a") != 0) {
        fgen->argument_ref = luaL_ref(L, (-1000000 - 1000));
    } else {
        lua_settop(L, -(1)-1);
    }
    if (lua_getfield(L, 2, "n") == 4) {
        size_t len = 0;
        const char *name = lua_tolstring(L, -1, &len);
        strncpy(fgen->name, name, 80);
    } else {
        strncpy(fgen->name, "anonymous", 80);
        lua_settop(L, -(1)-1);
    }
    if (lua_getfield(L, 2, "u") == 3) {
        int uobj_queues = luaL_checkinteger(L, -1);
        if (uobj_queues < 1 || uobj_queues > (255)) {
            do { lua_pushfstring(L, "user obj ('u') in fgen:ready must be between 1 and %d", (255)); lua_error(L); } while (0);
            return 0;
        }
...
int mcplib_rcontext_handle_set_cb(lua_State *L) {
    ...
    luaL_checktype(L, 2, 3);
    luaL_checktype(L, 3, 6);
    int handle = lua_tointegerx(L,(2),((void *)0));
    ...
    rqu->cb_ref = luaL_ref(L, (-1000000 - 1000));
    return 0;
}
static void _mcplib_rcontext_queue(lua_State *L, mcp_rcontext_t *rctx, mcp_request_t *rq, int handle) {
    ...
    if (rqu->obj_type == 3) {
        mcp_rcontext_t *subrctx = rqu->obj;
        lua_pushvalue(L, 2);
        lua_rawgeti(subrctx->Lc, (-1000000 - 1000), subrctx->function_ref);
        lua_xmove(L, subrctx->Lc, 1);
    }
    rqu->req_ref = luaL_ref(L, (-1000000 - 1000));
...
int mcplib_rcontext_enqueue(lua_State *L) {
    ...
    if (type == 3) {
        int handle = lua_tointegerx(L,(3),((void *)0));
        lua_pushvalue(L, 2);
        ...
        for (int x = 0; x < len; x++) {
            ...
            int handle = lua_tointegerx(L,(4),((void *)0));
            lua_settop(L, -(1)-1);
            lua_pushvalue(L, 2);
            _mcplib_rcontext_queue(L, rctx, rq, handle);
        }
...
static void _mcp_resume_rctx_process_error(mcp_rcontext_t *rctx, struct mcp_rqueue_s *rqu) {
    ...
    r->resp.code = 23;
    ((void) (0));
    rqu->res_ref = luaL_ref(rctx->Lc, (-1000000 - 1000));
...
static void _mcp_start_rctx_process_error(mcp_rcontext_t *rctx, struct mcp_rqueue_s *rqu) {
    ...
    ((void) (0));
    rqu->res_ref = luaL_ref(rctx->Lc, (-1000000 - 1000));
    io_pending_proxy_t *p = mcp_queue_rctx_io(rctx->parent, ((void *)0), ((void *)0), r);
...
static void mcp_start_subrctx(mcp_rcontext_t *rctx) {
    ...
    if (res == 0) {
        ...
        mcp_resp_t *r = ((void *)0);
        if (type == 7 && (r = luaL_testudata(rctx->Lc, 1, "mcp.response")) != ((void *)0)) {
            ((void) (0));
            rqu->res_ref = luaL_ref(rctx->Lc, (-1000000 - 1000));
            io_pending_proxy_t *p = mcp_queue_rctx_io(rctx->parent, ((void *)0), ((void *)0), r);
            p->return_cb = proxy_return_rqu_cb;
            ...
            io_pending_proxy_t *p = mcp_queue_rctx_io(rctx->parent, ((void *)0), ((void *)0), ((void *)0));
            p->return_cb = proxy_return_rqu_cb;
            p->queue_handle = rctx->parent_handle;
            p->background = true;
        } else {
...
int mcp_process_rqueue_return(mcp_rcontext_t *rctx, int handle, mcp_resp_t *res) {
    ...
    if (rqu->cb_ref) {
        lua_settop(rctx->Lc, 0);
        lua_rawgeti(rctx->Lc, (-1000000 - 1000), rqu->cb_ref);
        lua_rawgeti(rctx->Lc, (-1000000 - 1000), rqu->res_ref);
        lua_rawgeti(rctx->Lc, (-1000000 - 1000), rqu->req_ref);
        if (lua_pcallk(rctx->Lc, (2), (2), (0), 0, ((void *)0)) != 0) {
            do { logger *myl = ((void *)0); if (((void *)0) == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<11)) logger_log(myl, LOGGER_PROXY_ERROR, ((void *)0), lua_tolstring(rctx->Lc, (-1), ((void *)0))); } while (0);
        } else if (lua_isinteger(rctx->Lc, 1)) {
            enum mcp_rqueue_e mode = lua_tointegerx(rctx->Lc,(1),((void *)0));
            switch (mode) {
                case QWAIT_GOOD:
                    flag = (1<<3);
                    break;
...
void mcp_run_rcontext_handle(mcp_rcontext_t *rctx, int handle) {
    struct mcp_rqueue_s *rqu = ((void *)0);
    rqu = &rctx->qslots[handle];
    if (rqu->state == RQUEUE_QUEUED) {
        rqu->state = RQUEUE_ACTIVE;
        if (rqu->obj_type == 1) {
            mcp_request_t *rq = rqu->rq;
            mcp_backend_t *be = mcplib_pool_proxy_call_helper(rqu->obj, (&(&rq->pr)->request[(&rq->pr)->tok.tokens[(&rq->pr)->keytoken]]), rq->pr.klen);
            ...
            if (resp == ((void *)0)) {
                ...
                r->status = -1;
                r->resp.code = 23;
                io_pending_proxy_t *p = mcp_queue_rctx_io(rctx, ((void *)0), ((void *)0), rqu->res_obj);
                p->return_cb = proxy_return_rqu_cb;
                p->queue_handle = handle;
                p->background = true;
                rctx->pending_reqs++;
                ...
                io_queue_t *q = thread_io_queue_get(rctx->fgen->thread, 1);
                io_pending_proxy_t *io = (io_pending_proxy_t *)resp->io_pending;
                io->queue_handle = handle;
                io->client_resp = rqu->res_obj;
                do { ((((io_pending_t *)io))->iop_next.stqe_next) = ((void *)0); *(&q->stack)->stqh_last = ((io_pending_t *)io); (&q->stack)->stqh_last = &((((io_pending_t *)io))->iop_next.stqe_next); } while (0);
                io->rctx = rctx;
                io->c = rctx->c;
...
static inline int _mcplib_rcontext_wait_prep(lua_State *L, mcp_rcontext_t *rctx, int argc) {
    ...
    if (argc > 2) {
        mode = lua_tointegerx(L,(3),((void *)0));
    }
    switch (mode) {
        case QWAIT_ANY:
        case QWAIT_OK:
...
int mcplib_rcontext_wait_cond(lua_State *L) {
    ...
    if (rctx->wait_count == 0) {
        io_pending_proxy_t *p = mcp_queue_rctx_io(rctx, ((void *)0), ((void *)0), ((void *)0));
    ...
    lua_pushinteger(L, 2);
    return lua_yieldk(L, (1), 0, ((void *)0));
}
int mcplib_rcontext_enqueue_and_wait(lua_State *L) {
    mcp_rcontext_t *rctx = lua_touserdata(L, 1);
    mcp_request_t *rq = luaL_checkudata(L, 2, "mcp.request");
...
int mcplib_rcontext_res_good(lua_State *L) {
    ...
    if (rqu->flags & (1<<3)) {
        lua_rawgeti(L, (-1000000 - 1000), rqu->res_ref);
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int mcplib_rcontext_result(lua_State *L) {
    ...
    if (rqu->flags & ((1<<5)|(1<<4)|(1<<3))) {
        lua_rawgeti(L, (-1000000 - 1000), rqu->res_ref);
        lua_pushinteger(L, rqu->flags & ((1<<5)|(1<<4)|(1<<3)));
    } else {
        lua_pushnil(L);
        lua_pushnil(L);
    }
...
int mcplib_rcontext_best_result(lua_State *L) {
    ...
    if ((lua_type(L, (2)) == 5)) {
        ...
        for (int x = 0; x < len; x++) {
            ...
            if (!rqu->flags) {
                continue;
            } else if (rqu->flags & (1<<3)) {
                final_handle = handle;
                break;
            } else if (rqu->flags & (1<<4)) {
                final_handle = handle;
                final_flag = (1<<4);
            } else if (final_flag != (1<<4)) {
                final_handle = handle;
            }
        }
        if (final_handle != -1) {
...
int mcplib_rcontext_request_new(lua_State *L) {
    ...
    _mcplib_rcontext_ref_uobj(L, rctx, rq, 5);
    return 1;
}
int mcplib_rcontext_response_new(lua_State *L) {
    mcp_rcontext_t *rctx = lua_touserdata(L, 1);
    ...
    lua_setmetatable(L, -2);
    _mcplib_rcontext_ref_uobj(L, rctx, r, 6);
    return 1;
}
void mcp_rcontext_push_rqu_res(lua_State *L, mcp_rcontext_t *rctx, int handle) {
    struct mcp_rqueue_s *rqu = &rctx->qslots[handle];
...
static inline const char *_mcp_router_shortsep(const char *key, const int klen, const char needle, size_t *len) {
    ...
    const char *lookup = ((void *)0);
    end = _Generic (0 ? (key) : (void *) 1, const void *: (const void *) (memchr (key, needle, klen)), default: memchr (key, needle, klen));
    if (end == ((void *)0)) {
        lookup = key;
    } else {
        lookup = key;
...
static inline const char *_mcp_router_longsep(const char *key, const int klen, const char *needle, size_t *len) {
    ...
    end = _Generic (0 ? (key) : (void *) 1, const void *: (const void *) (memchr (key, needle[0], klen)), default: memchr (key, needle[0], klen));
    if (end == ((void *)0)) {
        return key;
    }
...
static mcp_funcgen_t *mcp_funcgen_route(lua_State *L, mcp_funcgen_t *fgen, mcp_parser_t *pr) {
    ...
    lua_rawgeti(L, (-1000000 - 1000), fr->map_ref);
...
static int mcp_funcgen_router_cleanup(lua_State *L, mcp_funcgen_t *fgen) {
    ...
    if (fr->map_ref) {
        ...
        lua_settop(L, -(1)-1);
        luaL_unref(L, (-1000000 - 1000), fr->map_ref);
        fr->map_ref = 0;
    }
...
static const char *_mcplib_router_new_check(lua_State *L, const char *arg, size_t *len) {
    ...
    if (type == 4) {
        ...
        if (*len == 0) {
            do { lua_pushfstring(L, "must pass a non-zero length string to %s in mcp.router_new", arg); lua_error(L); } while (0);
        } else if (*len > 5) {
            do { lua_pushfstring(L, "%s is too long in mcp.router_new", arg); lua_error(L); } while (0);
        }
        lua_settop(L, -(1)-1);
        return sep;
    } else if (type != 0) {
        do { lua_pushfstring(L, "must pass a string to %s in mcp.router_new", arg); lua_error(L); } while (0);
    }
    return ((void *)0);
}
static void _mcplib_router_new_cmapcheck(lua_State *L) {
    ...
    while (lua_next(L, tidx) != 0) {
        ...
        int cmd = lua_tointegerx(L,(-2),((void *)0));
...
static void _mcplib_router_new_mode(lua_State *L, struct mcp_funcgen_router *fr) {
    const char *type = lua_tolstring(L, (-1), ((void *)0));
    size_t len = 0;
    ...
    if (strcmp(type, "prefix") == 0) {
        ...
        if (sep == ((void *)0) && usep == ((void *)0)) {
            fr->type = FGEN_ROUTER_ANCHORSM;
            fr->conf.anchorsm[0] = '/';
            fr->conf.anchorsm[1] = '/';
        } else if (sep == ((void *)0) || usep == ((void *)0)) {
...
int mcplib_router_new(lua_State *L) {
    ...
    if (lua_getfield(L, 1, "map") != 0) {
        route_count = _mcplib_router_new_mapcheck(L);
        has_map = true;
    }
    lua_settop(L, -(1)-1);
    if (lua_getfield(L, 1, "cmap") != 0) {
        if (!(lua_type(L, (-1)) == 5)) {
    ...
    strncpy(fgen->name, "mcp_router", 80);
    if (has_map) {
        lua_createtable(L, 0, route_count);
        lua_pushvalue(L, -1);
        router->map_ref = luaL_ref(L, (-1000000 - 1000));
    ...
    LIBEVENT_THREAD *t = (*(LIBEVENT_THREAD **)((void *)((char *)(L) - (sizeof(void *)))));
    fgen->thread = t;
    mcp_sharedvm_delta(t->proxy_ctx, 1, "mcp_router", 1);
    return 1;
}
