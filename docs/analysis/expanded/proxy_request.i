...
int mcp_request_render(mcp_request_t *rq, int idx, const char flag, const char *tok, size_t len) {
    ...
    void *vbuf = pr->vbuf;
    int vlen = pr->vlen;
    memset(pr, 0, sizeof(mcp_parser_t));
...
int mcp_request_append(mcp_request_t *rq, const char flag, const char *tok, size_t len) {
    ...
    char *p = (char *)pr->request + pr->reqlen - 2;
    ((void) (0));
    if (pr->reqlen + len + 2 >= 250 * 2) {
        return -1;
    }
...
int mcplib_request(lua_State *L) {
    ...
    if (type == 4) {
        val = luaL_optlstring(L, 2, ((void *)0), &vlen);
...
int mcplib_request_key(lua_State *L) {
    ...
    lua_pushlstring(L, (&(&rq->pr)->request[(&rq->pr)->tok.tokens[(&rq->pr)->keytoken]]), rq->pr.klen);
    return 1;
}
int mcplib_request_ltrimkey(lua_State *L) {
    mcp_request_t *rq = luaL_checkudata(L, -2, "mcp.request");
    int totrim = luaL_checkinteger(L, -1);
    char *key = (char *) (&(&rq->pr)->request[(&rq->pr)->tok.tokens[(&rq->pr)->keytoken]]);
...
int mcplib_request_token_int(lua_State *L) {
    ...
    int64_t token = 0;
    if (mcmc_token_get_64(rq->pr.request, &rq->pr.tok, x-1, &token) != 0) {
        lua_pushnil(L);
    } else {
...
int mcplib_request_flag_add(lua_State *L) {
    ...
    char tostring[30];
    if (mcmc_token_has_flag(rq->pr.request, &rq->pr.tok, flag) == 0) {
        lua_pushboolean(L, 0);
        return 1;
    }
    size_t tlen = 0;
...
int mcplib_request_flag_del(lua_State *L) {
    ...
    int x = mcmc_token_get_flag_idx(rq->pr.request, &rq->pr.tok, flag);
    if (x > 0) {
        if (mcp_request_render(rq, x, 0, ((void *)0), 0) != 0) {
            lua_pushboolean(L, 0);
            return 1;
        }
    } else {
...
int mcplib_request_match_res(lua_State *L) {
    ...
    if (mcmc_parse_buf(rs->buf, rs->blen, &reresp) != 0) {
        lua_pushboolean(L, 0);
    ...
    int matched = 0;
    int rs_len = 0;
    if (mcmc_token_has_flag(rs->buf, &rs->tok, 'k') == 0) {
        const char *rs_token = mcmc_token_get_flag(rs->buf, &rs->tok, 'k', &rs_len);
        matched = _mcplib_request_match_check(L, (&(&rq->pr)->request[(&rq->pr)->tok.tokens[(&rq->pr)->keytoken]]), rq->pr.klen,
                rs_token, rs_len);
        if (!matched) {
            lua_pushboolean(L, 0);
    ...
    if (opaque_token) {
        if (mcmc_token_has_flag(rs->buf, &rs->tok, 'O') == 0) {
