...
extern __inline __attribute__ ((__always_inline__)) __attribute__ ((__gnu_inline__)) __attribute__ ((__artificial__))
 int
poll (struct pollfd * __fds, nfds_t __nfds,
      int __timeout)
...
static unsigned int lru_type_map[4] = {0, 64, 128, 192};
typedef struct {
    uint64_t evicted;
...
static itemstats_t itemstats[256];
static unsigned int sizes[256];
static uint64_t sizes_bytes[256];
static unsigned int *stats_sizes_hist = ((void *)0);
static int stats_sizes_buckets = 0;
static uint64_t cas_id = 1;
static volatile int do_run_lru_maintainer_thread = 0;
static pthread_mutex_t lru_maintainer_lock = { { 0, 0, 0, 0, PTHREAD_MUTEX_TIMED_NP, 0, 0, { ((void *)0), ((void *)0) } } };
static pthread_mutex_t cas_id_lock = { { 0, 0, 0, 0, PTHREAD_MUTEX_TIMED_NP, 0, 0, { ((void *)0), ((void *)0) } } };
void item_stats_reset(void) {
    int i;
    for (i = 0; i < 256; i++) {
...
static lru_bump_buf *bump_buf_head = ((void *)0);
static lru_bump_buf *bump_buf_tail = ((void *)0);
static pthread_mutex_t bump_buf_lock = { { 0, 0, 0, 0, PTHREAD_MUTEX_TIMED_NP, 0, 0, { ((void *)0), ((void *)0) } } };
...
item *do_item_alloc_pull(const size_t ntotal, const unsigned int id) {
    ...
    for (i = 0; i < 10; i++) {
        if (!settings.lru_segmented) {
            lru_pull_tail(id, 128, 0, 0, 0, ((void *)0));
        }
        it = slabs_alloc(id, 0);
        if (it == ((void *)0)) {
            if (lru_pull_tail(id, 128, 0, 1, 0, ((void *)0)) <= 0) {
                if (settings.lru_segmented) {
                    lru_pull_tail(id, 0, 0, 0, 0, ((void *)0));
                } else {
                    break;
                }
            }
...
item *do_item_alloc(const char *key, const size_t nkey, const client_flags_t flags,
    ...
    it->slabs_clsid = id;
    while(0);
    it->it_flags |= settings.use_cas ? 2 : 0;
    it->it_flags |= nsuffix != 0 ? 256 : 0;
    it->nkey = nkey;
    it->nbytes = nbytes;
    memcpy((((char*)&((it)->data)) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), key, nkey);
    it->exptime = exptime;
    if (nsuffix > 0) {
        memcpy(((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), &flags, sizeof(flags));
    }
    if (it->it_flags & 32) {
        item_chunk *chunk = (item_chunk *) ((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0));
        chunk->next = 0;
        chunk->prev = 0;
        chunk->used = 0;
        chunk->size = 0;
...
void do_item_link_fixup(item *it) {
    item **head, **tail;
    int ntotal = (sizeof(struct _stritem) + (it)->nkey + 1 + (it)->nbytes + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0));
    uint32_t hv = hash((((char*)&((it)->data)) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), it->nkey);
    assoc_insert(it, hv);
...
static void do_item_link_q(item *it) {
    ...
    *head = it;
    ...
    sizes[it->slabs_clsid]++;
    if (it->it_flags & 128) {
        sizes_bytes[it->slabs_clsid] += ((sizeof(struct _stritem) + (it)->nkey + 1 + (it)->nbytes + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)) - it->nbytes) + sizeof(item_hdr);
    } else {
        sizes_bytes[it->slabs_clsid] += (sizeof(struct _stritem) + (it)->nkey + 1 + (it)->nbytes + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0));
    }
    return;
}
static void item_link_q(item *it) {
...
int do_item_link(item *it, const uint32_t hv, const uint64_t cas) {
    ...
    STATS_LOCK();
    stats_state.curr_bytes += (sizeof(struct _stritem) + (it)->nkey + 1 + (it)->nbytes + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0));
    stats_state.curr_items += 1;
    stats.total_items += 1;
    STATS_UNLOCK();
    { if ((it)->it_flags & 2) { (it)->data->cas = cas; } };
    assoc_insert(it, hv);
    item_link_q(it);
    ++(it->refcount);
    item_stats_sizes_add(it);
...
void do_item_unlink(item *it, const uint32_t hv) {
    ...
    if ((it->it_flags & 1) != 0) {
        ...
        assoc_delete((((char*)&((it)->data)) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), it->nkey, hv);
        item_unlink_q(it);
        do_item_remove(it);
    }
}
...
void do_item_update(item *it) {
    ...
    if (settings.lru_segmented) {
        ((void) (0));
        if ((it->it_flags & 1) != 0) {
            if (((it)->slabs_clsid & (3<<6)) == 128 && (it->it_flags & 16)) {
                it->time = current_time;
                item_unlink_q(it);
                it->slabs_clsid = ((it)->slabs_clsid & ~(3<<6));
                it->slabs_clsid |= 64;
                it->it_flags &= ~16;
                item_link_q_warm(it);
            } else {
    ...
    } else if (it->time < current_time - 60) {
        ((void) (0));
        if ((it->it_flags & 1) != 0) {
            it->time = current_time;
            item_unlink_q(it);
...
void item_flush_expired(void) {
    ...
    for (i = 0; i < 256; i++) {
        pthread_mutex_lock(&lru_locks[i]);
        for (iter = heads[i]; iter != ((void *)0); iter = next) {
            void *hold_lock = ((void *)0);
            next = iter->next;
            if (iter->time == 0 && iter->nkey == 0 && iter->it_flags == 1) {
                continue;
            }
            uint32_t hv = hash((((char*)&((iter)->data)) + (((iter)->it_flags & 2) ? sizeof(uint64_t) : 0)), iter->nkey);
            if ((hold_lock = item_trylock(hv)) == ((void *)0)) {
                continue;
            }
            if (iter->time >= settings.oldest_live) {
                if ((iter->it_flags & 4) == 0) {
                    do { storage_delete(ext_storage, iter); } while (0);
                    do_item_unlink_nolock(iter, hash((((char*)&((iter)->data)) + (((iter)->it_flags & 2) ? sizeof(uint64_t) : 0)), iter->nkey));
                }
...
char *item_cachedump(const unsigned int slabs_clsid, const unsigned int limit, unsigned int *bytes) {
    ...
    while (it != ((void *)0) && (limit == 0 || shown < limit)) {
        ((void) (0));
        if ((it->nbytes == 0 && it->nkey == 0) || (it->it_flags & 4096)) {
            it = it->next;
            continue;
        }
        strncpy(key_temp, (((char*)&((it)->data)) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), it->nkey);
...
void item_stats_totals(ADD_STAT add_stats, void *c) {
    ...
    append_stat("expired_unfetched", add_stats, c, "%llu", (unsigned long long)totals.expired_unfetched);;
    append_stat("evicted_unfetched", add_stats, c, "%llu", (unsigned long long)totals.evicted_unfetched);;
    if (settings.lru_maintainer_thread) {
        append_stat("evicted_active", add_stats, c, "%llu", (unsigned long long)totals.evicted_active);;
    }
    append_stat("evictions", add_stats, c, "%llu", (unsigned long long)totals.evicted);;
    append_stat("reclaimed", add_stats, c, "%llu", (unsigned long long)totals.reclaimed);;
    append_stat("crawler_reclaimed", add_stats, c, "%llu", (unsigned long long)totals.crawler_reclaimed);;
    append_stat("crawler_items_checked", add_stats, c, "%llu", (unsigned long long)totals.crawler_items_checked);;
    append_stat("lrutail_reflocked", add_stats, c, "%llu", (unsigned long long)totals.lrutail_reflocked);;
    if (settings.lru_maintainer_thread) {
        append_stat("moves_to_cold", add_stats, c, "%llu", (unsigned long long)totals.moves_to_cold);;
        append_stat("moves_to_warm", add_stats, c, "%llu", (unsigned long long)totals.moves_to_warm);;
        append_stat("moves_within_lru", add_stats, c, "%llu", (unsigned long long)totals.moves_within_lru);;
        append_stat("direct_reclaims", add_stats, c, "%llu", (unsigned long long)totals.direct_reclaims);;
        append_stat("lru_bumps_dropped", add_stats, c, "%llu", (unsigned long long)lru_total_bumps_dropped());;
    }
}
void item_stats(ADD_STAT add_stats, void *c) {
    ...
    for (n = 0; n < (63 + 1); n++) {
        ...
        for (x = 0; x < 4; x++) {
            ...
            if (lru_type_map[x] == 128 && tails[i] != ((void *)0)) {
                age = current_time - tails[i]->time;
            } else if (lru_type_map[x] == 0 && tails[i] != ((void *)0)) {
                age_hot = current_time - tails[i]->time;
            } else if (lru_type_map[x] == 64 && tails[i] != ((void *)0)) {
                age_warm = current_time - tails[i]->time;
            }
            if (lru_type_map[x] == 128)
        ...
        if (size == 0)
            continue;
        klen = snprintf(key_str, 128, fmt, n, "number"); vlen = snprintf(val_str, 128, "%u", size); add_stats(key_str, klen, val_str, vlen, c);;
        if (settings.lru_maintainer_thread) {
            klen = snprintf(key_str, 128, fmt, n, "number_hot"); vlen = snprintf(val_str, 128, "%u", lru_size_map[0]); add_stats(key_str, klen, val_str, vlen, c);;
            klen = snprintf(key_str, 128, fmt, n, "number_warm"); vlen = snprintf(val_str, 128, "%u", lru_size_map[1]); add_stats(key_str, klen, val_str, vlen, c);;
            klen = snprintf(key_str, 128, fmt, n, "number_cold"); vlen = snprintf(val_str, 128, "%u", lru_size_map[2]); add_stats(key_str, klen, val_str, vlen, c);;
            if (settings.temp_lru) {
                klen = snprintf(key_str, 128, fmt, n, "number_temp"); vlen = snprintf(val_str, 128, "%u", lru_size_map[3]); add_stats(key_str, klen, val_str, vlen, c);;
            }
            klen = snprintf(key_str, 128, fmt, n, "age_hot"); vlen = snprintf(val_str, 128, "%u", age_hot); add_stats(key_str, klen, val_str, vlen, c);;
            klen = snprintf(key_str, 128, fmt, n, "age_warm"); vlen = snprintf(val_str, 128, "%u", age_warm); add_stats(key_str, klen, val_str, vlen, c);;
        }
        klen = snprintf(key_str, 128, fmt, n, "age"); vlen = snprintf(val_str, 128, "%u", age); add_stats(key_str, klen, val_str, vlen, c);;
        klen = snprintf(key_str, 128, fmt, n, "mem_requested"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.mem_requested); add_stats(key_str, klen, val_str, vlen, c);;
        klen = snprintf(key_str, 128, fmt, n, "evicted"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.evicted); add_stats(key_str, klen, val_str, vlen, c);;
        klen = snprintf(key_str, 128, fmt, n, "evicted_nonzero"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.evicted_nonzero); add_stats(key_str, klen, val_str, vlen, c);;
        klen = snprintf(key_str, 128, fmt, n, "evicted_time"); vlen = snprintf(val_str, 128, "%u", totals.evicted_time); add_stats(key_str, klen, val_str, vlen, c);;
        klen = snprintf(key_str, 128, fmt, n, "outofmemory"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.outofmemory); add_stats(key_str, klen, val_str, vlen, c);;
        klen = snprintf(key_str, 128, fmt, n, "tailrepairs"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.tailrepairs); add_stats(key_str, klen, val_str, vlen, c);;
        klen = snprintf(key_str, 128, fmt, n, "reclaimed"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.reclaimed); add_stats(key_str, klen, val_str, vlen, c);;
        klen = snprintf(key_str, 128, fmt, n, "expired_unfetched"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.expired_unfetched); add_stats(key_str, klen, val_str, vlen, c);;
        klen = snprintf(key_str, 128, fmt, n, "evicted_unfetched"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.evicted_unfetched); add_stats(key_str, klen, val_str, vlen, c);;
        if (settings.lru_maintainer_thread) {
            klen = snprintf(key_str, 128, fmt, n, "evicted_active"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.evicted_active); add_stats(key_str, klen, val_str, vlen, c);;
        }
        klen = snprintf(key_str, 128, fmt, n, "crawler_reclaimed"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.crawler_reclaimed); add_stats(key_str, klen, val_str, vlen, c);;
        klen = snprintf(key_str, 128, fmt, n, "crawler_items_checked"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.crawler_items_checked); add_stats(key_str, klen, val_str, vlen, c);;
        klen = snprintf(key_str, 128, fmt, n, "lrutail_reflocked"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.lrutail_reflocked); add_stats(key_str, klen, val_str, vlen, c);;
        if (settings.lru_maintainer_thread) {
            klen = snprintf(key_str, 128, fmt, n, "moves_to_cold"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.moves_to_cold); add_stats(key_str, klen, val_str, vlen, c);;
            klen = snprintf(key_str, 128, fmt, n, "moves_to_warm"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.moves_to_warm); add_stats(key_str, klen, val_str, vlen, c);;
            klen = snprintf(key_str, 128, fmt, n, "moves_within_lru"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.moves_within_lru); add_stats(key_str, klen, val_str, vlen, c);;
            klen = snprintf(key_str, 128, fmt, n, "direct_reclaims"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.direct_reclaims); add_stats(key_str, klen, val_str, vlen, c);;
            klen = snprintf(key_str, 128, fmt, n, "hits_to_hot"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.hits_to_hot); add_stats(key_str, klen, val_str, vlen, c);;
            klen = snprintf(key_str, 128, fmt, n, "hits_to_warm"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.hits_to_warm); add_stats(key_str, klen, val_str, vlen, c);;
            klen = snprintf(key_str, 128, fmt, n, "hits_to_cold"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.hits_to_cold); add_stats(key_str, klen, val_str, vlen, c);;
            klen = snprintf(key_str, 128, fmt, n, "hits_to_temp"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long)totals.hits_to_temp); add_stats(key_str, klen, val_str, vlen, c);;
        }
    }
    ...
    if (stats_sizes_hist != ((void *)0)) {
        ...
        for (i = 0; i < stats_sizes_buckets; i++) {
            if (stats_sizes_hist[i] != 0) {
                ...
                append_stat(key, add_stats, c, "%u", stats_sizes_hist[i]);;
            }
        }
    } else {
        append_stat("sizes_status", add_stats, c, "disabled", "");;
    }
    add_stats(((void *)0), 0, ((void *)0), 0, c);
}
item *do_item_get(const char *key, const size_t nkey, const uint32_t hv, LIBEVENT_THREAD *t, const bool do_update) {
    ...
    if (it != ((void *)0)) {
        ...
        if (item_is_flushed(it)) {
            do_item_unlink(it, hv);
            do { storage_delete(t->storage, it); } while (0);
            do_item_remove(it);
            it = ((void *)0);
    ...
    do { logger *myl = t->l; if (t->l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<2)) logger_log(myl, LOGGER_ITEM_GET, ((void *)0), was_found, key, nkey, (it) ? it->nbytes : 0, (it) ? ((it)->slabs_clsid & ~(3<<6)) : 0, t->cur_sfd); } while (0);
    return it;
}
void do_item_bump(LIBEVENT_THREAD *t, item *it, const uint32_t hv) {
    if (settings.lru_segmented) {
...
item *do_item_touch(const char *key, size_t nkey, uint32_t exptime,
    ...
    item *it = do_item_get(key, nkey, hv, t, true);
    if (it != ((void *)0)) {
        it->exptime = exptime;
    }
    return it;
...
int lru_pull_tail(const int orig_id, const int cur_lru,
    ...
    for (; tries > 0 && search != ((void *)0); tries--, search=next_it) {
        ...
        uint32_t hv = hash((((char*)&((search)->data)) + (((search)->it_flags & 2) ? sizeof(uint64_t) : 0)), search->nkey);
        if ((hold_lock = item_trylock(hv)) == ((void *)0))
            continue;
        if (++(search->refcount) != 2) {
            ...
            if (settings.tail_repair_time &&
                ...
                search->refcount = 1;
                do { storage_delete(ext_storage, search); } while (0);
        ...
        switch (cur_lru) {
            ...
            case 128:
                ...
                if (flags & 1) {
                    ...
                    if ((search->it_flags & 16)) {
                        itemstats[id].evicted_active++;
                    }
                    do { logger *myl = ((void *)0); if (((void *)0) == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<6)) logger_log(myl, LOGGER_EVICTION, search); } while (0);
                    do { storage_delete(ext_storage, search); } while (0);
                    do_item_unlink_nolock(search, hv);
                    removed++;
    ...
    if (do_slab_reassign) {
        slabs_reassign(settings.slab_rebal, -1, orig_id, 1);
    }
    return removed;
}
...
void *item_lru_bump_buf_create(void) {
    ...
    b->buf = bipbuf_new(sizeof(lru_bump_entry) * 8192);
    if (b->buf == ((void *)0)) {
        free(b);
        return ((void *)0);
    }
...
static int lru_maintainer_juggle(const int slabs_clsid) {
    ...
    if (settings.temp_lru) {
        for (i = 0; i < 500; i++) {
            if (lru_pull_tail(slabs_clsid, 192, 0, 0, 0, ((void *)0)) <= 0) {
                break;
            } else {
                did_moves++;
            }
    ...
    if (settings.lru_segmented) {
        pthread_mutex_lock(&lru_locks[slabs_clsid|128]);
        if (tails[slabs_clsid|128]) {
            cold_age = current_time - tails[slabs_clsid|128]->time;
        }
        total_bytes += sizes_bytes[slabs_clsid|128];
        pthread_mutex_unlock(&lru_locks[slabs_clsid|128]);
        hot_age = cold_age * settings.hot_max_factor;
        warm_age = cold_age * settings.warm_max_factor;
        pthread_mutex_lock(&lru_locks[slabs_clsid|0]);
        total_bytes += sizes_bytes[slabs_clsid|0];
        pthread_mutex_unlock(&lru_locks[slabs_clsid|0]);
        pthread_mutex_lock(&lru_locks[slabs_clsid|64]);
        total_bytes += sizes_bytes[slabs_clsid|64];
        pthread_mutex_unlock(&lru_locks[slabs_clsid|64]);
    }
    for (i = 0; i < 500; i++) {
        int do_more = 0;
        if (lru_pull_tail(slabs_clsid, 0, total_bytes, 2, hot_age, ((void *)0)) ||
            lru_pull_tail(slabs_clsid, 64, total_bytes, 2, warm_age, ((void *)0))) {
            do_more++;
        }
        if (settings.lru_segmented) {
            do_more += lru_pull_tail(slabs_clsid, 128, total_bytes, 2, 0, ((void *)0));
        }
        if (do_more == 0)
            break;
        did_moves++;
...
static void lru_maintainer_crawler_check(struct crawler_expired_data *cdata, logger *l) {
    int i;
    static rel_time_t next_crawls[256];
    static rel_time_t next_crawl_wait[256];
    uint8_t todo[256];
    memset(todo, 0, sizeof(uint8_t) * 256);
    bool do_run = false;
    ...
    for (i = 1; i < 256; i++) {
        ...
        if (s->run_complete) {
            ...
            if (next_crawl_wait[i] > 60 * 60) {
                next_crawl_wait[i] = 60 * 60;
            }
            ...
            do { logger *myl = l; if (l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<1)) logger_log(myl, LOGGER_CRAWLER_STATUS, ((void *)0), (i & ~(3<<6)), lru_name, (unsigned long long)low_watermark, (unsigned long long)available_reclaims, (unsigned int)since_run, next_crawls[i] - current_time, s->end_time - s->start_time, s->seen, s->reclaimed); } while (0);
    ...
    if (do_run) {
        ...
        lru_crawler_start(todo, tocrawl_limit, CRAWLER_AUTOEXPIRE, cdata, ((void *)0), 0);
    }
}
static pthread_t lru_maintainer_tid;
static void *lru_maintainer_thread(void *arg) {
    int i;
    useconds_t to_sleep = 1000;
    useconds_t last_sleep = 1000;
    rel_time_t last_crawler_check = 0;
    useconds_t next_juggles[(63 + 1)] = {0};
    useconds_t backoff_juggles[(63 + 1)] = {0};
    ...
    pthread_mutex_init(&cdata->lock, ((void *)0));
    cdata->crawl_complete = true;
    ...
    while (do_run_lru_maintainer_thread) {
        ...
        for (i = 1; i < (63 + 1); i++) {
            ...
            if (did_moves == 0) {
                ...
                if (backoff_juggles[i] > (1000000-1))
                    backoff_juggles[i] = (1000000-1);
            } else if (backoff_juggles[i] > 0) {
                backoff_juggles[i] /= 2;
                if (backoff_juggles[i] < 1000) {
                    backoff_juggles[i] = 0;
                }
            }
