...
void proxy_gc_poke(LIBEVENT_THREAD *t) {
    ...
    int vm_kb = lua_gc(L, 3) + t->proxy_vm_extra_kb;
...
static void proxy_gc_timerpoke(int fd, short event, void *arg) {
    LIBEVENT_THREAD *t = arg;
    struct timeval next = { 4, 0 };
    event_add((t->proxy_gc_timer), (&next));
    int curmem = lua_gc(t->L, 3);
    ...
    if (t->proxy_vm_gcrunning == -1) {
        if (lua_gc(t->L, 5, 0)) {
            ...
            t->proxy_vm_gcpokemem = lua_gc(t->L, 3);
        }
    }
}
bool proxy_bufmem_checkadd(LIBEVENT_THREAD *t, int len) {
...
void proxy_stats(void *arg, ADD_STAT add_stats, void *c) {
    ...
    append_stat("proxy_config_reloads", add_stats, c, "%llu", (unsigned long long)ctx->global_stats.config_reloads);;
    append_stat("proxy_config_reload_fails", add_stats, c, "%llu", (unsigned long long)ctx->global_stats.config_reload_fails);;
    append_stat("proxy_config_cron_runs", add_stats, c, "%llu", (unsigned long long)ctx->global_stats.config_cron_runs);;
    append_stat("proxy_config_cron_fails", add_stats, c, "%llu", (unsigned long long)ctx->global_stats.config_cron_fails);;
    append_stat("proxy_backend_total", add_stats, c, "%llu", (unsigned long long)ctx->global_stats.backend_total);;
    append_stat("proxy_backend_marked_bad", add_stats, c, "%llu", (unsigned long long)ctx->global_stats.backend_marked_bad);;
    append_stat("proxy_backend_failed", add_stats, c, "%llu", (unsigned long long)ctx->global_stats.backend_failed);;
    append_stat("proxy_request_failed_depth", add_stats, c, "%llu", (unsigned long long)ctx->global_stats.request_failed_depth);;
...
void process_proxy_stats(void *arg, ADD_STAT add_stats, void *c) {
    ...
    pthread_mutex_unlock(&ctx->stats_lock);;
    if (buffer_memory_limit == (18446744073709551615UL)) {
        buffer_memory_limit = 0;
    } else {
        buffer_memory_limit *= settings.num_threads;
    }
    if (req_limit == (9223372036854775807L)) {
        req_limit = 0;
    } else {
        req_limit *= settings.num_threads;
    }
    append_stat("active_req_limit", add_stats, c, "%lld", (long long int)req_limit);;
    append_stat("buffer_memory_limit", add_stats, c, "%llu", (unsigned long long)buffer_memory_limit);;
    append_stat("buffer_memory_used", add_stats, c, "%llu", (unsigned long long)buffer_memory_used);;
    append_stat("vm_gc_runs", add_stats, c, "%llu", (unsigned long long)istats.vm_gc_runs);;
    append_stat("vm_memory_kb", add_stats, c, "%llu", (unsigned long long)istats.vm_memory_kb);;
    append_stat("cmd_mg", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_MG]);;
    append_stat("cmd_ms", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_MS]);;
    append_stat("cmd_md", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_MD]);;
    append_stat("cmd_mn", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_MN]);;
    append_stat("cmd_ma", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_MA]);;
    append_stat("cmd_me", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_ME]);;
    append_stat("cmd_get", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_GET]);;
    append_stat("cmd_gat", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_GAT]);;
    append_stat("cmd_set", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_SET]);;
    append_stat("cmd_add", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_ADD]);;
    append_stat("cmd_cas", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_CAS]);;
    append_stat("cmd_gets", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_GETS]);;
    append_stat("cmd_gats", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_GATS]);;
    append_stat("cmd_incr", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_INCR]);;
    append_stat("cmd_decr", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_DECR]);;
    append_stat("cmd_touch", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_TOUCH]);;
    append_stat("cmd_append", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_APPEND]);;
    append_stat("cmd_prepend", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_PREPEND]);;
    append_stat("cmd_delete", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_DELETE]);;
    append_stat("cmd_replace", add_stats, c, "%llu", (unsigned long long)istats.counters[CMD_REPLACE]);;
}
void process_proxy_funcstats(void *arg, ADD_STAT add_stats, void *c) {
    char key_str[128];
    if (!arg) {
    ...
    while (lua_next(L, 1) != 0) {
        int n = lua_tointegerx(L,(-1),((void *)0));
        lua_settop(L, -(1)-1);
        if (n != 0) {
            lua_pushvalue(L, -1);
            lua_rawget(L, 2);
            int slots = lua_tointegerx(L,(-1),((void *)0));
            lua_settop(L, -(1)-1);
            const char *name = lua_tolstring(L, (-1), ((void *)0));
            snprintf(key_str, 128 -1, "funcs_%s", name);
            append_stat(key_str, add_stats, c, "%d", n);;
            snprintf(key_str, 128 -1, "slots_%s", name);
            append_stat(key_str, add_stats, c, "%d", slots);;
        } else {
        }
    }
...
void process_proxy_bestats(void *arg, ADD_STAT add_stats, void *c) {
    ...
    while (lua_next(L, 3) != 0) {
        ...
        if (n != 0) {
            const char *name = lua_tolstring(L, (-1), ((void *)0));
            snprintf(key_str, 128 -1, "bad_%s", name);
            append_stat(key_str, add_stats, c, "%d", n);;
        } else {
            const char *name = lua_tolstring(L, (-1), ((void *)0));
            lua_pushnil(L);
...
void *proxy_init(bool use_uring, bool proxy_memprofile) {
    ...
    pthread_mutex_init(&ctx->config_lock, ((void *)0));
    pthread_cond_init(&ctx->config_cond, ((void *)0));
    pthread_mutex_init(&ctx->worker_lock, ((void *)0));
    pthread_cond_init(&ctx->worker_cond, ((void *)0));
    pthread_mutex_init(&ctx->manager_lock, ((void *)0));
    pthread_cond_init(&ctx->manager_cond, ((void *)0));
    pthread_mutex_init(&ctx->stats_lock, ((void *)0));
    ctx->active_req_limit = (9223372036854775807L);
    ctx->buffer_memory_limit = (18446744073709551615UL);
    ...
    ctx->tunables.gc_ratio = 2.0;
    ctx->tunables.backend_depth_limit = 0;
    ctx->tunables.max_ustats = 1024;
    ctx->tunables.use_iothread = false;
    ctx->tunables.use_tls = false;
    do { (((&ctx->manager_head))->stqh_first) = ((void *)0); (&ctx->manager_head)->stqh_last = &(((&ctx->manager_head))->stqh_first); } while (0);
    lua_State *L = ((void *)0);
    if (ctx->memprofile) {
    ...
    ctx->proxy_state = L;
    luaL_openlibs(L);
    proxy_register_libs(ctx, ((void *)0), L);
    lua_createtable(L, 0, 0);
    ctx->cron_ref = luaL_ref(L, (-1000000 - 1000));
    ctx->cron_next = 0x7fffffff;
    pthread_mutex_init(&ctx->sharedvm_lock, ((void *)0));
    ctx->proxy_sharedvm = luaL_newstate();
    luaL_openlibs(ctx->proxy_sharedvm);
    lua_createtable(ctx->proxy_sharedvm, 0, 0);
    lua_createtable(ctx->proxy_sharedvm, 0, 0);
    lua_createtable(ctx->proxy_sharedvm, 0, 0);
    proxy_event_thread_t *t = calloc(1, sizeof(proxy_event_thread_t));
    ctx->proxy_io_thread = t;
    proxy_init_event_thread(t, ctx, ((void *)0));
    pthread_create(&t->thread_id, ((void *)0), proxy_event_thread, t);
...
void proxy_thread_init(void *ctx, LIBEVENT_THREAD *thr) {
    ...
    thr->proxy_vm_last_kb = lua_gc(L, 3);
    ((void) (0));
    thr->proxy_gc_timer = event_new((thr->base), -1, 0, (proxy_gc_timerpoke), (thr));
...
void proxy_submit_cb(io_queue_t *q) {
    ...
    iop_head_t head;
    be_head_t w_head;
    do { (((&head))->stqh_first) = ((void *)0); (&head)->stqh_last = &(((&head))->stqh_first); } while (0);
    do { (((&w_head))->stqh_first) = ((void *)0); (&w_head)->stqh_last = &(((&w_head))->stqh_first); } while (0);
    while (!((&q->stack)->stqh_first == ((void *)0))) {
        mcp_backend_t *be;
        io_pending_proxy_t *p = (io_pending_proxy_t *)((&q->stack)->stqh_first);
        ...
        be = p->backend;
        if (be->use_io_thread) {
            do { ((((io_pending_t *)p))->iop_next.stqe_next) = ((void *)0); *(&head)->stqh_last = ((io_pending_t *)p); (&head)->stqh_last = &((((io_pending_t *)p))->iop_next.stqe_next); } while (0);
        } else {
            do { ((((io_pending_t *)p))->iop_next.stqe_next) = ((void *)0); *(&be->iop_head)->stqh_last = ((io_pending_t *)p); (&be->iop_head)->stqh_last = &((((io_pending_t *)p))->iop_next.stqe_next); } while (0);
            ((void) (0));
            be->depth++;
            if (!be->stacked) {
                be->stacked = true;
                do { (((be))->be_next.stqe_next) = ((void *)0); *(&w_head)->stqh_last = (be); (&w_head)->stqh_last = &(((be))->be_next.stqe_next); } while (0);
            }
        }
    }
    if (!((&head)->stqh_first == ((void *)0))) {
        ...
        if (((&e->iop_head_in)->stqh_first == ((void *)0))) {
            do_notify = true;
        }
        do { if (!(((&head))->stqh_first == ((void *)0))) { *(&e->iop_head_in)->stqh_last = (&head)->stqh_first; (&e->iop_head_in)->stqh_last = (&head)->stqh_last; do { ((((&head)))->stqh_first) = ((void *)0); ((&head))->stqh_last = &((((&head)))->stqh_first); } while (0); } } while (0);
...
int try_read_command_proxy(conn *c) {
    ...
    if (c->rbytes == 0)
        return 0;
    el = _Generic (0 ? (c->rcurr) : (void *) 1, const void *: (const void *) (memchr (c->rcurr, '\n', c->rbytes)), default: memchr (c->rcurr, '\n', c->rbytes));
    if (!el) {
        if (c->rbytes > 1024) {
            char *ptr = c->rcurr;
    ...
    proxy_process_command(c, c->rcurr, cont - c->rcurr, false);
...
void complete_nread_proxy(conn *c) {
    ...
    do { rctx->resp->suspended = true; rctx->c->resps_suspended++; } while (0);
    proxy_run_rcontext(rctx);
    lua_settop(L, 0);
    return;
}
...
static void _set_noreply_mode(mc_resp *resp, mcp_request_t *rq, mcp_resp_t *r) {
    if (rq->pr.cmd_type == CMD_TYPE_META
        && (((&rq->pr.tok)->metaflags & (uint64_t)1<<48) ? 0 : -2) == 0) {
        if (r->resp.code == 17) {
            resp->skip = true;
        } else if (r->cmd != CMD_MG && r->resp.code == 15) {
            resp->skip = true;
        }
    } else if (rq->pr.noreply && r->status == 0) {
        resp->skip = true;
    }
}
...
int proxy_run_rcontext(mcp_rcontext_t *rctx) {
    ...
    ((void) (0));
    int cores = lua_resume(Lc, ((void *)0), rctx->lua_narg, &nresults);
    rctx->lua_narg = 1;
    size_t rlen = 0;
    mc_resp *resp = rctx->resp;
    if (cores == 0) {
        if (!rctx->parent) {
            ...
            mcp_resp_t *r = ((void *)0);
            ;
            if (type == 7 && (r = luaL_testudata(Lc, 1, "mcp.response")) != ((void *)0)) {
                _set_noreply_mode(resp, rctx->request, r);
                if (r->status != 0 && r->resp.type != 108) {
                    proxy_out_errstring(resp, "SERVER_ERROR ", "backend failure");
                } else if (r->cresp) {
                ...
                size_t l = rlen > 1024 ? 1024 : rlen;
                memcpy(resp->wbuf, s, l);
                resp_add_iov(resp, resp->wbuf, l);
                lua_settop(Lc, -(1)-1);
            } else {
                proxy_out_errstring(resp, "SERVER_ERROR ", "bad response");
            }
            if (rctx->ascii_multiget && !resp->skip) {
                struct iovec *iov = &resp->iov[resp->iovcnt-1];
                if (iov->iov_len >= (sizeof("END\r\n")-1) &&
                        memcmp(((char *)iov->iov_base)+(iov->iov_len-(sizeof("END\r\n")-1)), "END\r\n", (sizeof("END\r\n")-1)) == 0) {
                    iov->iov_len -= (sizeof("END\r\n")-1);
                    resp->tosend -= (sizeof("END\r\n")-1);
                    if (resp->tosend == 0) {
                        resp->skip = true;
                    }
                }
            }
            do { resp->suspended = false; rctx->c->resps_suspended--; ((void) (0)); if (rctx->c->resps_suspended == 0) { conn_worker_readd(rctx->c); } } while (0);
            rctx->c = ((void *)0);
            rctx->pending_reqs--;
            mcp_funcgen_return_rctx(rctx);
        } else {
        ...
        int yield_type = lua_tointegerx(Lc,(-1),((void *)0));
        ;
        ((void) (0));
        lua_settop(Lc, -(1)-1);
        int res = 0;
        switch (yield_type) {
            case 1:
                if (rctx->parent) {
                    do { logger *myl = ((void *)0); if (((void *)0) == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<11)) logger_log(myl, LOGGER_PROXY_ERROR, ((void *)0), "cannot run mcp.internal from a sub request"); } while (0);
                    rctx->pending_reqs--;
                    return 2;
                } else {
                    ...
                    if (res == 0) {
                        ...
                        ((void) (0));
                        proxy_out_errstring(resp, "SERVER_ERROR ", "bad request");
                    }
                }
                break;
            case 2:
        ...
        ;
        do { logger *myl = ((void *)0); if (((void *)0) == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<11)) logger_log(myl, LOGGER_PROXY_ERROR, ((void *)0), lua_tolstring(Lc, (-1), ((void *)0))); } while (0);
        if (settings.verbose) {
            luaL_traceback(Lc, Lc, lua_tolstring(Lc, (-1), ((void *)0)), 1);
            fprintf(stderr, "proxy_lua_error: %s\n", lua_tolstring(Lc, (-1), ((void *)0)));
        }
        if (!rctx->parent) {
            proxy_out_errstring(resp, "SERVER_ERROR ", "lua failure");
            do { resp->suspended = false; rctx->c->resps_suspended--; ((void) (0)); if (rctx->c->resps_suspended == 0) { conn_worker_readd(rctx->c); } } while (0);
            rctx->c = ((void *)0);
            rctx->pending_reqs--;
            mcp_funcgen_return_rctx(rctx);
...
static void proxy_process_command(conn *c, char *command, size_t cmdlen, bool multiget) {
    ...
    if (ret != 0) {
        { pthread_mutex_lock(&c->thread->stats.mutex); c->thread->stats.proxy_conn_errors += 1; pthread_mutex_unlock(&c->thread->stats.mutex); };
        if (!resp_start(c)) {
            conn_set_state(c, conn_closing);
            return;
        }
        proxy_out_errstring(c->resp, "CLIENT_ERROR ", "parsing request");
        if (ret == -2) {
            conn_set_state(c, conn_closing);
        }
        return;
    ...
    if (!hook_ref.lua_ref) {
        c->proxy_rctx = ((void *)0);
        process_command_ascii(c, command, cmdlen);
        return;
    }
    if (!multiget && pr.cmd_type == CMD_TYPE_GET && pr.tok.ntokens > pr.keytoken+1) {
        ...
        size_t endlen = 0;
        if (cmd_prefix_len > 20) {
            if (!resp_start(c)) {
                conn_set_state(c, conn_closing);
                return;
            }
            proxy_out_errstring(c->resp, "CLIENT_ERROR ", "malformed request");
            return;
        }
        ((void) (0));
        ...
        while (klen != 0) {
            ...
            if (klen > 250) {
                ...
                proxy_out_errstring(c->resp, "CLIENT_ERROR ", "key too long");
            } else {
                ...
                *cur = '\0';
                ;
                proxy_process_command(c, temp, cur - temp, true);
            }
            curkey += klen;
            ...
            if (remain) {
                const char *s = _Generic (0 ? (curkey) : (void *) 1, const void *: (const void *) (memchr (curkey, ' ', remain)), default: memchr (curkey, ' ', remain));
                if (s != ((void *)0)) {
                    klen = s - curkey;
                } else {
                    klen = remain;
        ...
        memcpy(c->resp->wbuf, "END\r\n", (sizeof("END\r\n")-1));
        resp_add_iov(c->resp, c->resp->wbuf, (sizeof("END\r\n")-1));
        return;
    }
    if (cmdlen >= 250 * 2) {
        { pthread_mutex_lock(&c->thread->stats.mutex); c->thread->stats.proxy_conn_errors += 1; pthread_mutex_unlock(&c->thread->stats.mutex); };
        ...
        proxy_out_errstring(c->resp, "CLIENT_ERROR ", "request too long");
        conn_set_state(c, conn_closing);
        return;
    }
    if (!resp_start(c)) {
    ...
    if (active_reqs >= ctx->active_req_limit) {
        proxy_out_errstring(c->resp, "SERVER_ERROR ", "active request limit reached");
        if (pr.vlen != 0) {
            c->sbytes = pr.vlen;
    ...
    if (rctx == ((void *)0)) {
        proxy_out_errstring(c->resp, "SERVER_ERROR ", "lua start failure");
        if (pr.vlen != 0) {
            c->sbytes = pr.vlen;
            conn_set_state(c, conn_swallow);
        }
    ...
    rctx->pending_reqs++;
    rctx->resp = c->resp;
    lua_rawgeti(rctx->Lc, (-1000000 - 1000), rctx->function_ref);
    lua_rawgeti(rctx->Lc, (-1000000 - 1000), rctx->request_ref);
    if (pr.vlen != 0) {
        c->item = ((void *)0);
        ...
        if (c->item == ((void *)0)) {
            ...
            mcp_funcgen_return_rctx(rctx);
            lua_settop(L, 0);
            proxy_out_errstring(c->resp, "SERVER_ERROR ", "out of memory");
            c->sbytes = pr.vlen;
            conn_set_state(c, conn_swallow);
            return;
        }
...
void mcp_set_resobj(mcp_resp_t *r, mcp_request_t *rq, mcp_backend_t *be, LIBEVENT_THREAD *t) {
    ...
    gettimeofday(&r->start, ((void *)0));
    if (rq->pr.cmd_type == CMD_TYPE_META) {
        if ((((&rq->pr.tok)->metaflags & (uint64_t)1<<48) ? 0 : -2) == 0) {
...
io_pending_proxy_t *mcp_queue_rctx_io(mcp_rcontext_t *rctx, mcp_request_t *rq, mcp_backend_t *be, mcp_resp_t *r) {
    conn *c = rctx->c;
    io_queue_t *q = thread_io_queue_get(rctx->fgen->thread, 2);
    io_pending_proxy_t *p = do_cache_alloc(c->thread->io_cache);
    if (p == ((void *)0)) {
        { pthread_mutex_lock(&c->thread->stats.mutex); c->thread->stats.proxy_conn_oom += 1; pthread_mutex_unlock(&c->thread->stats.mutex); };
...
static void mcp_gobj_enqueue(proxy_ctx_t *ctx, struct mcp_globalobj_s *g) {
    ...
    do { (((g))->next.stqe_next) = ((void *)0); *(&ctx->manager_head)->stqh_last = (g); (&ctx->manager_head)->stqh_last = &(((g))->next.stqe_next); } while (0);
...
void mcp_gobj_ref(lua_State *L, struct mcp_globalobj_s *g) {
    ...
    if (g->self_ref == 0) {
        g->self_ref = -luaL_ref(L, (-1000000 - 1000));
        g->refcount++;
        proxy_ctx_t *ctx = (*(proxy_ctx_t **)((void *)((char *)(L) - (sizeof(void *)))));
    ...
    for (; i < top + 1; i++) {
        ...
        if (lua_getmetatable(L, i) != 0) {
            ...
            if (lua_rawget(L, -2) != 0) {
                ...
                fprintf(stderr, "--|%d| [%s] (%s) [ptr: %p]\n", i, lua_typename(L, type), lua_tolstring(L, (-1), ((void *)0)), udata);
                lua_settop(L, -(2)-1);
                continue;
            }
            lua_settop(L, -(2)-1);
        ...
        if (type == 4) {
            fprintf(stderr, "--|%d| [%s] | %s\n", i, lua_typename(L, type), lua_tolstring(L, (i), ((void *)0)));
        } else {
            if (type == 7) {
                udata = lua_touserdata(L, i);
            }
...
__attribute__((unused)) void dump_registry(lua_State *L, const char *msg) {
    int ref_size = lua_rawlen(L, (-1000000 - 1000));
    fprintf(stderr, "--LUA REGISTRY TABLE [%d] | %s\n", ref_size, msg);
    int ridx = lua_absindex(L, (-1000000 - 1000));
    int udata = 0;
    int number = 0;
    int string = 0;
    int function = 0;
...
__attribute__((unused)) void dump_funcgen(lua_State *L, const char *name, const char *msg) {
    ...
    while (lua_next(L, ridx) != 0) {
        ...
        if (type == 7) {
            ...
            if (f != ((void *)0) && strcmp(name, f->name) == 0) {
        ...
        if (be != ((void *)0)) {
            ...
            for (int i = 0; i < be->conncount; i++) {
                ...
                fprintf(stderr, "    --bec[%d] bad: [%d] failcnt: [%d] depth: [%d] pendread: [%d] state: [%d] can_write[%d] write_event[%d]\n",
                        i, bec->bad, bec->failed_count, bec->depth, bec->pending_read, bec->state, bec->can_write, event_pending(&bec->timeout_event, 0x04, ((void *)0)));
            }
        }
    }
    fprintf(stderr, "=======\n");
