...
static void _mcplib_append_section_stats(const char *key, const uint16_t klen,
    ...
    if (sscanf(key, "items:%ld:%s", &section, stat) == 2
        ...
        if (lua_rawgeti(L, tidx, section) == 0) {
            lua_settop(L, -(1)-1);
            lua_createtable(L, 0, 0);
            lua_pushvalue(L, -1);
            lua_rawseti(L, tidx, section);
        }
...
static int mcplib_server_stats(lua_State *L) {
    ...
    if (argc == 0 || (lua_type(L, (1)) == 0)) {
        server_stats(&_mcplib_append_stats, &c);
        get_stats(((void *)0), 0, &_mcplib_append_stats, &c);
    } else {
        const char *cmd = (luaL_checklstring(L, (1), ((void *)0)));
...
static int mcplib_register_cron(lua_State *L) {
    ...
    lua_settop(L, -(1)-1);
    if (lua_getfield(L, 2, "every") != 0) {
        luaL_checktype(L, -1, 3);
        int every = lua_tointegerx(L,(-1),((void *)0));
        if (every < 1) {
    ...
    lua_rawgeti(L, (-1000000 - 1000), ctx->cron_ref);
...
static int mcplib_backend_wrap_gc(lua_State *L) {
    ...
    if (bew->be != ((void *)0)) {
        ...
        do { (((be))->beconn_next.stqe_next) = ((void *)0); *(&e->beconn_head_in)->stqh_last = (be); (&e->beconn_head_in)->stqh_last = &(((be))->beconn_next.stqe_next); } while (0);
    ...
    { pthread_mutex_lock(&ctx->stats_lock); ctx->global_stats.backend_total -= 1; pthread_mutex_unlock(&ctx->stats_lock); };
    return 0;
}
...
static int _mcplib_backend_log(lua_State *L, mcp_backend_label_t *be) {
    be->use_logging = true;
    if (lua_getfield(L, -1, "deadline") != 0) {
...
static int mcplib_backend(lua_State *L) {
    ...
    be->conncount = 1;
    (lua_getfield(L, (-1000000 - 1000), ("mcp.backend")));
    lua_setmetatable(L, -2);
    if ((lua_type(L, (1)) == 5)) {
        if (lua_getfield(L, 1, "label") != 0) {
        ...
        if (lua_getfield(L, 1, "tcpkeepalive") != 0) {
            be->tunables.tcp_keepalive = lua_toboolean(L, -1);
        }
        lua_settop(L, -(1)-1);
        if (lua_getfield(L, 1, "tls") != 0) {
            be->tunables.use_tls = lua_toboolean(L, -1);
        }
        lua_settop(L, -(1)-1);
        if (lua_getfield(L, 1, "failurelimit") != 0) {
        ...
        lua_settop(L, -(1)-1);
        if (lua_getfield(L, 1, "depthlimit") != 0) {
            int limit = luaL_checkinteger(L, -1);
            if (limit < 0) {
        ...
        lua_settop(L, -(1)-1);
        if (lua_getfield(L, 1, "connecttimeout") != 0) {
            lua_Number secondsf = luaL_checknumber(L, -1);
            lua_Integer secondsi = (lua_Integer) secondsf;
            lua_Number subseconds = secondsf - secondsi;
            be->tunables.connect.tv_sec = secondsi;
            be->tunables.connect.tv_usec = ((subseconds) * 1E6 + 0.5);
        }
        lua_settop(L, -(1)-1);
        if (lua_getfield(L, 1, "retrytimeout") != 0) {
            be->tunables.retry.tv_sec =
                _mcplib_backend_get_waittime(luaL_checknumber(L, -1));
        }
        lua_settop(L, -(1)-1);
        if (lua_getfield(L, 1, "retrywaittime") != 0) {
            be->tunables.retry.tv_sec =
                _mcplib_backend_get_waittime(luaL_checknumber(L, -1));
        }
        lua_settop(L, -(1)-1);
        if (lua_getfield(L, 1, "retrytimeout") != 0) {
            ...
            be->tunables.retry.tv_usec = ((subseconds) * 1E6 + 0.5);
        }
        lua_settop(L, -(1)-1);
        if (lua_getfield(L, 1, "readtimeout") != 0) {
            lua_Number secondsf = luaL_checknumber(L, -1);
            lua_Integer secondsi = (lua_Integer) secondsf;
            lua_Number subseconds = secondsf - secondsi;
            be->tunables.read.tv_sec = secondsi;
            be->tunables.read.tv_usec = ((subseconds) * 1E6 + 0.5);
        }
        lua_settop(L, -(1)-1);
        if (lua_getfield(L, 1, "down") != 0) {
            int down = lua_toboolean(L, -1);
            be->tunables.down = down;
        }
        lua_settop(L, -(1)-1);
        if (lua_getfield(L, 1, "flaptime") != 0) {
            lua_Number secondsf = luaL_checknumber(L, -1);
            lua_Integer secondsi = (lua_Integer) secondsf;
            lua_Number subseconds = secondsf - secondsi;
            be->tunables.flap.tv_sec = secondsi;
            be->tunables.flap.tv_usec = ((subseconds) * 1E6 + 0.5);
        }
        lua_settop(L, -(1)-1);
        if (lua_getfield(L, 1, "flapbackofframp") != 0) {
        ...
        lua_settop(L, -(1)-1);
        if (lua_getfield(L, 1, "flapbackoffmax") != 0) {
            luaL_checknumber(L, -1);
            uint32_t max = lua_tointegerx(L,(-1),((void *)0));
            be->tunables.flap_backoff_max = max;
        }
        lua_settop(L, -(1)-1);
        if (lua_getfield(L, 1, "connections") != 0) {
...
static mcp_backend_wrap_t *_mcplib_backend_checkcache(lua_State *L, mcp_backend_label_t *bel) {
    int ret = lua_gettable(L, ((-1000000 - 1000) - (1)));
    if (ret != 0) {
        mcp_backend_wrap_t *be_orig = luaL_checkudata(L, -1, "mcp.backendwrap");
        if (strncmp(be_orig->be->name, bel->name, 255) == 0
                && strncmp(be_orig->be->port, bel->port, 6) == 0
...
static mcp_backend_wrap_t *_mcplib_make_backendconn(lua_State *L, mcp_backend_label_t *bel,
    ...
    (lua_getfield(L, (-1000000 - 1000), ("mcp.backendwrap")));
    lua_setmetatable(L, -2);
    ...
    bew->be = be;
    strncpy(be->name, bel->name, 255 +1);
    strncpy(be->port, bel->port, 6 +1);
    strncpy(be->label, bel->label, 512 +1);
    ...
    do { (((&be->iop_head))->stqh_first) = ((void *)0); (&be->iop_head)->stqh_last = &(((&be->iop_head))->stqh_first); } while (0);
    for (int x = 0; x < bel->conncount; x++) {
        struct mcp_backendconn_s *bec = &be->be[x];
        bec->be_parent = be;
        memcpy(&bec->tunables, &bel->tunables, sizeof(bel->tunables));
        do { (((&bec->iop_write))->stqh_first) = ((void *)0); (&bec->iop_write)->stqh_last = &(((&bec->iop_write))->stqh_first); } while (0);
        do { (((&bec->iop_read))->stqh_first) = ((void *)0); (&bec->iop_read)->stqh_last = &(((&bec->iop_read))->stqh_first); } while (0);
        bec->state = mcp_backend_read;
        bec->rbuf = malloc(16384);
        if (bec->rbuf == ((void *)0)) {
            proxy_lua_error(L, "out of memory allocating backend");
            return ((void *)0);
        }
        bec->client = malloc(mcmc_size(0));
        if (bec->client == ((void *)0)) {
            proxy_lua_error(L, "out of memory allocating backend");
            return ((void *)0);
        }
        int flags = 1;
...
static int mcplib_pool_gc(lua_State *L) {
    ...
    luaL_unref(L, (-1000000 - 1000), p->phc_ref);
    for (int x = 0; x < p->pool_be_total; x++) {
        if (p->pool[x].ref) {
            luaL_unref(L, (-1000000 - 1000), p->pool[x].ref);
        }
    }
    return 0;
}
static const char *mcp_key_hash_filter_stop(const char *conf, const char *key, size_t klen, size_t *newlen) {
    ...
    *newlen = klen;
    ...
    temp[klen] = '\0';
    char *found = _Generic (0 ? (temp) : (void *) 1, const void *: (const char *) (strstr (temp, conf)), default: strstr (temp, conf));
    if (found) {
        *newlen = found - temp;
    }
    return key;
...
static const char *mcp_key_hash_filter_tag(const char *conf, const char *key, size_t klen, size_t *newlen) {
    *newlen = klen;
    const char *t1 = _Generic (0 ? (key) : (void *) 1, const void *: (const void *) (memchr (key, conf[0], klen)), default: memchr (key, conf[0], klen));
    if (t1) {
        size_t remain = klen - (t1 - key);
        if (remain > 1) {
            const char *t2 = _Generic (0 ? (t1+1) : (void *) 1, const void *: (const void *) (memchr (t1+1, conf[1], remain-1)), default: memchr (t1+1, conf[1], remain-1));
            if (t2) {
                *newlen = t2 - t1 - 1;
                return t1+1;
            }
...
static void _mcplib_pool_dist(lua_State *L, mcp_pool_t *p) {
    ...
    lua_pushvalue(L, 2);
    int res = lua_pcallk(L, (2), (2), (0), 0, ((void *)0));
    if (res != 0) {
        lua_error(L);
        return;
    }
    struct proxy_hash_caller *phc;
    luaL_checktype(L, -1, 2);
    luaL_checktype(L, -2, 7);
    phc = lua_touserdata(L, -1);
    memcpy(&p->phc, phc, sizeof(*phc));
    lua_settop(L, -(1)-1);
    p->phc_ref = luaL_ref(L, (-1000000 - 1000));
}
...
static int mcplib_pool(lua_State *L) {
    ...
    luaL_checktype(L, 1, 5);
    int n = luaL_len(L, 1);
    int workers = settings.num_threads;
    ...
    p->key_hasher = XXH_INLINE_XXH3_64bits_withSeed;
    pthread_mutex_init(&p->g.lock, ((void *)0));
    p->ctx = (*(proxy_ctx_t **)((void *)((char *)(L) - (sizeof(void *)))));
    luaL_setmetatable(L, "mcp.pool");
    int type = lua_type(L, 2);
    if (argc == 1 || type == 0) {
        _mcplib_pool_make_be(L, p);
        lua_getglobal(L, "mcp");
        if (lua_getfield(L, -1, "dist_jump_hash") != 0) {
    ...
    luaL_checktype(L, 2, 5);
    if (lua_getfield(L, 2, "iothread") != 0) {
        luaL_checktype(L, -1, 1);
    ...
    if (lua_getfield(L, 2, "beprefix") != 0) {
    ...
    if (lua_getfield(L, 2, "filter") != 0) {
        luaL_checktype(L, -1, 4);
        const char *f_type = lua_tolstring(L, (-1), ((void *)0));
        if (strcmp(f_type, "stop") == 0) {
            ...
            do { lua_pushfstring(L, "unknown hash filter specified: %s\n", f_type); lua_error(L); } while (0);
        }
        lua_settop(L, -(1)-1);
        if (lua_getfield(L, 2, "filter_conf") == 4) {
            size_t len = 0;
            const char *conf = lua_tolstring(L, -1, &len);
            if (len < 2 || len > 5) {
                do { lua_pushfstring(L, "hash filter conf must be between 2 and %d characters", 5); lua_error(L); } while (0);
            }
    ...
    if (lua_getfield(L, 2, "seed") != 0) {
        luaL_checktype(L, -1, 4);
        size_t seedlen;
        const char *seedstr = lua_tolstring(L, -1, &seedlen);
        p->hash_seed = XXH_INLINE_XXH3_64bits(seedstr, seedlen);
        lua_settop(L, -(1)-1);
    } else {
        lua_settop(L, -(1)-1);
    }
    if (p->phc.selector_func == ((void *)0)) {
...
static int mcplib_pool_proxy_gc(lua_State *L) {
    ...
    if (p->g.refcount == 0) {
        ...
        do { (((&p->g))->next.stqe_next) = ((void *)0); *(&ctx->manager_head)->stqh_last = (&p->g); (&ctx->manager_head)->stqh_last = &(((&p->g))->next.stqe_next); } while (0);
...
static int mcplib_backend_connect_timeout(lua_State *L) {
    ...
    ctx->tunables.connect.tv_usec = ((subseconds) * 1E6 + 0.5);
    pthread_mutex_unlock(&ctx->stats_lock);;
    return 0;
}
...
static int mcplib_backend_read_timeout(lua_State *L) {
    ...
    ctx->tunables.read.tv_usec = ((subseconds) * 1E6 + 0.5);
    pthread_mutex_unlock(&ctx->stats_lock);;
    return 0;
}
static int mcplib_backend_flap_time(lua_State *L) {
    ...
    ctx->tunables.flap.tv_usec = ((subseconds) * 1E6 + 0.5);
    pthread_mutex_unlock(&ctx->stats_lock);;
    return 0;
}
...
static int mcplib_attach(lua_State *L) {
    LIBEVENT_THREAD *t = (*(LIBEVENT_THREAD **)((void *)((char *)(L) - (sizeof(void *)))));
    ...
    mcp_funcgen_t *fgen = ((void *)0);
    if ((lua_type(L, (2)) == 6)) {
        lua_pushvalue(L, 2);
        mcplib_funcgenbare_new(L);
        fgen = luaL_checkudata(L, -1, "mcp.funcgen");
        (lua_copy(L, -1, (2)), lua_settop(L, -(1)-1));
        lua_settop(L, -(1)-1);
    } else if ((fgen = luaL_testudata(L, 2, "mcp.funcgen")) != ((void *)0)) {
    } else {
    ...
    }
    {
        ...
        for (int x = loop_start; x < loop_end; x++) {
            ...
            if (href->lua_ref) {
                luaL_unref(L, (-1000000 - 1000), href->lua_ref);
                mcp_funcgen_dereference(L, href->ctx);
            }
            lua_pushvalue(L, -1);
            mcp_funcgen_reference(L);
            href->lua_ref = luaL_ref(L, (-1000000 - 1000));
            href->ctx = fgen;
            ((void) (0));
        }
    }
...
static int mcplib_ct_log(lua_State *L) {
    ...
    do { logger *myl = ((void *)0); if (((void *)0) == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<12)) logger_log(myl, LOGGER_PROXY_USER, ((void *)0), msg); } while (0);
    return 0;
}
static int mcplib_log(lua_State *L) {
    LIBEVENT_THREAD *t = (*(LIBEVENT_THREAD **)((void *)((char *)(L) - (sizeof(void *)))));
    const char *msg = (luaL_checklstring(L, (-1), ((void *)0)));
    do { logger *myl = t->l; if (t->l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<12)) logger_log(myl, LOGGER_PROXY_USER, ((void *)0), msg); } while (0);
    return 0;
}
static int mcplib_log_req(lua_State *L) {
    ...
    size_t dlen = 0;
    const char *detail = luaL_optlstring(L, 3, ((void *)0), &dlen);
    ...
    logger_log(l, LOGGER_PROXY_REQ, ((void *)0), rq->pr.request, rq->pr.reqlen, elapsed, rtype, rcode, rstatus, flag, cfd, detail, dlen, rname, rport);
    return 0;
}
...
void mcplib_rqu_log(mcp_request_t *rq, mcp_resp_t *rs, int flag, int cfd) {
    ...
    if (pl->rate == 1) {
        do_log = true;
    } else if (pl->all_errors && rstatus != 0) {
        do_log = true;
...
static int mcplib_log_reqsample(lua_State *L) {
    ...
    size_t dlen = 0;
    const char *detail = luaL_optlstring(L, 6, ((void *)0), &dlen);
...
static void proxy_register_defines(lua_State *L) {
    lua_pushinteger(L, 8); lua_setfield(L, -2, "MCMC_CODE_STORED");;
    lua_pushinteger(L, 9); lua_setfield(L, -2, "MCMC_CODE_EXISTS");;
    lua_pushinteger(L, 10); lua_setfield(L, -2, "MCMC_CODE_DELETED");;
    lua_pushinteger(L, 11); lua_setfield(L, -2, "MCMC_CODE_TOUCHED");;
    lua_pushinteger(L, 12); lua_setfield(L, -2, "MCMC_CODE_VERSION");;
    lua_pushinteger(L, 13); lua_setfield(L, -2, "MCMC_CODE_NOT_FOUND");;
    lua_pushinteger(L, 14); lua_setfield(L, -2, "MCMC_CODE_NOT_STORED");;
    lua_pushinteger(L, 15); lua_setfield(L, -2, "MCMC_CODE_OK");;
    lua_pushinteger(L, 16); lua_setfield(L, -2, "MCMC_CODE_NOP");;
    lua_pushinteger(L, 17); lua_setfield(L, -2, "MCMC_CODE_END");;
    lua_pushinteger(L, 21); lua_setfield(L, -2, "MCMC_CODE_ERROR");;
    lua_pushinteger(L, 22); lua_setfield(L, -2, "MCMC_CODE_CLIENT_ERROR");;
    lua_pushinteger(L, 23); lua_setfield(L, -2, "MCMC_CODE_SERVER_ERROR");;
    lua_pushinteger(L, -1); lua_setfield(L, -2, "MCMC_ERR");;
    lua_pushinteger(L, 0); lua_setfield(L, -2, "P_OK");;
    lua_pushinteger(L, CMD_ANY); lua_setfield(L, -2, "CMD_ANY");;
    lua_pushinteger(L, CMD_ANY_STORAGE); lua_setfield(L, -2, "CMD_ANY_STORAGE");;
    lua_pushinteger(L, QWAIT_ANY); lua_setfield(L, -2, "WAIT_ANY");;
    lua_pushinteger(L, QWAIT_OK); lua_setfield(L, -2, "WAIT_OK");;
    lua_pushinteger(L, QWAIT_GOOD); lua_setfield(L, -2, "WAIT_GOOD");;
    lua_pushinteger(L, QWAIT_FASTGOOD); lua_setfield(L, -2, "WAIT_FASTGOOD");;
    lua_pushinteger(L, (1<<3)); lua_setfield(L, -2, "RES_GOOD");;
    lua_pushinteger(L, (1<<4)); lua_setfield(L, -2, "RES_OK");;
    lua_pushinteger(L, (1<<5)); lua_setfield(L, -2, "RES_ANY");;
    lua_pushinteger(L, CMD_MG); lua_setfield(L, -2, "CMD_MG"); lua_pushinteger(L, CMD_MS); lua_setfield(L, -2, "CMD_MS"); lua_pushinteger(L, CMD_MD); lua_setfield(L, -2, "CMD_MD"); lua_pushinteger(L, CMD_MN); lua_setfield(L, -2, "CMD_MN"); lua_pushinteger(L, CMD_MA); lua_setfield(L, -2, "CMD_MA"); lua_pushinteger(L, CMD_ME); lua_setfield(L, -2, "CMD_ME"); lua_pushinteger(L, CMD_GET); lua_setfield(L, -2, "CMD_GET"); lua_pushinteger(L, CMD_GAT); lua_setfield(L, -2, "CMD_GAT"); lua_pushinteger(L, CMD_SET); lua_setfield(L, -2, "CMD_SET"); lua_pushinteger(L, CMD_ADD); lua_setfield(L, -2, "CMD_ADD"); lua_pushinteger(L, CMD_CAS); lua_setfield(L, -2, "CMD_CAS"); lua_pushinteger(L, CMD_GETS); lua_setfield(L, -2, "CMD_GETS"); lua_pushinteger(L, CMD_GATS); lua_setfield(L, -2, "CMD_GATS"); lua_pushinteger(L, CMD_INCR); lua_setfield(L, -2, "CMD_INCR"); lua_pushinteger(L, CMD_DECR); lua_setfield(L, -2, "CMD_DECR"); lua_pushinteger(L, CMD_TOUCH); lua_setfield(L, -2, "CMD_TOUCH"); lua_pushinteger(L, CMD_APPEND); lua_setfield(L, -2, "CMD_APPEND"); lua_pushinteger(L, CMD_DELETE); lua_setfield(L, -2, "CMD_DELETE"); lua_pushinteger(L, CMD_REPLACE); lua_setfield(L, -2, "CMD_REPLACE"); lua_pushinteger(L, CMD_PREPEND); lua_setfield(L, -2, "CMD_PREPEND"); lua_pushinteger(L, CMD_END_STORAGE); lua_setfield(L, -2, "CMD_END_STORAGE"); lua_pushinteger(L, CMD_QUIT); lua_setfield(L, -2, "CMD_QUIT"); lua_pushinteger(L, CMD_STATS); lua_setfield(L, -2, "CMD_STATS"); lua_pushinteger(L, CMD_SLABS); lua_setfield(L, -2, "CMD_SLABS"); lua_pushinteger(L, CMD_WATCH); lua_setfield(L, -2, "CMD_WATCH"); lua_pushinteger(L, CMD_LRU); lua_setfield(L, -2, "CMD_LRU"); lua_pushinteger(L, CMD_VERSION); lua_setfield(L, -2, "CMD_VERSION"); lua_pushinteger(L, CMD_SHUTDOWN); lua_setfield(L, -2, "CMD_SHUTDOWN"); lua_pushinteger(L, CMD_EXTSTORE); lua_setfield(L, -2, "CMD_EXTSTORE"); lua_pushinteger(L, CMD_FLUSH_ALL); lua_setfield(L, -2, "CMD_FLUSH_ALL"); lua_pushinteger(L, CMD_VERBOSITY); lua_setfield(L, -2, "CMD_VERBOSITY"); lua_pushinteger(L, CMD_LRU_CRAWLER); lua_setfield(L, -2, "CMD_LRU_CRAWLER"); lua_pushinteger(L, CMD_REFRESH_CERTS); lua_setfield(L, -2, "CMD_REFRESH_CERTS"); lua_pushinteger(L, CMD_CACHE_MEMLIMIT); lua_setfield(L, -2, "CMD_CACHE_MEMLIMIT");
    lua_pushboolean(L, 1);
    lua_setfield(L, -2, "WAIT_RESUME");
}
static void proxy_register_startarg(lua_State *L) {
    int idx = lua_absindex(L, -1);
    if (settings.proxy_startarg == ((void *)0)) {
        lua_pushboolean(L, 0);
        lua_setfield(L, idx, "start_arg");
        return;
    }
    char *sarg = strdup(settings.proxy_startarg);
    if (_Generic (0 ? (sarg) : (void *) 1, const void *: (const char *) (strchr (sarg, ':')), default: strchr (sarg, ':')) == ((void *)0)) {
        ...
        for (char *p = strtok_r(sarg, ":", &b);
            ...
            char *value = strtok_r(((void *)0), "_", &e);
            if (value == ((void *)0)) {
                lua_pushboolean(L, 1);
            } else {
...
int proxy_register_libs(void *ctx, LIBEVENT_THREAD *t, void *state) {
    ...
    void **extra = ((void *)((char *)(L) - (sizeof(void *))));
    if (t != ((void *)0)) {
        *extra = t;
        ...
        lua_pushvalue(L, -1);
        lua_setfield(L, -2, "__index");
        luaL_setfuncs(L, mcplib_response_m, 0);
        lua_settop(L, -(1)-1);
        luaL_newmetatable(L, "mcp.pool_proxy");
        lua_pushvalue(L, -1);
        lua_setfield(L, -2, "__index");
        luaL_setfuncs(L, mcplib_pool_proxy_m, 0);
        lua_settop(L, -(1)-1);
        luaL_newmetatable(L, "mcp.ratelim_tbf");
        lua_pushvalue(L, -1);
        lua_setfield(L, -2, "__index");
        luaL_setfuncs(L, mcplib_ratelim_tbf_m, 0);
        lua_settop(L, -(1)-1);
        luaL_newmetatable(L, "mcp.ratelim_proxy_tbf");
        lua_pushvalue(L, -1);
        lua_setfield(L, -2, "__index");
        luaL_setfuncs(L, mcplib_ratelim_proxy_tbf_m, 0);
        lua_settop(L, -(1)-1);
        luaL_newmetatable(L, "mcp.inspector");
        lua_pushvalue(L, -1);
        lua_setfield(L, -2, "__index");
        luaL_setfuncs(L, mcplib_inspector_m, 0);
        lua_settop(L, -(1)-1);
        luaL_newmetatable(L, "mcp.mutator");
        lua_pushvalue(L, -1);
        lua_setfield(L, -2, "__index");
        luaL_setfuncs(L, mcplib_mutator_m, 0);
        lua_settop(L, -(1)-1);
        luaL_newmetatable(L, "mcp.rcontext");
        lua_pushvalue(L, -1);
        lua_setfield(L, -2, "__index");
        luaL_setfuncs(L, mcplib_rcontext_m, 0);
        lua_settop(L, -(1)-1);
        luaL_newmetatable(L, "mcp.funcgen");
        lua_pushvalue(L, -1);
        lua_setfield(L, -2, "__index");
        luaL_setfuncs(L, mcplib_funcgen_m, 0);
        lua_settop(L, -(1)-1);
        ...
        lua_settop(L, -(1)-1);
        lua_createtable(L, 0, sizeof(mcplib_f_routes)/sizeof((mcplib_f_routes)[0]) - 1);
    } else {
        *extra = ctx;
        luaL_newmetatable(L, "mcp.backend");
        lua_pushvalue(L, -1);
        lua_setfield(L, -2, "__index");
        luaL_setfuncs(L, mcplib_backend_m, 0);
        lua_settop(L, -(1)-1);
        luaL_newmetatable(L, "mcp.backendwrap");
        lua_pushvalue(L, -1);
        lua_setfield(L, -2, "__index");
        luaL_setfuncs(L, mcplib_backend_wrap_m, 0);
        lua_settop(L, -(1)-1);
        luaL_newmetatable(L, "mcp.pool");
        lua_pushvalue(L, -1);
        lua_setfield(L, -2, "__index");
        luaL_setfuncs(L, mcplib_pool_m, 0);
        lua_settop(L, -(1)-1);
        luaL_newmetatable(L, "mcp.ratelim_global_tbf");
        lua_pushvalue(L, -1);
        lua_setfield(L, -2, "__index");
        luaL_setfuncs(L, mcplib_ratelim_global_tbf_m, 0);
        lua_settop(L, -(1)-1);
        lua_createtable(L, 0, sizeof(mcplib_f_config)/sizeof((mcplib_f_config)[0]) - 1);
    }
    lua_newuserdatauv(L, 1, 0);
    (lua_getfield(L, (-1000000 - 1000), ("mcp.internal_be")));
    lua_setmetatable(L, -2);
    ...
    lua_setmetatable(L, -2);
    if (t != ((void *)0)) {
        luaL_setfuncs(L, mcplib_f_routes, 1);
    } else {
        luaL_setfuncs(L, mcplib_f_config, 1);
    }
