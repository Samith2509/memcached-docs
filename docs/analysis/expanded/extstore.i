...
extern int lockf (int __fd, int __cmd, off_t __len) __attribute__ ((__warn_unused_result__));
extern int lockf64 (int __fd, int __cmd, off64_t __len) __attribute__ ((__warn_unused_result__));
...
struct store_engine {
    ...
    store_io_thread *bg_thread;
    store_page **page_buckets;
    store_page **free_page_buckets;
    size_t page_size;
    unsigned int version;
    ...
    unsigned int page_bucketcount;
    unsigned int free_page_bucketcount;
    unsigned int io_depth;
...
static store_io_thread *_get_io_thread(store_engine *e) {
    int tid = -1;
    long long int low = 0x7fffffffffffffffLL;
...
void *extstore_init(struct extstore_conf_file *fh, struct extstore_conf *cf,
    ...
    pthread_mutex_init(&e->maint.mutex, ((void *)0));
    ...
    e->bg_thread->e = e;
    pthread_mutex_init(&e->bg_thread->mutex, ((void *)0));
    pthread_cond_init(&e->bg_thread->cond, ((void *)0));
    pthread_create(&thread, ((void *)0), extstore_io_thread, e->bg_thread);
...
static void _evict_page(store_engine *e, unsigned int bucket,
    ...
    uint64_t low_version = (0x7fffffffffffffffLL * 2ULL + 1ULL);
    ...
    for (int i = 0; i < e->page_count; i++) {
        ...
        if (st.page_data[i].version == 0) {
            low_version = (0x7fffffffffffffffLL * 2ULL + 1ULL);
            break;
        }
    ...
    if (low_version != (0x7fffffffffffffffLL * 2ULL + 1ULL)) {
...
static store_page *_allocate_page(store_engine *e, unsigned int bucket,
    ...
    if (e->free_page_buckets[free_bucket] != ((void *)0)) {
        ((void) (0));
        tmp = e->free_page_buckets[free_bucket];
        e->free_page_buckets[free_bucket] = tmp->next;
    } else if (e->free_page_buckets[0] != ((void *)0)) {
        ((void) (0));
    ...
    if (tmp != ((void *)0)) {
        ...
        tmp->bucket = bucket;
        e->page_free--;
        { pthread_mutex_lock(&e->stats_mutex); e->stats.page_allocs += 1; pthread_mutex_unlock(&e->stats_mutex); };
    }
    if (tmp)
        ;
    return tmp;
