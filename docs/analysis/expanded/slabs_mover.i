...
struct slab_rebal_thread {
    ...
    pthread_t tid;
    logger *l;
    unsigned int am_version;
    struct timespec am_last;
    slab_automove_reg_t *sam;
    void *active_am;
...
static void slab_rebalance_prep(struct slab_rebal_thread *t) {
    ...
    t->new_it = slab_rebalance_alloc(t, s_clsid, 0);
    if (t->new_it == ((void *)0)) {
        if (!t->allow_evictions && t->rebal.busy_loops > 5000) {
            bool mem_limit_reached = false;
            global_page_pool_size(&mem_limit_reached);
            if (mem_limit_reached) {
                t->allow_evictions = true;
            } else {
                t->new_it = slab_rebalance_alloc(t, s_clsid, 1);
                if (t->new_it) {
                    return;
                }
            }
        ...
        if (t->allow_evictions) {
            for (int x = 0; x < 10; x++) {
                if (lru_pull_tail(s_clsid, 128, 0, 1, 0, ((void *)0)) <= 0) {
                    if (settings.lru_segmented) {
                        lru_pull_tail(s_clsid, 0, 0, 0, 0, ((void *)0));
                    }
                }
                t->new_it = slab_rebalance_alloc(t, s_clsid, 0);
                if (t->new_it != ((void *)0)) {
...
static int _slabs_locked_cb(void *arg) {
    ...
    if (it->it_flags != (4|8)) {
        ...
        } else if ((it->it_flags & 1) != 0) {
            a->hv = hash((((char*)&((it)->data)) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), it->nkey);
            if ((a->hold_lock = item_trylock(a->hv)) == ((void *)0)) {
                status = MOVE_LOCKED;
            } else {
                bool is_linked = (it->it_flags & 1);
...
static void slab_rebalance_rescue(struct slab_rebal_thread *t, struct _locked_st *a) {
    ...
    if (ch == ((void *)0)) {
        ...
        new_it->it_flags &= ~1;
        new_it->refcount = 0;
        do_item_replace(it, new_it, a->hv, (((it)->it_flags & 2) ? (it)->data->cas : (uint64_t)0));
        if (new_it->it_flags & 32) {
            item_chunk *fch = (item_chunk *) ((char*) &((new_it)->data) + (new_it)->nkey + 1 + (((new_it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((new_it)->it_flags & 2) ? sizeof(uint64_t) : 0));
            fch->next->prev = fch;
            while (fch) {
                fch->head = new_it;
                fch = fch->next;
...
static int slab_rebalance_active_rescue(struct slab_rebal_thread *t, struct _locked_st *a) {
    ...
    if (ch == ((void *)0) && (it->it_flags & 32) == 0) {
        ...
        if (t->rebal.busy_loops > 5000) {
            do { logger *myl = t->l; if (t->l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<6)) logger_log(myl, LOGGER_EVICTION, it); } while (0);
...
static int slab_rebalance_check_automove(struct slab_rebal_thread *t,
    ...
    if (src != -1 && dst != -1) {
        ...
        do { logger *myl = t->l; if (t->l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<1)) logger_log(myl, LOGGER_SLAB_MOVE, ((void *)0), src, dst, msg); } while (0);
        if (dst != 0) {
...
struct slab_rebal_thread *start_slab_maintenance_thread(void *storage) {
    ...
    if (t == ((void *)0))
        return ((void *)0);
    pthread_mutex_init(&t->lock, ((void *)0));
