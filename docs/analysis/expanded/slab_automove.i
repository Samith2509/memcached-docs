...
typedef struct {
    ...
    item_stats_automove iam_before[(63 + 1)];
    item_stats_automove iam_after[(63 + 1)];
    slab_stats_automove sam_before[(63 + 1)];
    slab_stats_automove sam_after[(63 + 1)];
} slab_automove;
void *slab_automove_init(struct settings *settings) {
    ...
    if (a == ((void *)0))
        return ((void *)0);
    a->window_data = calloc(window_size * (63 + 1), sizeof(struct window_data));
...
void slab_automove_run(void *arg, int *src, int *dst) {
    ...
    *dst = -1;
    ...
    for (n = 1; n < (63 + 1); n++) {
        ...
        if (a->sam_after[n].free_chunks > a->sam_after[n].chunks_per_page * 2.5) {
        ...
        if (age > oldest_age && a->sam_after[n].total_pages > 2) {
            oldest = n;
            oldest_age = age;
        }
    ...
    memcpy(a->iam_before, a->iam_after,
            sizeof(item_stats_automove) * (63 + 1));
    memcpy(a->sam_before, a->sam_after,
            sizeof(slab_stats_automove) * (63 + 1));
