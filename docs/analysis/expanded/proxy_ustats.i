...
int mcplib_add_stat(lua_State *L) {
    ...
    if (idx > ctx->tunables.max_ustats) {
        do { lua_pushfstring(L, "stat index must be %d or less", ctx->tunables.max_ustats); lua_error(L); } while (0);
        return 0;
    }
    if (strlen(name) > 128 - 6) {
        do { lua_pushfstring(L, "stat name too long: %s\n", name); lua_error(L); } while (0);
        return 0;
    }
    for (int x = 0; x < strlen(name); x++) {
        if (((*__ctype_b_loc ())[(int) ((name[x]))] & (unsigned short int) _ISspace)) {
