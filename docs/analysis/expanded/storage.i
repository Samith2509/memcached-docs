...
bool storage_validate_item(void *e, item *it) {
    item_hdr *hdr = (item_hdr *)((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0));
...
void storage_delete(void *e, item *it) {
    if (it->it_flags & 128) {
        ...
        extstore_delete(e, hdr->page_id, hdr->page_version,
                1, (sizeof(struct _stritem) + (it)->nkey + 1 + (it)->nbytes + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)));
    }
}
void process_extstore_stats(ADD_STAT add_stats, void *c) {
    int i;
    ...
    for (i = 0; i < st.page_count; i++) {
        klen = snprintf(key_str, 128, "%d:%s", i, "version"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long) st.page_data[i].version); add_stats(key_str, klen, val_str, vlen, c);;
        klen = snprintf(key_str, 128, "%d:%s", i, "bytes"); vlen = snprintf(val_str, 128, "%llu", (unsigned long long) st.page_data[i].bytes_used); add_stats(key_str, klen, val_str, vlen, c);;
        klen = snprintf(key_str, 128, "%d:%s", i, "bucket"); vlen = snprintf(val_str, 128, "%u", st.page_data[i].bucket); add_stats(key_str, klen, val_str, vlen, c);;
        klen = snprintf(key_str, 128, "%d:%s", i, "free_bucket"); vlen = snprintf(val_str, 128, "%u", st.page_data[i].free_bucket); add_stats(key_str, klen, val_str, vlen, c);;
    }
    free(st.page_data);
}
void storage_stats(ADD_STAT add_stats, void *c) {
    ...
    if (ext_storage) {
        STATS_LOCK();
        append_stat("extstore_memory_pressure", add_stats, c, "%.2f", stats_state.extstore_memory_pressure);;
        append_stat("extstore_compact_lost", add_stats, c, "%llu", (unsigned long long)stats.extstore_compact_lost);;
        append_stat("extstore_compact_rescues", add_stats, c, "%llu", (unsigned long long)stats.extstore_compact_rescues);;
        append_stat("extstore_compact_resc_cold", add_stats, c, "%llu", (unsigned long long)stats.extstore_compact_resc_cold);;
        append_stat("extstore_compact_resc_old", add_stats, c, "%llu", (unsigned long long)stats.extstore_compact_resc_old);;
        append_stat("extstore_compact_skipped", add_stats, c, "%llu", (unsigned long long)stats.extstore_compact_skipped);;
        STATS_UNLOCK();
        extstore_get_stats(ext_storage, &st);
        append_stat("extstore_page_allocs", add_stats, c, "%llu", (unsigned long long)st.page_allocs);;
        append_stat("extstore_page_evictions", add_stats, c, "%llu", (unsigned long long)st.page_evictions);;
        append_stat("extstore_page_reclaims", add_stats, c, "%llu", (unsigned long long)st.page_reclaims);;
        append_stat("extstore_pages_free", add_stats, c, "%llu", (unsigned long long)st.pages_free);;
        append_stat("extstore_pages_used", add_stats, c, "%llu", (unsigned long long)st.pages_used);;
        append_stat("extstore_objects_evicted", add_stats, c, "%llu", (unsigned long long)st.objects_evicted);;
        append_stat("extstore_objects_read", add_stats, c, "%llu", (unsigned long long)st.objects_read);;
        append_stat("extstore_objects_written", add_stats, c, "%llu", (unsigned long long)st.objects_written);;
        append_stat("extstore_objects_used", add_stats, c, "%llu", (unsigned long long)st.objects_used);;
        append_stat("extstore_bytes_evicted", add_stats, c, "%llu", (unsigned long long)st.bytes_evicted);;
        append_stat("extstore_bytes_written", add_stats, c, "%llu", (unsigned long long)st.bytes_written);;
        append_stat("extstore_bytes_read", add_stats, c, "%llu", (unsigned long long)st.bytes_read);;
        append_stat("extstore_bytes_used", add_stats, c, "%llu", (unsigned long long)st.bytes_used);;
        append_stat("extstore_bytes_fragmented", add_stats, c, "%llu", (unsigned long long)st.bytes_fragmented);;
        append_stat("extstore_limit_maxbytes", add_stats, c, "%llu", (unsigned long long)(st.page_count * st.page_size));;
        append_stat("extstore_io_queue", add_stats, c, "%llu", (unsigned long long)(st.io_queue));;
    }
}
static void _storage_get_item_cb(void *e, obj_io *io, int ret) {
    ...
    if (ret < 1) {
        ...
        int x;
        if (io->iov != ((void *)0)) {
            crc2 = crc32c(0, (char *)io->iov[0].iov_base+__builtin_offsetof (item, nbytes), io->iov[0].iov_len-__builtin_offsetof (item, nbytes));
            io->iov[0].iov_len = 0;
            for (x = 1; x < io->iovcnt; x++) {
                crc2 = crc32c(crc2, (char *)io->iov[x].iov_base, io->iov[x].iov_len);
            }
        } else {
            crc2 = crc32c(0, (char *)read_it+__builtin_offsetof (item, nbytes), io->len-__builtin_offsetof (item, nbytes));
        }
        if (crc != crc2) {
            miss = true;
            p->badcrc = true;
    ...
    if (miss) {
        if (p->noreply) {
            ...
            if (resp->binary_prot) {
                ...
                uint32_t body_len = __bswap_32 (header->response.bodylen);
                ...
                header->response.status = (uint16_t)__bswap_16 (PROTOCOL_BINARY_RESPONSE_KEY_ENOENT);
                header->response.bodylen = __bswap_32 (body_len);
        ...
        ((void) (0));
        if ((read_it->it_flags & 32) == 0) {
            resp->iov[p->iovec_data].iov_base = ((char*) &((read_it)->data) + (read_it)->nkey + 1 + (((read_it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((read_it)->it_flags & 2) ? sizeof(uint64_t) : 0));
        }
        p->miss = false;
    }
    p->active = false;
...
int storage_get_item(LIBEVENT_THREAD *t, item *it, mc_resp *resp) {
    item_hdr *hdr = (item_hdr *)((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0));
    io_queue_t *q = thread_io_queue_get(t, 1);
    size_t ntotal = (sizeof(struct _stritem) + (it)->nkey + 1 + (it)->nbytes + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0));
    ...
    if (ntotal > settings.slab_chunk_size_max) {
        ...
        new_it = item_alloc((((char*)&((it)->data)) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), it->nkey, flags, it->exptime, it->nbytes);
        ((void) (0));
        chunked = true;
    } else {
    ...
    p->resp = resp;
    p->io_queue_type = 1;
    p->payload = __builtin_offsetof (io_pending_storage_t, io_ctx);
    obj_io *eio = &p->io_ctx;
    if (chunked) {
        unsigned int ciovcnt = 0;
        size_t remain = new_it->nbytes;
        item_chunk *chunk = (item_chunk *) ((char*) &((new_it)->data) + (new_it)->nkey + 1 + (((new_it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((new_it)->it_flags & 2) ? sizeof(uint64_t) : 0));
        eio->iov = malloc(sizeof(struct iovec) * 1024);
        if (eio->iov == ((void *)0)) {
            item_remove(new_it);
        ...
        eio->iov[0].iov_len = (sizeof(struct _stritem) + (new_it)->nkey + 1 + (new_it)->nbytes + (((new_it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((new_it)->it_flags & 2) ? sizeof(uint64_t) : 0)) - new_it->nbytes;
        ciovcnt++;
        while (remain > 0) {
            chunk = do_item_alloc_chunk(chunk, remain);
            if (chunk == ((void *)0) || ciovcnt > 1024 -1) {
                item_remove(new_it);
                free(eio->iov);
                eio->iov = ((void *)0);
                do_cache_free(t->io_cache, p);
    ...
    eio->buf = (void *)new_it;
    do { ((((io_pending_t *)p))->iop_next.stqe_next) = ((void *)0); *(&q->stack)->stqh_last = ((io_pending_t *)p); (&q->stack)->stqh_last = &((((io_pending_t *)p))->iop_next.stqe_next); } while (0);
    eio->data = (void *)p;
...
void storage_submit_cb(io_queue_t *q) {
    void *eio_head = ((void *)0);
    while(!((&q->stack)->stqh_first == ((void *)0))) {
        io_pending_t *p = ((&q->stack)->stqh_first);
        do { if (((((&q->stack))->stqh_first) = (((((&q->stack))->stqh_first))->iop_next.stqe_next)) == ((void *)0)) (&q->stack)->stqh_last = &(((&q->stack))->stqh_first); } while (0);
...
static void recache_or_free(io_pending_t *pending) {
    ...
    if (p->active) {
        do_free = false;
        size_t ntotal = (sizeof(struct _stritem) + (p->hdr_it)->nkey + 1 + (p->hdr_it)->nbytes + (((p->hdr_it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((p->hdr_it)->it_flags & 2) ? sizeof(uint64_t) : 0));
        slabs_free(it, slabs_clsid(ntotal));
        p->resp->suspended = false;
        c->resps_suspended--;
        io_queue_t *q = thread_io_queue_get(p->thread, p->io_queue_type);
        do { ; if ((((&q->stack))->stqh_first) == (pending)) { do { if ((((((&q->stack)))->stqh_first) = ((((((&q->stack)))->stqh_first))->iop_next.stqe_next)) == ((void *)0)) ((&q->stack))->stqh_last = &((((&q->stack)))->stqh_first); } while (0); } else { struct _io_pending_t *curelm = ((&q->stack)->stqh_first); while (((curelm)->iop_next.stqe_next) != (pending)) curelm = ((curelm)->iop_next.stqe_next); do { if ((((curelm)->iop_next.stqe_next) = ((((curelm)->iop_next.stqe_next))->iop_next.stqe_next)) == ((void *)0)) (&q->stack)->stqh_last = &(((curelm))->iop_next.stqe_next); } while (0); } ; } while (0);
        ...
        if (hold_lock != ((void *)0)) {
            item *h_it = p->hdr_it;
            uint8_t flags = 1|8|16;
            if (((h_it->it_flags & flags) == flags) &&
                    h_it->time > current_time - 60 &&
                    c->recache_counter++ % settings.ext_recache_rate == 0) {
                do_free = false;
                it->exptime = h_it->exptime;
                it->it_flags &= ~1;
                it->it_flags |= (h_it->it_flags & ((8|512|2048|4096)));
                it->refcount = 0;
                it->h_next = ((void *)0);
                do { storage_delete(c->thread->storage, h_it); } while (0);
                item_replace(h_it, it, hv, (((h_it)->it_flags & 2) ? (h_it)->data->cas : (uint64_t)0));
                pthread_mutex_lock(&c->thread->stats.mutex);
    ...
    if (do_free)
        slabs_free(it, ((it)->slabs_clsid & ~(3<<6)));
    p->io_ctx.buf = ((void *)0);
    p->io_ctx.next = ((void *)0);
    p->active = false;
    item_remove(p->hdr_it);
}
static void storage_return_cb(io_pending_t *pending) {
    do { pending->resp->suspended = false; pending->c->resps_suspended--; ((void) (0)); if (pending->c->resps_suspended == 0) { conn_worker_readd(pending->c); } } while (0);
}
static void storage_finalize_cb(io_pending_t *pending) {
...
static int storage_write(void *storage, const int clsid, const int item_age) {
    ...
    it_info.it = ((void *)0);
    lru_pull_tail(clsid, 128, 0, 4, 0, &it_info);
    if (it_info.it == ((void *)0)) {
        return did_moves;
    }
    obj_io io;
    item *it = it_info.it;
    size_t orig_ntotal = (sizeof(struct _stritem) + (it)->nkey + 1 + (it)->nbytes + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0));
    client_flags_t flags;
    if ((it->it_flags & 128) == 0 &&
            (item_age == 0 || current_time - it->time > item_age)) {
        { if ((it)->it_flags & 256) { flags = *((client_flags_t *)((char*) &(((it))->data) + ((it))->nkey + 1 + ((((it))->it_flags & 2) ? sizeof(uint64_t) : 0))); } else { flags = 0; } };
        item *hdr_it = do_item_alloc((((char*)&((it)->data)) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), it->nkey, flags, it->exptime, sizeof(item_hdr));
        if (hdr_it != ((void *)0)) {
            int bucket = (it->it_flags & 32) ?
                2 : 0;
            ...
            if (extstore_write_request(storage, bucket, bucket, &io) == 0) {
                ...
                if (it->it_flags & 32) {
                    item_chunk *sch = (item_chunk *) ((char*) &((it)->data) + (it)->nkey + 1 + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0));
                    int remain = orig_ntotal;
                    int copied = 0;
                    int hdrtotal = (sizeof(struct _stritem) + (it)->nkey + 1 + (it)->nbytes + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)) - it->nbytes;
                    memcpy((char *)io.buf+__builtin_offsetof (item, nbytes), (char *)it+__builtin_offsetof (item, nbytes), hdrtotal - __builtin_offsetof (item, nbytes));
                    copied = hdrtotal;
                    while (sch && remain) {
                        ((void) (0));
                    ...
                    memcpy((char *)io.buf+__builtin_offsetof (item, nbytes), (char *)it+__builtin_offsetof (item, nbytes), io.len-__builtin_offsetof (item, nbytes));
                }
                buf_it->it_flags &= ~1;
                buf_it->exptime = crc32c(0, (char*)io.buf+__builtin_offsetof (item, nbytes), orig_ntotal-__builtin_offsetof (item, nbytes));
                extstore_write(storage, &io);
                item_hdr *hdr = (item_hdr *) ((char*) &((hdr_it)->data) + (hdr_it)->nkey + 1 + (((hdr_it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((hdr_it)->it_flags & 2) ? sizeof(uint64_t) : 0));
                hdr->page_version = io.page_version;
                hdr->page_id = io.page_id;
                hdr->offset = io.offset;
                hdr_it->nbytes = it->nbytes;
                hdr_it->it_flags |= (it->it_flags & ((8|512|2048|4096)));
                item_replace(it, hdr_it, it_info.hv, (((it)->it_flags & 2) ? (it)->data->cas : (uint64_t)0));
                do_item_remove(hdr_it);
                did_moves = 1;
                do { logger *myl = ((void *)0); if (((void *)0) == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<6)) logger_log(myl, LOGGER_EXTSTORE_WRITE, it, bucket); } while (0);
            } else {
                slabs_free(hdr_it, ((hdr_it)->slabs_clsid & ~(3<<6)));
            }
        }
    }
    do_item_remove(it);
...
static void *storage_write_thread(void *arg) {
    void *storage = arg;
    unsigned int backoff[(63 + 1)] = {0};
    unsigned int counter = 0;
    useconds_t to_sleep = 200;
    ...
    while (1) {
        ...
        unsigned int global_pages = global_page_pool_size(((void *)0));
        bool do_sleep = true;
        int target_pages = 0;
...
int start_storage_write_thread(void *arg) {
    int ret;
    pthread_mutex_init(&storage_write_plock, ((void *)0));
    if ((ret = pthread_create(&storage_write_tid, ((void *)0),
...
static int storage_compact_check(void *storage, logger *l,
    ...
    struct extstore_stats st;
    _storage_buk buckets[6];
    _storage_buk *buk = ((void *)0);
    uint64_t frag_limit;
    extstore_get_stats(storage, &st);
    if (st.pages_used == 0)
        return 0;
    for (int x = 0; x < 6; x++) {
        ...
        buckets[x].low_version = (0x7fffffffffffffffLL * 2ULL + 1ULL);
        buckets[x].lowest_version = (0x7fffffffffffffffLL * 2ULL + 1ULL);
    }
    flags->drop_unread = 0;
    frag_limit = st.page_size * settings.ext_max_frag;
    do { logger *myl = l; if (l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<1)) logger_log(myl, LOGGER_COMPACT_FRAGINFO, ((void *)0), settings.ext_max_frag, frag_limit); } while (0);
    st.page_data = calloc(st.page_count, sizeof(struct extstore_page_data));
    extstore_get_page_data(storage, &st);
    ...
    *page_size = st.page_size;
    ...
    if (buk->pages_total != 0) {
        ...
        if (buk->pages_free == 0 && buk->lowest_version != (0x7fffffffffffffffLL * 2ULL + 1ULL)) {
...
static void storage_compact_readback(void *storage, logger *l,
    ...
    while (offset < read_size) {
        ...
        ntotal = (sizeof(struct _stritem) + (it)->nkey + 1 + (it)->nbytes + (((it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0));
        uint32_t hv = (uint32_t)it->time;
        item_lock(hv);
        hdr_it = assoc_find((((char*)&((it)->data)) + (((it)->it_flags & 2) ? sizeof(uint64_t) : 0)), it->nkey, hv);
        if (hdr_it != ((void *)0)) {
            bool do_write = false;
            int bucket = flags.use_old ? 5 : 1;
            ++(hdr_it->refcount);
            if ((hdr_it->it_flags & 128) && !item_is_flushed(hdr_it) &&
                   (hdr_it->exptime == 0 || hdr_it->exptime > current_time)) {
                hdr = (item_hdr *)((char*) &((hdr_it)->data) + (hdr_it)->nkey + 1 + (((hdr_it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((hdr_it)->it_flags & 2) ? sizeof(uint64_t) : 0));
            ...
            if (do_write) {
                ...
                if (do_update) {
                    ...
                    if (hdr_it->refcount == 2) {
                        ...
                        client_flags_t flags;
                        { if ((hdr_it)->it_flags & 256) { flags = *((client_flags_t *)((char*) &(((hdr_it))->data) + ((hdr_it))->nkey + 1 + ((((hdr_it))->it_flags & 2) ? sizeof(uint64_t) : 0))); } else { flags = 0; } };
                        item *new_it = do_item_alloc((((char*)&((hdr_it)->data)) + (((hdr_it)->it_flags & 2) ? sizeof(uint64_t) : 0)), hdr_it->nkey, flags, hdr_it->exptime, sizeof(item_hdr));
                        if (new_it) {
                            new_it->it_flags = hdr_it->it_flags & (~1);
                            new_it->time = hdr_it->time;
                            new_it->nbytes = hdr_it->nbytes;
                            item_hdr *new_hdr = (item_hdr *) ((char*) &((new_it)->data) + (new_it)->nkey + 1 + (((new_it)->it_flags & 256) ? sizeof(client_flags_t) : 0) + (((new_it)->it_flags & 2) ? sizeof(uint64_t) : 0));
                            new_hdr->page_version = io.page_version;
                            new_hdr->page_id = io.page_id;
                            new_hdr->offset = io.offset;
                            item_replace(hdr_it, new_it, hv, (((hdr_it)->it_flags & 2) ? (hdr_it)->data->cas : (uint64_t)0));
    ...
    STATS_UNLOCK();
    do { logger *myl = l; if (l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<1)) logger_log(myl, LOGGER_COMPACT_READ_END, ((void *)0), page_id, offset, rescues, rescues_realloc, lost, lost_oom, skipped); } while (0);
}
...
static void *storage_compact_thread(void *arg) {
    ...
    while (1) {
        if (!compacting && storage_compact_check(storage, l,
            ...
            page_offset = 0;
            compacting = true;
            do { logger *myl = l; if (l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<1)) logger_log(myl, LOGGER_COMPACT_START, ((void *)0), page_id, page_version); } while (0);
        } else {
        ...
        while (compacting) {
            ...
            if (page_offset < page_size && !wrap.done && !wrap.submitted) {
                ...
                do { logger *myl = l; if (l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<1)) logger_log(myl, LOGGER_COMPACT_ABORT, ((void *)0), page_id); } while (0);
                wrap.done = false;
                wrap.submitted = false;
                compacting = false;
            } else if (wrap.submitted && wrap.done) {
                do { logger *myl = l; if (l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<1)) logger_log(myl, LOGGER_COMPACT_READ_START, ((void *)0), page_id, page_offset); } while (0);
                ...
                do { logger *myl = l; if (l == ((void *)0)) myl = ((logger *) pthread_getspecific(logger_key));; if (myl->eflags & (1<<1)) logger_log(myl, LOGGER_COMPACT_END, ((void *)0), page_id); } while (0);
                usleep(1000);
            }
            pthread_mutex_unlock(&wrap.lock);
        }
...
int start_storage_compact_thread(void *arg) {
    int ret;
    pthread_mutex_init(&storage_compact_plock, ((void *)0));
    pthread_cond_init(&storage_compact_cond, ((void *)0));
    if ((ret = pthread_create(&storage_compact_tid, ((void *)0),
...
struct extstore_conf_file *storage_conf_parse(char *arg) {
    struct extstore_conf_file *cf = ((void *)0);
    char *b = ((void *)0);
    ...
    unit = (__extension__ ({ int __res; if (sizeof (p[strlen(p)-1]) > 1) { if (__builtin_constant_p (p[strlen(p)-1])) { int __c = (p[strlen(p)-1]); __res = __c < -128 || __c > 255 ? __c : (*__ctype_tolower_loc ())[__c]; } else __res = tolower (p[strlen(p)-1]); } else __res = (*__ctype_tolower_loc ())[(int) (p[strlen(p)-1])]; __res; }));
    p[strlen(p)-1] = '\0';
    switch (unit) {
        case 'm':
...
void *storage_init_config(struct settings *s) {
    ...
    s->ext_item_size = 512;
    s->ext_item_age = (0x7fffffff * 2U + 1U);
    s->ext_low_ttl = 0;
